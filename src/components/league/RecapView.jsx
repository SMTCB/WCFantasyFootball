...
export default function RecapView({ leagueId, tournamentId, members, currentUser }) {
  const [matchday, setMatchday]     = useState(null); // e.g. '426-r6'
  const [scores, setScores]         = useState([]);   // [{ user_id, username, pts, rank }]
  const [fixtures, setFixtures]     = useState([]);
  const [loading, setLoading]       = useState(true);

  useEffect(() => {
    if (!leagueId || !tournamentId) { setLoading(false); return; }
    let cancelled = false;

    (async () => {
      setLoading(true);

      // 1. Latest completed matchday for this tournament
      ...
      if (!deadlineRow?.matchday_id || cancelled) { setLoading(false); return; }
      const md = deadlineRow.matchday_id;
      if (!cancelled) setMatchday(md);

      // 2. All squads in this league
      const { data: squadRows } = await supabase
        .from('squads')
        .select('id, user_id')
        .eq('league_id', leagueId);

      if (!squadRows?.length || cancelled) { setLoading(false); return; }

      // Deduplicate: keep one squad per user (latest created)
      const latestByUser = {};
      squadRows.forEach(s => {
        if (!latestByUser[s.user_id]) latestByUser[s.user_id] = s.id;
      });
      const squadIds = Object.values(latestByUser);

      // 3. Fantasy points for those squads this matchday
      ...
      // Build scored list
      const fpMap = Object.fromEntries((fpRows ?? []).map(r => [r.squad_id, Number(r.total_points)]));
  ...
  }, [leagueId, tournamentId, members]);

  const roundLabel = matchday ? `GW ${String(matchday).replace(/^.*-r/, '')}` : '—';
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="hidden lg:grid" style={{ flex: 1, gridTemplateColumns: '1fr 380px', minHeight: 0 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--rule)' }}>
          ...
          />
          {/* Header row */}
          <div style={{ display: 'grid', gridTemplateColumns: '40px 1fr 80px 80px', gap: 12, padding: '10px 24px', borderBottom: '1px solid var(--rule)', color: 'var(--mute)' }}>
                ...
                return (
                  <div key={s.user_id} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                      ...
                      <span style={{ fontFamily: DISPLAY, fontSize: 13, letterSpacing: '-0.01em', whiteSpace: 'nowrap' }}>{s.username}</span>
                      {isTop && <span style={{ fontFamily: MONO, fontSize: 8, background: 'var(--gold)', color: 'var(--ink)', padding: '1px 4px', letterSpacing: '.1em' }}>TOP</span>}
                    </div>
                    ...
                    <div style={{ textAlign: 'right', fontFamily: MONO, fontSize: 11, color: 'var(--mute)' }}>
                      {s.rank != null ? Math.round(members.find(m => m.user_id === s.user_id)?.total_points ?? 0) : '—'}
                    </div>
                  </div>
                );
              })}
            ...
            return (
              <div key={f.id} style={{ padding: '10px 18px', borderBottom: '1px solid var(--rule)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                  ...
                  <span style={{ fontFamily: MONO, fontSize: 10, color: 'var(--paper)', flex: 1, textAlign: 'left', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{f.away_team}</span>
                </div>
                {live && (
                  <div style={{ textAlign: 'center', marginTop: 3 }}>
                    <span style={{ fontFamily: MONO, fontSize: 8, color: 'var(--danger)', letterSpacing: '.18em' }}>● LIVE</span>
                  </div>
                )}
                {finished && (
                  <div style={{ textAlign: 'center', marginTop: 3 }}>
                    <span style={{ fontFamily: MONO, fontSize: 8, color: 'var(--mute)', letterSpacing: '.14em' }}>FT</span>
                  </div>
                )}
              </div>
            );
