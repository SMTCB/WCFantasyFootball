...
function MiniChart({ history }) {
  if (!history?.length) return <div style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)' }}>— NO DATA YET —</div>;
      ...
      {history.map((r, i) => (
        <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, justifyContent: 'flex-end' }}>
          <div style={{
            ...
            <span style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--ink)', letterSpacing: 0 }}>{r.totalPts}</span>
          </div>
          <div style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)', letterSpacing: '.06em' }}>{r.gw}</div>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', borderBottom: '1px solid var(--rule)', flexShrink: 0 }}>
      <div style={{ flex: '0 0 88px', padding: compact ? '7px 12px' : '10px 16px', borderRight: '1px solid var(--rule)', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 2 }}>
        <div style={{ ...MO, fontSize: 'var(--fs-micro)', color: posC, letterSpacing: '.22em' }}>POS STATS</div>
      ...
      {posStats.map((s, i) => (
        <div key={i} style={{ flex: 1, padding: compact ? '7px 10px' : '10px 14px', borderRight: i < posStats.length - 1 ? '1px solid var(--rule)' : 'none' }}>
          <div style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)', marginBottom: compact ? 2 : 3, letterSpacing: '.18em' }}>{s.label}</div>
...
function GWRow({ r, selected, onClick, columns, fontSize }) {
  return (
    <div onClick={onClick} style={{
      ...
      <span style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)' }}>{r.gw}</span>
      <span style={{ fontFamily: 'Archivo, sans-serif', fontSize }}>
        {r.opponent}{r.isHome ? ' (H)' : ' (A)'}
      </span>
      <span style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)' }}>{Math.round(r.minutes_played || 0)}'</span>
    ...
    <>
      {items.map((item, i) => (
        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: '1px solid var(--rule)' }}>
          <span style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)', letterSpacing: '.1em' }}>{item.label}</span>
          <span style={{ fontFamily: 'Archivo Black, sans-serif', fontSize: 'var(--fs-body-lg)', letterSpacing: '-0.01em', color: item.kind === 'pos' ? 'var(--positive)' : item.kind === 'bonus' ? 'var(--gold)' : 'var(--danger)' }}>
            {fmtPts(item.pts)}
          </span>
        </div>
      ))}
      {!items.length && (
        <div style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)', padding: '12px 0' }}>NO SCORING DATA FOR THIS GW</div>
      )}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', padding: '11px 0 4px', marginTop: 4, borderTop: '2px solid var(--rule)' }}>
        <div>
          <span style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--paper)' }}>TOTAL</span>
          <div style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)', marginTop: 2, letterSpacing: '.16em' }}>ROUNDED TO NEAREST PT</div>
        </div>
        <span style={{ fontFamily: 'Archivo Black, sans-serif', fontSize: 'var(--fs-title)', color: 'var(--positive)', letterSpacing: '-0.02em' }}>~{sel.totalPts}</span>
      </div>
    </>
  );
}
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center" style={{ background: 'rgba(6,7,10,.86)' }} onClick={onClose}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 20px', borderBottom: '1px solid var(--rule)', flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: 'Archivo Black, sans-serif', fontSize: 'var(--fs-heading)', letterSpacing: '-0.01em', textTransform: 'uppercase' }}>{player.name}</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 3 }}>
              <StatusDot status={status} />
              <span style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)' }}>{player.club}</span>
              <span style={{ ...MO, fontSize: 'var(--fs-micro)', color: STATUS_C[status] }}>· {STATUS_L[status]}</span>
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontFamily: 'Archivo Black, sans-serif', fontSize: 'var(--fs-heading)' }}>€{Number(player.price || 0).toFixed(1)}M</div>
            <div style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)', marginTop: 2 }}>PRICE</div>
          </div>
          <button onClick={onClose} style={{ width: 24, height: 24, border: '1px solid var(--rule)', color: 'var(--mute)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 'var(--fs-body)', background: 'transparent', flexShrink: 0 }}>✕</button>
        </div>
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0 }}>
          ...
          <div style={{ flex: '0 0 55%', borderRight: '1px solid var(--rule)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)', padding: '8px 16px', borderBottom: '1px solid var(--rule)', flexShrink: 0, letterSpacing: '.2em' }}>GW HISTORY</div>
            <div style={{ display: 'grid', gridTemplateColumns: '44px 1fr 54px 36px 36px 50px', padding: '6px 16px', borderBottom: '1px solid var(--rule)', ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)', flexShrink: 0 }}>
            ...
            <div style={{ flex: 1, overflow: 'auto' }}>
              {loading && <div style={{ padding: '18px 16px', ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)' }}>LOADING…</div>}
              ...
              {!loading && !gwHistory.length && (
                <div style={{ padding: '18px 16px', ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)' }}>— NO GAMEWEEKS PLAYED —</div>
              )}
            </div>
            <div style={{ borderTop: '1px solid var(--rule)', padding: '10px 16px', flexShrink: 0 }}>
              <div style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)', marginBottom: 8, letterSpacing: '.2em' }}>POINTS HISTORY</div>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 16px', borderBottom: '1px solid var(--rule)', flexShrink: 0 }}>
              <span style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)', letterSpacing: '.2em' }}>POINT BREAKDOWN</span>
              {sel ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--cyan)', background: 'rgba(0,180,216,.1)', border: '1px solid rgba(0,180,216,.25)', padding: '3px 8px', letterSpacing: '.12em' }}>{sel.gw}</div>
                  <span style={{ fontFamily: 'Archivo, sans-serif', fontSize: 'var(--fs-micro)', color: 'var(--mute)' }}>{sel.opponent}{sel.isHome ? ' (H)' : ' (A)'}</span>
                </div>
              ) : (
                <span style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)', letterSpacing: '.12em' }}>SELECT A ROW ←</span>
              )}
            </div>
            {sel ? (
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: .5 }}>
                <div style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)' }}>TAP ANY GW ROW</div>
                <div style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)', letterSpacing: '.1em' }}>TO SEE POINT BREAKDOWN</div>
              </div>
            )}
          </div>
        </div>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', borderBottom: '1px solid var(--rule)', flexShrink: 0 }}>
          <div onClick={onClose} style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--cyan)', cursor: 'pointer' }}>← BACK</div>
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
            <PosBadge pos={player.position} small />
            <span style={{ fontFamily: 'Archivo Black, sans-serif', fontSize: 'var(--fs-body)', letterSpacing: '-0.01em', textTransform: 'uppercase', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{player.name}</span>
          </div>
          <span style={{ fontFamily: 'Archivo Black, sans-serif', fontSize: 'var(--fs-body)', flexShrink: 0 }}>€{Number(player.price || 0).toFixed(1)}M</span>
        </div>
        {/* meta */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '7px 16px', borderBottom: '1px solid var(--rule)', flexShrink: 0 }}>
          ...
          <span style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)' }}>{player.club}</span>
          <span style={{ ...MO, fontSize: 'var(--fs-micro)', color: STATUS_C[status] }}>· {STATUS_L[status]}</span>
          <span style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)', marginLeft: 'auto' }}>OWNED {ownershipPct ?? 0}%</span>
        </div>
        {/* stats strip */}
        ...
        <div style={{ display: 'flex', gap: 20, padding: '9px 16px 0', borderBottom: '1px solid var(--rule)', flexShrink: 0 }}>
          {['HISTORY', 'BREAKDOWN'].map(t => (
            <div key={t} onClick={() => setMobileTab(t.toLowerCase())} style={{ ...MO, fontSize: 'var(--fs-micro)', paddingBottom: 7, cursor: 'pointer', position: 'relative', color: mobileTab === t.toLowerCase() ? 'var(--paper)' : 'var(--mute)' }}>
              {t}
        ...
        <div style={{ flex: 1, overflow: 'auto' }}>
          {mobileTab === 'history' && <>
            <div style={{ display: 'grid', gridTemplateColumns: '34px 1fr 44px 28px 28px 38px', padding: '6px 16px', borderBottom: '1px solid var(--rule)', position: 'sticky', top: 0, background: 'var(--ink)', ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)' }}>
              <span>GW</span><span>FIXTURE</span><span>MIN</span>
            ...
            <div style={{ padding: '12px 16px' }}>
              {sel ? <>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                  <div style={{ ...MO, fontSize: 'var(--fs-micro)', color: 'var(--cyan)', background: 'rgba(0,180,216,.1)', border: '1px solid rgba(0,180,216,.25)', padding: '3px 8px' }}>{sel.gw}</div>
                  <span style={{ fontFamily: 'Archivo, sans-serif', fontSize: 'var(--fs-micro)', color: 'var(--mute)' }}>{sel.opponent}{sel.isHome ? ' (H)' : ' (A)'} · {Math.round(sel.minutes_played || 0)}'</span>
                </div>
                <BreakdownItems sel={sel} items={breakdownItems} />
              </> : (
                <div style={{ padding: '20px 0', ...MO, fontSize: 'var(--fs-micro)', color: 'var(--mute)' }}>GO TO HISTORY AND TAP A GAMEWEEK</div>
              )}
            </div>
          )}
        </div>
