...
export default function F1ReportScreen() {
  ...
  if (loading) {
    return <div style={{ textAlign: 'center', padding: 40, color: 'var(--mute)', fontFamily: 'JetBrains Mono, monospace', fontSize: 'var(--fs-micro)' }}>LOADING…</div>;
  }
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      <div style={{ background: 'var(--shell)', padding: '16px 16px 12px' }}>
        ...
        <h1 style={{ fontFamily: 'Archivo Black, sans-serif', fontSize: 'var(--fs-title)', color: 'var(--on-shell)', margin: 0 }}>RESULTS</h1>
      </div>
          ...
          {races.map(race => {
            ...
            return (
              <div key={race.id} style={{ background: 'var(--card)', border: '1px solid var(--rule)', borderRadius: 8, overflow: 'hidden' }}>
                ...
                >
                  <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 'var(--fs-micro)', color: 'var(--mute)', minWidth: 24 }}>R{race.round_number}</span>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 'var(--fs-micro)', color: 'var(--mute)', marginTop: 1 }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    {score ? (
                      <>
                        <div style={{ fontFamily: 'Archivo Black, sans-serif', fontSize: 'var(--fs-heading)', color: 'var(--f1)' }}>{score.total_points}</div>
                        <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 'var(--fs-micro)', color: 'var(--mute)' }}>PTS</div>
                      </>
                    ) : (
                      <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 'var(--fs-micro)', color: 'var(--mute)' }}>—</div>
                    )}
                  </div>
                  <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 'var(--fs-label)', color: 'var(--mute)', marginLeft: 4 }}>{isOpen ? '▴' : '▾'}</span>
                </button>
                ...
                {isOpen && (
                  <div style={{ borderTop: '1px solid var(--rule)', padding: '12px 14px' }}>
                    ...
                    <div style={{ marginBottom: 12 }}>
                      <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 'var(--fs-micro)', letterSpacing: '0.12em', color: 'var(--mute)', marginBottom: 6 }}>RACE RESULT</div>
                        ...
                        {[
                          ...
                          <div key={label} style={{ display: 'flex', flexDirection: 'column' }}>
                            ...
                            <div style={{ padding: '8px 10px', background: 'var(--elev)', borderRadius: '0 0 6px 6px' }}>
                              <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 'var(--fs-micro)', color: 'var(--mute)', marginBottom: 3 }}>{label}</div>
                    ...
                    {bet && score && (
                      <div>
                        <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 'var(--fs-micro)', letterSpacing: '0.12em', color: 'var(--mute)', marginBottom: 6 }}>MY PICKS</div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                          {[
                            ...
                            <div key={row.key} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', background: row.pts > 0 ? 'rgba(22,101,52,0.06)' : 'rgba(185,28,28,0.04)', borderRadius: 6, border: `1px solid ${row.pts > 0 ? 'rgba(22,101,52,0.2)' : 'rgba(185,28,28,0.1)'}` }}>
                              <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 'var(--fs-micro)', color: 'var(--mute)', minWidth: 52 }}>{row.label}</span>
                              <span style={{ fontFamily: 'Archivo, sans-serif', fontSize: 'var(--fs-body)', color: 'var(--paper)', flex: 1 }}>{row.pick}</span>
                              <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 'var(--fs-micro)', fontWeight: 700, color: row.pts > 0 ? 'var(--positive)' : 'var(--danger)' }}>
                                {row.pts > 0 ? `+${row.pts}` : '+0'}
                              </span>
                            </div>
                          ))}
                          {breakdown.bonus > 0 && (
                            <div style={{ padding: '7px 10px', background: 'rgba(184,114,14,0.08)', borderRadius: 6, border: '1px solid rgba(184,114,14,0.2)', fontFamily: 'JetBrains Mono, monospace', fontSize: 'var(--fs-micro)', color: 'var(--gold)' }}>
                              ⭐ All Correct Bonus +{breakdown.bonus}
                            </div>
                          )}
                          <div style={{ padding: '8px 10px', background: 'var(--elev)', borderRadius: 6, display: 'flex', justifyContent: 'space-between', fontFamily: 'Archivo Black, sans-serif', fontSize: 'var(--fs-body-lg)', color: 'var(--f1)' }}>
                            <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 'var(--fs-micro)', color: 'var(--mute)', alignSelf: 'center' }}>TOTAL</span>
                    ...
                    {!bet && <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 'var(--fs-micro)', color: 'var(--mute)', textAlign: 'center', padding: '12px 0' }}>No picks submitted for this race.</div>}
                  </div>
                )}
              </div>
            );
