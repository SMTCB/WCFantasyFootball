-- Migration 162: set_lineup must not enforce/create lineup_locks before a round starts
--
-- PROBLEM:
--   set_lineup() unconditionally (a) rejects subbing in any player listed in
--   lineup_locks[matchday_id] (PLAYER_LOCKED), and (b) appends the benched player
--   to lineup_locks[matchday_id] on every successful swap.
--
--   This "can't bring a benched player back until next matchday" rule exists to
--   stop flip-flopping during a LIVE round. But it also fires before the round has
--   even started, so pre-competition lineup tinkering ("sub at will" — same
--   guarantee as transfers) permanently locks managers out of their own players.
--
--   Reported case: goncalofandrade@hotmail.com (Mundial do Eder, tournament 429,
--   round 1 — all fixtures still 'scheduled') made 3 lineup swaps pre-competition
--   and is now blocked from bringing Pervis Estupiñán back into the XI with
--   "already subbed out this round and cannot return".
--
-- FIX:
--   Resolve v_round_started = EXISTS any fixture for (tournament_id, round_number)
--   with status IN ('live','finished'). If the round hasn't started:
--     - skip the PLAYER_LOCKED check entirely
--     - don't add p_player_out to lineup_locks
--   Once the round starts, locking behaves exactly as before.
--
-- BACKFILL:
--   Clear lineup_locks->'429-r1' for the 4 squads that accumulated stale
--   pre-competition locks in WC round 1 (all 429-r1 fixtures are 'scheduled'):
--     - goncalofandrade@hotmail.com   (Mundial do Eder)
--     - joshuajenkins311@gmail.com    (RANKS FC World Cup Fantasy)
--     - jackjcollins@ymail.com        (RANKS FC World Cup Fantasy)
--     - reid.gardner.13@gmail.com     (RANKS FC World Cup Fantasy)
--   Other squads' lineup_locks (623-r1 / 623-r4, both started) are untouched.
...
BEGIN
  ...
  -- Has this round actually kicked off yet? If not, lineup locks (the
  -- "can't bring a benched player back this round" rule) don't apply —
  -- pre-competition, lineups can be set freely, same guarantee as transfers.
  SELECT EXISTS (
  ...
  IF v_round_started AND p_player_in = ANY(v_lock_array) THEN
...
-- Backfill: clear stale pre-competition locks for WC round 1 (429-r1, all
-- fixtures still 'scheduled' as of this migration). SELECT shown in session
-- notes before this UPDATE was approved.
UPDATE squads
SET lineup_locks = lineup_locks - '429-r1'
WHERE lineup_locks ? '429-r1'
  AND id IN (
    '7b4e85bf-b82c-471f-ae3f-3294f7056d54', -- goncalofandrade@hotmail.com (Mundial do Eder)
    'f60b970c-1874-4ffc-bfcd-ee51a938735b', -- joshuajenkins311@gmail.com (RANKS FC)
    '08088b9e-3e4e-42d7-9d94-266e5347c932', -- jackjcollins@ymail.com (RANKS FC)
    'e7ca3dbb-696b-4d0c-af39-f39d2174a45d'  -- reid.gardner.13@gmail.com (RANKS FC)
  );
