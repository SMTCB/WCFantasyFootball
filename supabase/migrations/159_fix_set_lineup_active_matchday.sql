-- Migration 159: fix set_lineup to determine active matchday from fixtures, not from
-- squad.matchday_id which is only advanced when a transfer is made.
--
-- Problem: if a manager never transfers after the draft, squad.matchday_id stays on
-- the first matchday ('623-r1' / '426-r1'). set_lineup then checks round 1 fixtures
-- for the FIXTURE_COMPLETED guard. Any player whose team played in round 1 is
-- permanently blocked from being subbed in, even 10 rounds later.
--
-- Fix: resolve the active round as the lowest round_number with any non-finished
-- fixture in the tournament. This is always the "current or upcoming" matchday
-- regardless of the squad's stored matchday_id. Fall back to squad matchday only
-- if all tournament fixtures are finished (end of season).
