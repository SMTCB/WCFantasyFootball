-- Migration 150: fix accept_trade_proposal points transfer + gazette entry
--
-- Bug: accept_trade_proposal subtracted points_sweetener from proposer but
-- never added them to the target manager. Half the transfer was missing.
--
-- Also: no gazette entry was written on accept, so trade results never appeared
-- in the League Activity or Frontpage sections.
...
BEGIN
  ...
  IF v_proposal.points_sweetener > 0 THEN
    ...
    -- FIX: was missing — add points to target
    UPDATE league_members
      SET total_points = total_points + v_proposal.points_sweetener
  ...
  -- Write gazette entry so trade appears in League Activity + Frontpage
