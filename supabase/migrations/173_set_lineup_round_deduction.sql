-- Migration 173: set_lineup returns a rounded deduction matching on-screen points
--
-- Reported (TEST_WC_CONTROL_CLASSIC): subbing out a player who'd scored showed
-- "−1.4 pts deducted" in the toast, but every points display in SquadScreen
-- (the confirm dialog, the pitch/bench/list rows, fixture-status line) rounds
-- with Math.round(player.points) — that same player shows "1 pt" everywhere
-- else on screen. The raw v_pout_pts (player_match_stats.fantasy_points) can
-- carry decimals from per-60-minute scoring (e.g. 45 min = 0.5 of the minutes
-- component). fantasy_points.total was already deducted by ROUND(v_deduction)
-- — only the *returned* value (and therefore the toast) was unrounded.
--
-- Fix: round v_deduction to an integer before returning it, so the toast
-- matches the rounded points shown everywhere else for that player.
...
BEGIN
  ...
  IF v_pout_status = 'finished' THEN
    ...
    -- Round to match the per-player points shown everywhere in SquadScreen
    -- (Math.round(player.points)) — the toast should agree with the rest of
    -- the UI, not show the raw decimal scoring component.
    v_deduction := ROUND(v_pout_pts::numeric);

    UPDATE fantasy_points
       SET total = GREATEST(total - v_deduction, 0)
