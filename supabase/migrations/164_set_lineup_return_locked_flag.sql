-- Migration 164: set_lineup returns whether the swap actually applied a lineup lock
--
-- BUG (Mundial do Eder, tournament 429, pre-competition round):
--   SquadScreen.doSwap() unconditionally sets `isLineupLocked: true` on the
--   newly-benched player in its optimistic local state, regardless of whether
--   migration 162's `v_round_started` gate actually wrote that player into
--   squads.lineup_locks.
--
--   Pre-competition (round not started), set_lineup correctly does NOT lock the
--   bench player server-side — but the client still marks it locked. The very
--   next swap attempt (subbing that player back in) is then blocked CLIENT-SIDE
--   by SquadScreen's own `if (benchPlayer.isLineupLocked)` guard before the RPC
--   is even called, showing "already subbed out this round and cannot return".
--   A hard refresh re-fetches lineup_locks from the DB (empty), which fixes the
--   symptom — confirming the client's optimistic flag was the stale piece.
--
-- FIX:
--   set_lineup's RETURN now includes `'locked', v_round_started` so the client
--   can mirror server truth exactly: mark the benched player locked only when
--   the round has actually started and the lock was written.
--
-- No other behavioural change — this is the same function body as migration 162
-- with one additional key in the final RETURN jsonb_build_object.
