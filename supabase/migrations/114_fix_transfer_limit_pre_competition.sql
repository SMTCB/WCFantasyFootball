-- Migration 114: Bypass per-round transfer limit for non-standard matchday IDs
--
-- Root cause: squads created before a WC competition starts are assigned
-- matchday_id = 'current' (a placeholder).  execute_transfer_atomic tries to
-- extract a round number via regexp_match('current', '-r(\d+)$'), which returns
-- NULL.  The wildcard-round check is:
--
--   IF v_wildcard_round IS NULL   ← always true when config is null
--      OR v_matchday_round IS DISTINCT FROM v_wildcard_round
--   THEN v_enforce_limit := true
--
-- So "current" matchdays hit v_enforce_limit = true and accumulate a counter,
-- blocking auto-fill and manual transfers after 3 moves despite the UI
-- showing "WINDOW OPEN · Unlimited".
--
-- Fix: after parsing v_matchday_round, skip limit enforcement when it is NULL
-- (i.e., the matchday_id is not in the standard '{tournament}-rN' format).
-- Real rounds (429-r1, 426-r35, etc.) are still enforced as before.
--
-- Also resets the stale {"current": 3} counter on any squad whose round_transfers
-- has a "current" key, so affected managers can transfer immediately.
...
BEGIN
  ...
  -- Only enforce when both league and matchday are known, AND the matchday_id
  -- parses as a real round number (e.g. '429-r1').  Non-standard IDs like
  -- 'current' (pre-competition placeholder) return NULL from regexp_match and
  -- are treated as unlimited — matching the "WINDOW OPEN · Unlimited" UI state.
  ...
  -- ── Increment round_transfers counter (only for real matchday rounds) ────────
...
-- ── 2. Reset stale "current" counter on all affected squads ──────────────────
-- Any squad that has a {"current": N} entry in round_transfers has been
-- incorrectly counting pre-competition transfers against the per-round limit.
-- Clear the "current" key so managers can use auto-fill without hitting the cap.

UPDATE squads
SET round_transfers = round_transfers - 'current'
WHERE round_transfers ? 'current';
