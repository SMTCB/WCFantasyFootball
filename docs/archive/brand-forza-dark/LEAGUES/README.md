...
There are **two surfaces** in scope here: Desktop (≥1024px) is fully designed. Mobile is **not designed in this pass** — it should follow the same pattern as the Live Centre mobile handoff (single column, tabs collapse into a horizontal switcher). Recommend reusing those patterns when mobile is needed.

All artboards in this handoff are 1440 design width. Heights vary because content density does (Frontpage and Bets are taller).
...
├── Leagues Redesigns.html          ← open in a browser to interact with the live prototype
├── design-canvas.jsx               ← design-canvas wrapper (host shell, NOT product code)
└── src/
    ├── league-data.jsx             ← mock managers, standings, frontpage, bets, auctions, chat, stats
    ├── league-shared.jsx           ← HubTopbar / HubActionBar / HubTabs / MgrTag / TrendPill / FormDots / Spark / HubSectionLabel
    ├── league-leaderboard.jsx      ← LeaderboardTab + ManagerProfileScreen
    ├── league-frontpage.jsx        ← FrontpageTab + ForzaArticleScreen + newspaper subcomponents
    ├── league-bets.jsx             ← BetsTab + MakePickScreen + BettingTab
    ├── league-auctions.jsx         ← AuctionsTab + AuctionDetailScreen
    ├── league-chat-stats.jsx       ← ChatTab + StatsTab + WeeklyChart + PositionDonut
...
1. **Mobile.** Not designed in this pass. Recommendation: collapse the 8 tabs into a horizontal scroll switcher (mono pills with the existing tone language), full-bleed content, and keep the standings / bets / auctions as single-column lists. The drill-ins translate to full-screen sheets pushed onto the navigation stack. The Frontpage should keep its serif typography on mobile but stack to a single column — it'll feel like a phone-format newspaper.
2. **Frontpage generation.** The prototype shows hand-curated copy. In production the editorial layout should be driven by a small set of templates filled from match data + league events. Recommended rule set: lead = biggest absolute swing in standings + a noteworthy player moment; secondaries = biggest GW score, biggest captain hit/miss, biggest auction outcome, biggest bet streak; classifieds = anyone who has flagged a player as for-trade in the past 7 days.
3. **Chat moderation.** Out of scope. Assume the admin role (visible as `ADMIN` tag in the members rail) has delete/pin/mute powers.
4. **Auctions atomic blocking.** When a manager wins an auction, that player must atomically move from the auction house into their squad AND become blocked for everyone else. Treat as a transaction.
5. **Stats charts.** SVG line + donut are coded inline. If the codebase has a charts library (Recharts, Visx), use that and match the visual treatment (dashed `var(--rule)` gridlines, JetBrains Mono 9 axis labels, no chart frame).
6. **Accessibility.** The tab bar needs `role="tablist"` and `aria-selected`. The bet option chips on Make Pick should be `<button role="radio">` inside a `role="radiogroup"`. The pulse animation should respect `prefers-reduced-motion` (clamp opacity to 1). Manager hues are not sole indicators — the monogram always carries the identity.
