...
> **Status (session 72, updated 2026-07-27)**: Step 2 ✅ PASS (Pécsi=5, Leno=4, Lindelöf=4, Burn=2 visible after SquadScreen `tournamentId` bug fix). Step 3 ✅ RESOLVED — `ScoringBreakdown.jsx` itself was never built under that name, but the per-stat breakdown is now live via `FormStrip.jsx`/`usePlayerStats.js` (PR #301, last-5-GW inline pts) plus `PlayerStatsPanel.jsx` ("Layer 2 player stats panel on Market screen", fixed in PR #533) for the full per-event breakdown (goals/assists/clean sheet/bonus) on click.

**Pass**: Breakdown shows per-event contribution from real match stats. ✓ (both steps)
...
| F-2 step 3 (scoring breakdown on click) | F-2 | ✅ Resolved 2026-07-27 — shipped via `FormStrip.jsx`/`usePlayerStats.js` (PR #301) + `PlayerStatsPanel.jsx` (PR #533). No longer a gap. |
