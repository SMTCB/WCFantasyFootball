...
**Status (2026-08-28, Foundation + F1/tennis UI coverage shipped)**: `npm run test:e2e:local` runs 265 passed, 31 skipped (service-role-gated draft-lottery scenarios — see below — plus a couple of graceful `test.skip()` fallbacks), 0 failed, 0 flaky across both `desktop-chrome` and `mobile-chrome` projects. Step 4 of the build order (new coverage for F1/tennis) is now **done**; CI wiring for this tier is **still not done** — tracked in BACKLOG.md.
...
- **Service-role env var scoping**: F1/tennis specs need a service-role client (via `E2E_LOCAL_SERVICE_ROLE_KEY`, set unconditionally by `scripts/e2e-local.mjs` since it's the ephemeral local stack's own key) to reset rows RLS restricts to admin/service-role writes (e.g. `trophy_ledger` has no client-writable policy), keeping cross-project reruns of the same admin-scoring specs idempotent against the one shared local DB. This is deliberately a *different* env var name than `SUPABASE_SERVICE_ROLE_KEY`, which `draft-mode-complete.spec.js`/`draft-allocation-e2e.spec.js` also check (via `if (!serviceSupabase) test.skip()`) to gate their own service-role-invoking test bodies. Those two files' service-role paths are pre-existing and independently broken (see BACKLOG.md's `BUG-DRAFT-SVC` — `run-draft-lottery` requires a real commissioner user JWT and rejects a service-role caller; a `draft_submissions` upsert targets an `onConflict` column pair that doesn't match the table's real unique constraint) — using a distinct env var name keeps those tests skipped (their known-good state) rather than accidentally un-skipping them into real failures just because this tier's harness now sets a service-role key for an unrelated reason.

**Still open** (tracked in BACKLOG.md):
- Realtime league chat, `resolve_bet` write-in UI flow (RPC-level already covered by Tier 1; the UI flow is not), coins/P2P beyond the RPC layer (`purchase-coins` function itself is untested), draft lottery/wishlist-draft timing (allocation math now covered at Tier 1 via `snakeDraft.test.js`, but `run-wishlist-draft`/`WishlistDraftScreen` as UI/orchestration are referenced in zero test files today, and the two pre-existing draft-allocation bugs noted above are also unfixed), cron-driven scoring orchestration (only the *extracted pure logic* of `calculate-scores`/`score-f1-race`/`score-tennis-tournament` is unit-tested — the functions themselves, as HTTP-invoked orchestration, are not), notification delivery, ATP Finals knockout phase, F1 Season Bets locked view.
...
| *(planned)* Tier 3 job | 3 | see Build order above | Not yet wired into CI — `npm run test:e2e:local` runs cleanly locally (265 passed, 31 skipped, 0 failed, 0 flaky as of 2026-08-28) but there's no GitHub Actions job invoking it yet |
