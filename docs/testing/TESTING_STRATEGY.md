...
**Status (2026-08-27, Foundation shipped)**: all 8 previously-manual specs now run cleanly against a local `npx supabase start` stack via `npm run test:e2e:local` — 225 passed, 31 skipped (service-role-gated scenarios and a couple of graceful `test.skip()` fallbacks), 0 failed. This is the "Foundation only" scope of the build order below (steps 1–3, minus CI wiring); step 4 (new coverage for F1/tennis/etc.) is **still not started** and CI wiring for this tier is **still not done** — both remain open, tracked in BACKLOG.md.
...
- **Local edge_runtime resource contention under full-suite concurrency**: running all 8 specs across both Playwright projects (desktop-chrome + mobile-chrome) concurrently means both hammer the same single local `edge_runtime` Docker container. A spec making several sequential Edge Function round-trips (e.g. `autofill-draft-classic.spec.js`'s `apiFillSquad`, one call per player bought) can exceed the file's default 20s test timeout under that load even though each individual call is correct — fixed there with a per-test `test.setTimeout(45000)` rather than raising the file-wide default. Worth the same fix in any future Tier 3 spec that does several sequential Edge Function calls in one test.

**Still open** (tracked in BACKLOG.md):
- **Step 4 — new coverage** for the confirmed zero-coverage feature areas: **F1** (bet-scoring math now covered at Tier 1 as of Pass 1 above, but no e2e specs reference it — race-picks UI, paddock membership, trophy award flow all untested), **tennis** (same split — `scorePlayer`/`scoreRoster` now Tier 1-tested, but the tournament UI, QF-captain flow, and player-box trophy award are untested), Realtime league chat, `resolve_bet` write-in UI flow (RPC-level already covered by Tier 1; the UI flow is not), coins/P2P beyond the RPC layer (`purchase-coins` function itself is untested), draft lottery/wishlist-draft timing (allocation math now covered at Tier 1 via `snakeDraft.test.js`, but `run-wishlist-draft`/`WishlistDraftScreen` as UI/orchestration are referenced in zero test files today), cron-driven scoring orchestration (only the *extracted pure logic* of `calculate-scores`/`score-f1-race`/`score-tennis-tournament` is unit-tested — the functions themselves, as HTTP-invoked orchestration, are not), `trophy_ledger` emission, notification delivery.
- **CI wiring** for this tier — `npm run test:e2e:local` is local-only today; no GitHub Actions job runs it yet.
...
2. ~~Write one committed synthetic seed script covering all three sports.~~ Done (`supabase/seed.sql`, EPL + WC; F1/tennis seed data is part of the still-open step 4 coverage work, not this seed script's current scope).
3. ~~Retarget the 8 existing specs to the local stack by default.~~ Done. Wiring a CI job for this tier is still open.
4. Add new specs for each confirmed gap above. **Not started.**
...
| *(planned)* Tier 3 job | 3 | see Build order above | Not yet wired into CI — `npm run test:e2e:local` runs cleanly locally (225 passed, 31 skipped, 0 failed as of 2026-08-27) but there's no GitHub Actions job invoking it yet |

**Local-only, not in CI**: `scripts/rehearse-schema.sh` (Tier 2, interactive), `npm run test:e2e:local` (Tier 3 — retargeted and passing locally, just not wired into CI yet), Tier 4 verification (`scripts/TEST_pilot_gate.js`, deliberately prod-only — never CI, per the tier's own scope).
...
# Tier 3 — local full-stack E2E (all 8 gated specs, freshly reseeded each run)
