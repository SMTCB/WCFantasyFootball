...
| ✅ | `[CODE]` | 🟢 ZERO | **P2P-LOAD** — 50-concurrent-challenge load test script. Done (PR #726): `scripts/p2p-load-test.js`, prod-URL guarded, never touches prod. **Still outstanding: actually running it** — requires a local Supabase/Docker instance. | [Phase 2 DD](#phase-2--post-3b-before-buyer-demos) |
...
**Rows 10, 11, 16, 19–27 pending** — 10/11 are not Supabase actions; 16 blocked on source access; 19, 26 + 27 are pure `CREATE OR REPLACE`/DDL (safe, no approval needed beyond "yes, run row N"); 20–25 are OPS-2 function deploys (each needs per-item approval). **Row 17 done. Row 18 — see 🛑 banner above, do not run.**

**Next migration on v2:** `225_`
...
| P2P-LOAD | Load test — 50 concurrent P2P challenges to verify coin ledger atomicity under contention. Deferred from Sprint P2P-6. | ~2h | ✅ Script done (PR #726) — `scripts/p2p-load-test.js`. Hard-guarded to refuse any `SUPABASE_URL` matching the prod project ref. Test A: single "whale" user fires concurrent challenges to probe the `create_p2p_challenge` `challenge_id` backfill race. Test B: 25 concurrent challenge pairs, concurrent accepts, then asserts `balance + escrow == 500` (starting coin grant) for every touched user. **⚠️ Written + lint/syntax-verified + CI-green, but never actually executed** — needs a local Supabase/Docker instance to run against; that run is still outstanding. |
