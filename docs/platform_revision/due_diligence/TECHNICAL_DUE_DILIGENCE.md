...
- **Fix — split into a ZERO-RISK half and a PILOT-IMPACTING half (do NOT bundle):**
  - **🟢 Code half (safe to do now, `v2` only):** (1) Refactor the 4 sync functions to call `getAdapter(tournament.provider).listEvents(...)` and write the canonical model (the `ForzaAdapter` methods already exist). (2) Migrate `sync-player-status` onto the shared client. (4) Write an adapter-conformance test so a buyer can prove an `opta.ts` reaches parity. *All code lands on `v2`; do not deploy the touched football functions to prod (that redeploys the live pipeline).*
  - **🔴 DB half — DEFER (pilot-impacting):** (3) Additive migration `220_provider_generalisation.sql` — rename `tournaments.forza_id`→`provider_key`, drop the football-implied `NOT NULL`. **`tournaments` is the live football spine; the rename cascades to pilot FK constraints. Do NOT apply to prod until a staging environment exists (OPS-1/B1) to rehearse it, or the pilot has ended.** Write the migration file on `v2`; leave it unapplied.
- **🛡️ Pilot-safety note:** nothing in this item may touch the running pilot. The code refactor is behaviourally neutral only if deployed — so keep the refactor (code on `v2`) separate from the function *deploys* and the column rename, both of which are 🔴 and deferred.
- **Done-when:** (code half) sync functions consume the canonical model on `v2` + conformance test passes; (DB half, later) `tournaments.provider_key` is the spine, rehearsed on staging first. A new provider = one new adapter file + registry line + conformance pass.
