...
**Completed so far (2026-07-01, updated 2026-07-27):** A2 ✅, BUILD-1 ✅, C3 ✅, B2-skeleton ✅, CODE-3 ✅, OPS-2 code ✅ (PRs #695 + #696 + #698). `VITE_SENTRY_DSN` is now set and live on Vercel; all 6 remaining functions have `logError` wired and deployed. B3 remaining: backend `SENTRY_DSN` Supabase secret only (tracked as `OPS-SENTRY` in BACKLOG.md) + scheduling the already-built `check-cron-health` function (part c) as a cron job.
...
3. **Failed-cron alerting:** ✅ code shipped as `check-cron-health` Edge Function (calls `get_cron_failure_streaks()`, migration 223; routes through the existing `logError()`/Sentry path; dedup cooldown) — **not yet wired to a schedule**, callable manually only. Remaining: add the pg_cron entry (approval-gated, see Pilot Safeguards) once `SENTRY_DSN` is set. The cautionary tale that motivated this: mig 124's auto-resolve cron failed on *every* call and was found by manual inspection.
