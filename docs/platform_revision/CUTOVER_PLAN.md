...
> **Status: CUTOVER EXECUTED (PR #742, 2026-07-24) — Phase 4 step 1 done: `MAINTENANCE_MODE` came down 2026-08-30, site is live and publicly reachable. Per the user's standing instruction (see CLAUDE.md), do not re-enable that flag without an explicit ask in a current session. Remaining Phase 4 steps (stability window, post-merge queue) — see §Phase 4 below for current state.**
...
1. ✅ **`MAINTENANCE_MODE` came down 2026-08-30** — confirmed via the user reporting the live app rendering normally (no `503`/"Work in progress") and `vercel env ls production` showing a recent update to the var. Site is publicly reachable again.
   - ⚠️ **Cron jobs not yet confirmed re-enabled as part of this.** All 21+ pg_cron jobs were frozen (`active=false`) on 2026-07-31 (see BACKLOG.md `DD-P0-2`) and, as of the last audit, still read 0 active — this flag flip does not appear to have included the cron re-enable step. Verify with `SELECT jobname FROM cron.job WHERE NOT active;` before assuming scoring/sync is running; re-enable via `SELECT cron.alter_job(job_id := jobid, active := true) FROM cron.job WHERE jobname IN (<names>);` (approval-gated write) if still frozen. A reopened site with no scoring/sync running will silently drift.
2. Stability window (suggest 3–7 days) with Sentry watch — now applicable; started 2026-08-30.
