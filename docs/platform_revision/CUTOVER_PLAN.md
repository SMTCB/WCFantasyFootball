...
> **Status: CUTOVER EXECUTED (PR #742, 2026-07-24) — Phase 2/most of Phase 3 done and prod-verified; deliberately paused before Phase 4 reopen. Site remains behind `MAINTENANCE_MODE` on the user's standing instruction (see CLAUDE.md) — do not resume Phase 3/4 or touch that flag without an explicit ask in a current session.**
...
>
> ⚠️ **SUPERSEDED as a per-session read (2026-07-27) for "what's open."** [BACKLOG.md](../../BACKLOG.md) is now the single source of truth for open items. This file remains the authoritative cutover runbook and revert playbook — keep reading it for that history and for Phase 4 reopen steps, but do not treat it as the place to check for outstanding work.
>
> **Read [TRACKER.md](../TRACKER.md) first** for anything that changed after this plan was written — as of 2026-07-26 it is the more current of the two documents; this file's §2/§4 checklists were reconciled against it the same day (see the 2026-07-26 update at the bottom).
...
- **Already applied & pilot-proven**: 187–189, 191–201 (F1/Clubhouse/Tennis schemas), 202–216, 220 — weeks of safe coexistence with the live pilot. ⚠️ **Corrected 2026-08-26: this line was wrong for 194–196 specifically.** Live production introspection that day (`information_schema`/`pg_proc`/`pg_trigger`) found `194_clubhouse_frontpage.sql`, `195_clubhouse_owner_settings.sql`, and `196_clubhouse_notification_triggers.sql` had never actually been applied — almost certainly dropped silently during the 2026-07-24 cutover despite this claim. Bounded precisely: 193 and 197 were confirmed live, only 194–196 were missing. Applied all three to prod 2026-08-26 (see [BACKLOG.md](../../BACKLOG.md) "✅ Clubhouse RPC/schema bugs" entry, PR #844) — genuinely live now, verified post-apply. Treat every "already applied" claim in this file about the 191–201 range as unverified until independently re-checked; this is now the second time this exact feature area (see the 195 header's own note about migration 188) has been recorded as shipped while actually unapplied.
...
- **217 (`circle_id NOT NULL`)**: ✅ **applied to prod 2026-07-26** — no live pilot exists anymore (platform on-hold behind the maintenance wall), so the stability-window gate no longer applies. Pre-flight: Q5 found 6 orphans (4 unprefixed "E2E EPL Classic" leagues, plus `TEST_1_F1` paddock and `TEST_WIMBLEDON_1` player_box) — user approved deleting the 4 leagues as dummy/test data; the 2 `TEST_`-prefixed rows were cleared the same way (single-row, no real data, all referencing FKs `CASCADE`). Backed up via `npx supabase db dump --linked` first (`backups/pre_migration_217_232_orphans_20260726_112015.sql`). `circle_id` confirmed `NOT NULL` on `leagues`, `paddocks`, `player_boxes` via `information_schema.columns`.
...
- **PITR will NOT be enabled — explicit user decision, 2026-07-23** ("PITR is not possible, it's not included in my plan and it has an extra cost I will not incur"). This is a final cost call, not open for re-litigation. The Pilot Safeguards backup requirement is satisfied instead by a full `pg_dump` at Phase 1 step 3 (free, no plan upgrade needed) — this is now the *only* rollback-to-a-point-in-time mechanism this plan has (see §5 revert playbook level 5, updated accordingly). No staging environment exists, but as of 2026-07-25 Docker is confirmed working on this machine again — `npx supabase db dump --linked` is available as a proper pg_dump backup method (used going forward for migration backups; this doesn't reopen the PITR decision above, which was a cost call, not a tooling one).

### ✅ Former code bug (not a migration) — fixed, applied to prod 2026-07-26
...
- **Applied 2026-07-26.** The 2026-07-18 hold (no prod touch while the pilot was live) no longer applies — no pilot exists, platform is on-hold behind the maintenance wall. Backed up via `npx supabase db dump --linked` first (`backups/pre_migration_217_232_orphans_20260726_112015.sql`), then applied and verified via `pg_get_functiondef(oid)` — the live `resolve_bet` body now updates `bet_instances.status = 'resolved'` before the points-type `aggregate_league_member_points()` loop, matching the fix.
...
| 6 | Phase 3B smoke passes: platform.spec ✅ (262/262 on 2026-07-17) · football · P2P (`MOCK_PAYMENTS=true`) · F1 · tennis | ✅ football/P2P/F1 done (P2P's two-manager Challenge-accept path still unexercised — needs a second manager in the same football league). ✅ **tennis — done 2026-07-27**: Modules 1–5 executed against an isolated TEST tournament fixture (real RPC calls, not mocked — see `docs/testing/TENNIS_MODULE_TEST_PLAN.md`, TRACKER.md "Session 2026-07-27"); Module 6 (admin UI) covered via static code review rather than live click-through, which is what surfaced `TENNIS-ADMIN-GAP` (see BACKLOG.md `ADMIN-1`). Fixture data cleaned up after the run. |
...
- ~~**Migrations 227/228/230/231**~~ ✅ all confirmed applied to prod (bundled into the 2026-07-23 batch — see §1 Database note and [TRACKER.md](TRACKER.md) rows 29/30/32/33).
- `000_baseline.sql` schema snapshot (Phase 1D-B): still not done — unrelated to the cutover itself, on hold until schema settles post-merge.
- OPS-2 leftovers: ✅ the 6 function deploys (rows 20–25) and (c) failed-cron alerting (migrations 240/241, cron job 73 — confirmed `active` 2026-07-26, see [TRACKER.md](TRACKER.md) rows 35/36 + Cross-Cutting Rule #12) are both done. Vercel's `VITE_SENTRY_DSN` is set, but the backend Supabase `SENTRY_DSN` secret (TRACKER row 11) is confirmed **still not set** — (a)/(b) are deployed code but inert without it. This is the one genuinely open OPS-2 item left.
...
- ~~Migration 217 — only after clubhouse backfill + stability window (§5 step 4).~~ ✅ Applied 2026-07-26 — see §1 Database note above.
...
- ~~`resolve_bet` statement-order fix (migration 232) — total_points lag on bet resolution~~ ✅ Applied 2026-07-26 — see §1 "Former code bug" note above.
...
6. DB actions from the Supabase-linked PC (per-item approval as usual):
   ...
   d. ✅ **217 applied 2026-07-26** — no live pilot exists anymore (platform on-hold behind the maintenance wall), so the original stability-window gate (§4 Phase 4 step 3) no longer applied; the standing "no migrations without an explicit ask" rule was superseded for this one item by the user's blanket cutover-completion authorization. Pre-flight §7 Q5 found 6 orphans (4 dummy/test leagues the user approved deleting, plus `TEST_1_F1`/`TEST_WIMBLEDON_1`); backed up via `npx supabase db dump --linked` first (`backups/pre_migration_217_232_orphans_20260726_112015.sql`). `circle_id` confirmed `NOT NULL` on `leagues`/`paddocks`/`player_boxes` via `information_schema.columns`. See [TRACKER.md](TRACKER.md) §1 Database note.
7. ✅ **All 19 Edge Functions deployed** — done as part of the 2026-07-23/24 cutover batch, confirmed via `.function-checksums.json` (see [TRACKER.md](TRACKER.md) line ~311).
8. Secrets/env: ✅ `VITE_SENTRY_DSN` (Vercel) confirmed set · ✅ `VITE_AUTH_ENABLED=true` confirmed present · ⬜ **`SENTRY_DSN` (Supabase backend secret) confirmed still NOT set** (`supabase secrets list`, 2026-07-26) — OPS-2 parts (a)/(b) are deployed but inert until this is added. Only genuinely open item in this step.
...
> 📌 **When Phase 4 is actually executed, the sequence around it lives in [BACKLOG.md → 🚀 Launch Plan — Pilot Relaunch](../../BACKLOG.md#-launch-plan--pilot-relaunch-living-plan-target-2026-09-04)** (added 2026-08-21, target reopen 2026-09-04). That plan's `L-4` is step 1's cron re-enable and its `L-12` is the `MAINTENANCE_MODE` decision — it orders them against the data, test and go/no-go work that has to happen first. **The commands stay here**; the Launch Plan links back to this section rather than duplicating them.

1. **Not yet done — deliberately paused, see status banner at top.** Set `MAINTENANCE_MODE="false"` (or remove the var) in Vercel Production env vars, `vercel deploy --prod`. Announce. Requires an explicit user ask in a current session before touching this.
   - **Re-enable cron jobs first (or alongside):** all 21 pg_cron jobs were frozen (`active=false`) on 2026-07-31 — 20 via explicit user approval that session, plus `run-draft-lottery` which was already inactive beforehand — since they were running live scoring/sync work against a walled site with no active competition (see BACKLOG.md ARCH-1 audit note). A 22nd job, `award-season-trophies` (migration 249, ARCH-1d), was added later the same day — created `active=false` from the start specifically so it wouldn't start firing ahead of this group; it needs the same re-enable step, not a separate one. Re-enable via `SELECT cron.alter_job(job_id := jobid, active := true) FROM cron.job WHERE jobname IN (<names>);` — pull the current list with `SELECT jobname FROM cron.job WHERE NOT active;` first since more jobs may be frozen by the time of reopen. Do this before or immediately after unwalling — a reopened site with no scoring/sync running will silently drift.
2. Stability window (suggest 3–7 days) with Sentry watch — n/a while still walled; re-evaluate once step 1 happens.
3. ~~Then, and only then: apply migration 217~~ — ✅ **already applied 2026-07-26**, ahead of reopen, since no live pilot remains to protect (see Phase 2 step 6d above). This step is now a no-op.
4. Start the post-merge queue (§2 🟢) — several items already picked off out of order (F1 data migration, `resolve_bet` fix) since they carried no pilot risk once the pilot ended; see [TRACKER.md](TRACKER.md) for current status of each.
...
Related: [TRACKER.md](../TRACKER.md) · [V2_BRANCH_PROTECTION.md](architecture/V2_BRANCH_PROTECTION.md) (⚠️ obsolete — v2 branch deleted 2026-07-25) · [CLUBHOUSE_CENTRIC_REDESIGN_IMPLEMENTATION_PLAN.md](architecture/CLUBHOUSE_CENTRIC_REDESIGN_IMPLEMENTATION_PLAN.md) · Phase 3B checklist in TRACKER · `backups/orphans_pre_217_20260629.json`

**Post-cutover update (2026-07-25):** the `v2` branch named throughout this document was deleted after the merge landed — repo is single-branch (`main`) again. Home/F1/Tennis Redesigns from the "Clubhouse-scoped screen redesign" mentioned above are now implemented (F1+Tennis via [PR #758](https://github.com/SMTCB/WCFantasyFootball/pull/758)); only Coin Challenges Redesign remains, see [`design_v2/README.md`](design_v2/README.md).

Last Updated: **2026-07-26** (📋 **Reconciled against verified live prod state and against [TRACKER.md](TRACKER.md)'s own same-day reconciliation pass.** This file's §2/§4 checklists had drifted well behind actual prod state since the 2026-07-24 cutover — corrected without re-litigating any decision: §2 row 6 (Phase 3B smoke) updated to football/P2P/F1 done, tennis plan-corrected-but-unexecuted ([PR #769](https://github.com/SMTCB/WCFantasyFootball/pull/769)); §2 🟡 recommended section updated (migrations 227/228/230/231 confirmed live, OPS-2 rows 20–25 + part (c) done, only the backend `SENTRY_DSN` secret still open); §4 Phase 2 step 6d updated to reflect migration 217 applied 2026-07-26 (no live pilot left to protect, so the original stability-window gate no longer applied); §4 Phase 2 steps 7–8 updated to reflect the 19-function deploy and env/secret state; §4 Phase 4 steps 1/3 updated to show migration 217 already done and reopen (`MAINTENANCE_MODE`) still deliberately paused pending explicit user ask. Status banner at top rewritten from "IN PROGRESS" to "CUTOVER EXECUTED, paused before reopen" to match reality. **This pass changed no code and applied no migrations — read-only verification queries (matching TRACKER.md's same-day pass) plus documentation edits only.**)

Previous: **2026-07-23** (✅ **maintenance-mode `/unlock` bug fixed + all 13 pending migrations applied & verified.** Post-deploy verification of the maintenance-mode gate (previous entry) found `/unlock?token=...` returning `MIDDLEWARE_INVOCATION_FAILED` in prod — root cause: `Response.redirect()` on Vercel's Edge Runtime returns headers that are immutable, so `res.headers.append('set-cookie', ...)` threw. Fixed by constructing headers via `new Headers()` before passing them into `new Response(...)` at construction time; landed via [PR #740](https://github.com/SMTCB/WCFantasyFootball/pull/740) (temp branch off fresh `main`, squash-merged); verified live end-to-end (block=503, unlock sets cookie + 307, cookie-holder=200). Then applied all 13 pending DB migrations, one file at a time via `npx supabase db query --linked -f <file>`, in order **218, 219, 223, 224, 225, 226, 227, 228, 229, 230, 231, 232, 233** — zero errors on any file. Note: **223/224/225 were not in this plan's original Phase 2 step 6a checklist** (which only named 218/219/226–232); they were surfaced by directly querying prod (`pg_proc`/`pg_constraint`/`pg_publication_tables`/`information_schema.tables`) for the actual objects each migration creates, rather than trusting the checklist — a stale/incomplete plan should never gate a live-DB decision. Deep-verified the two highest-stakes migrations: **233** (clubhouse backfill) — confirmed 7 leagues, 7 `circle_leagues` rows, 39 `circle_members` rows in prod, matching the 7 retained pilot leagues; **224** (P2P `resolve_p2p_challenge` payout bug fix) — re-read the live function body and confirmed both bugs from the migration's own description (tie-path double-credit, zero-amount loser-audit insert) are gone. §2 rows 3/5/7/9, §3 resolution note, §4 Phase 1 steps 1–3, §4 Phase 2 steps 6a/6c, §8 pickup guide all updated. Next: present the v2→main PR plan (merge commit, not squash) and get an explicit fresh go-ahead before opening it — Task #8.)
