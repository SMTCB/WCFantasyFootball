...
| 3 | **Clubhouse mapping decision (✅ resolved 2026-07-23 — keep all 7 leagues, see §3) + write/apply the `233_` backfill migration** | ✅ migration written 2026-07-23 (`supabase/migrations/233_clubhouse_backfill_pilot_leagues.sql`) — not yet applied, needs per-item approval |
...
**What this unblocked:** the backfill migration, written as **`233_`** (next-free renumbered from the stale "223_" placeholder — migrations 223–232 were taken by other v2 work between when this plan was written and now), targets **all 7 leagues**, consolidating them into one new clubhouse ("World Cup Pilot") per point 3 above, followed by migration 217 (`circle_id NOT NULL`). The 11 test-league orphans (unrelated to the 7 real leagues) still need their own delete-vs-map call at Phase 4 step 3 (§7 Q5). **Status:** `supabase/migrations/233_clubhouse_backfill_pilot_leagues.sql` written 2026-07-23 — not yet applied to prod; per CLAUDE.md, needs explicit per-item "yes, run it" approval before applying.
...
6. DB actions from the Supabase-linked PC (per-item approval as usual):
   ...
   c. Run the **clubhouse backfill** (`233_clubhouse_backfill_pilot_leagues.sql`), scoped to all 7 retained leagues, consolidating them into **one new clubhouse** ("World Cup Pilot", not one per league — see §3). This clubhouse is also the post-merge sanity-check dataset used in Phase 3 step 2.
...
To resume this work cold: (1) confirm session type = **platform revision (v2)**; (2) read this file top to bottom; (3) check [TRACKER.md](../TRACKER.md) for anything newer; (4) find the current phase — the first unchecked ⬜ in §2-critical / §4 — and continue from there. The §3 decision is **resolved** (2026-07-23 — keep all 7 leagues) and the `233_` backfill migration is **written** (`supabase/migrations/233_clubhouse_backfill_pilot_leagues.sql`) but not applied. PITR is **declined** (user, cost) — do not re-raise it; the `pg_dump` backup at Phase 1 step 3 is the only backup this plan has. Remaining work: local gates (§4 Phase 2 step 2), cutover backup (Phase 1 step 3), maintenance-mode activation, applying migrations 218/219/226–233, the v2→main PR itself, Edge Function deploys, and Phase 3 smoke tests — every DB write / deploy / the merge PR itself needs its own explicit per-item "yes, run it" approval in the live session, per CLAUDE.md.
