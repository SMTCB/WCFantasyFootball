/**
 * E2E Full Setup Script — EPL_OVERALL_E2E (revised)
 *
 * Uses existing mock users + RPCs to bypass RLS.
 *
 * Reads credentials from environment variables — no hardcoded secrets.
 * The canonical version of this script lives at scripts/e2e-setup.mjs.
 *
 * Usage:
...
async function submitBetAnswers(betIds) {
  // 3 bets × 8 managers — varied answers
  const ANSWER_SETS = [
  ...
  for (let mi = 0; mi < MANAGERS.length; mi++) {
    ...
    for (let bi = 0; bi < betIds.length; bi++) {
      ...
      // bet_submissions has RLS? check. Use direct insert.
...
async function resolveBets(betIds) {
  ...
  for (let i = 0; i < betIds.length; i++) {
    ...
    // Mark correct submissions
    await api(`/rest/v1/bet_submissions?bet_id=eq.${betIds[i]}&answer=eq.${encodeURIComponent(CORRECT[i])}`, {
      method: 'PATCH',
      body: { is_correct: true, reward_awarded: 0 }, // reward_awarded computed by trigger
    });
...
async function main() {
  ...
  log('\nVerifying squads…');
  const squads = await verifySquads();
