...
**Key pipeline facts (2026-06-30 — Mobile-First Redesign Phase M4 + workstream complete, v2 branch):**
- **Mobile-First Redesign (M0–M4) fully complete** — PR #689 closes the workstream. All phases: M0 (PRs #682–684), M1 (PRs #685–686), M2 (PR #687), M3 (PR #688), M4 (PR #689).
- **Tap-target sweep (M4)** — every interactive element in the audited screens is now ≥44px. RecapView `MatchdayNav` buttons: `padding: '4px 9px'` → `'8px 9px', minHeight:44`. SquadScreen action-sheet buttons: `py-2.5` → `py-3`. MarketScreen per-row action buttons (all six variants — LOCKED/SELLING/BUYING/SELL/CLUB FULL/BUY): `padding:'6px 10px'` → `'10px 10px', minHeight:44`.
- **`PlayerPickerSheet` createPortal (M4)** — `src/components/PlayerPickerSheet.jsx` now uses `createPortal(…, document.body)`. Before: the sheet's `position:fixed` children were trapped inside `AppLayout#main-content` (which has `WebkitOverflowScrolling:touch`), causing the iOS backdrop/z-index stacking context bug that made taps pass through incorrectly. After: portaled to `document.body`, identical pattern to `PrimaryActionBar`/`ScoringInfoModal`/`NewCompetitionFlow`. Also: all stale dark-theme tokens replaced — `var(--ink-2)` → `var(--card)`, `rgba` scrims → `var(--elev)`/`var(--rule)`.
- **Tablet tier (M4)** — `src/index.css` new `@media (min-width: 640px) and (max-width: 1023px)` block: `.fk-mob-sheet-wrap` is centred at `max-width:min(520px,92vw)` with `transform:translateX(-50%)` (no full-bleed on tablet); `.fk-competition-grid` two-column grid; `[data-sheet-tablet]` generic opt-in selector for future sheets.
- **MarketScreen progressive disclosure deferred** — the M4 plan called for collapsing the tall sticky header into a compact KPI strip + Filters BottomSheet. This was scoped out: per-row button heights fixed to 44px; full BottomSheet filter rewrite is a future P3 item. The header UX is acceptable at pilot scale.

**Key pipeline facts (2026-06-30 — Mobile-First Redesign Phases M0 + M1, v2 branch):**
- **`src/hooks/useViewport.js`** — new leaf hook: `useViewport()` returns `{isMobile, isTablet, isDesktop, width}`; `useIsMobile()`/`useIsTablet()`/`useIsDesktop()` named exports. SSR-safe (returns desktop defaults when `window` is undefined). Uses `window.addEventListener('resize', ...)` (not matchMedia). Breakpoints: mobile ≤639px, tablet 640–1023px, desktop ≥1024px. Import from here everywhere — do NOT duplicate the matchMedia check inline (was causing a private hook inside `CompetitionResultsHeader` that needed cleaning up).
- **Sport tokens** — `--f1: #E10600` (Ferrari red), `--ten: #1B7A52` (Wimbledon green), `--f1bg`, `--tenbg` now defined in `src/index.css` `:root`. These were referenced by 8+ components but undefined before PR #685. Also: `--r-sm: 4px`, `--r-md: 6px` aliases; `.safe-top`/`.pt-safe` safe-area helpers (`.safe-bottom`/`.pb-safe` already existed).
- **`AppLayout.jsx` mobile top bar** — `paddingTop: 'env(safe-area-inset-top)'` added to the `lg:hidden sticky top-0` div. Prevents content sitting behind the iOS status bar notch.
- **`CompetitionResultsHeader` card-mode wired** — `leadColumnKey` prop activates on all three sport consumer screens: F1StandingsScreen (`leadColumnKey="total"`), TennisLeaderboardScreen (`leadColumnKey="total"`). Football's `LeagueDetailView` left unchanged — already has bespoke `lg:hidden` mobile cards (Tier A) and its `CompetitionResultsHeader` is desktop-only.
- **Tennis breakdown table killed on mobile** — `TennisLeaderboardScreen`'s 14-column `<table>` wrapped in `className="hidden lg:block"`; new `className="lg:hidden"` per-manager chip-card alternative (tournament name + score as colored chips). No `useIsMobile` import in this screen — uses Tailwind CSS classes to avoid Rolldown TDZ (screen already transitively imports `useViewport` through `CompetitionResultsHeader`).
- **`src/components/shared/TabStrip.jsx`** — new scrollable tab bar primitive replacing two different local implementations. `variant="underline"` (Clubhouse): `flex: 0 0 auto` buttons with `borderBottom` active state, fixes the `flex: 1` clipping that cut off 8 tabs at 375px. `variant="pill"` (League): filled-pill active state, border-only inactive. Both: `overflowX: auto`, `scrollbarWidth: none`. Active tab auto-scrolls into view on change via `scrollIntoView({inline:'center'})`. Props: `tabs[]{key,label,count?,notify?}`, `active`, `onTab`, `accent`, `variant`, `style`. `count` renders as a numbered badge (danger color when inactive, accent when active); `notify` renders a 5px danger dot.
- **`ClubhouseScreen` TabBar replaced** — local `TabBar` function (was `flex:1`, text clipped beyond 4 tabs at 375px) removed; `<TabStrip variant="underline">` adopted for both CREATE/JOIN lobby strip (2-tab) and main 8-tab MAIN_TABS strip. INBOX tab moved from string-interpolated label (`INBOX (N)`) to structured `count` prop.
- **`HubTabPills` delegates to `TabStrip`** — `HubShared.jsx` `HubTabPills` body replaced with `<TabStrip variant="pill" accent="var(--cyan)" ...>` one-liner; internal `{ id, label }` changed to `{ key, label }`. External prop API unchanged — same function signature, same export name, no callers touched.
- **Python urllib merge reliability** — During M0 session, PRs #682/#683 were reported "Merged: True" by the Python urllib script but their content never landed in v2. Root cause unknown (possible silent branch protection rejection). Always verify a merge by doing `git pull origin v2` and checking that the expected files appear in the fast-forward output. If they don't, the merge was a false positive.
- **`HubShared.jsx` TDZ note** — uses `var MONO`/`var DISPLAY`/`var BODY` (not `const`) because LeagueScreen imports HubShared at depth 1 AND LeagueScreen's 6 child view components all re-import HubShared at depth 2. `var` is hoisted and can never TDZ. The `import TabStrip from '../shared/TabStrip'` added to HubShared is safe — TabStrip is a leaf module and LeagueScreen does not import it directly.

**Key pipeline facts (2026-06-30 — Shootout event detection bug + rescore):**
- **Forza's penalty shootout event type is `penalty_shootout_shot`, not `goal`/`missed_penalty`.** Migration 192 (`ingest-match-events`) originally looked for the same event types used for in-game penalties — `goal` (scored) and `missed_penalty`/`missed_goal+detail:'penalty'` (missed). Forza's actual shootout kicks are a unified `penalty_shootout_shot` event with a `scored: true/false` boolean. The period-level detection (`pType.includes('penalt')`) was always correct; only the event-type matching inside it was wrong. Result: all three shootout columns (`shootout_scored/missed/saved`) were silently zero despite the period being detected, with no error logged. Fixed in PR #678.
- **Every shootout miss credits the opposing GK as a save (+0.5 pts).** Forza's `penalty_shootout_shot` event has no field to distinguish an actual goalkeeper save from an off-target/post shot. Product decision: treat every miss as a GK save. Documented in code comment in `ingest-match-events/index.js`.
- **Rescore technique for already-finished rounds (non-roundComplete):** re-invoke `ingest-match-events` with `{"forza_match_id":"..."}` to re-populate `player_match_stats.shootout_*`, then `calculate-scores` with `{"fixture_id":"..."}` to update `fantasy_points.total`. Both use the same legacy service-role JWT from the `ingest-match-events-live` cron entry. **Note**: the `calculate-scores-live` cron command embeds a different key format (`38f7d3e4...` hex, not a JWT) that returns `{"error":"Unauthorized"}` when called externally — always use the JWT from `ingest-match-events-live` for manual rescore invocations.
- **Shootout breakdown display (PRs #679 + #680):** two separate components display per-player scoring line items: (1) `src/components/league/RecapView.jsx` `PlayerBreakdown` (Recap tab — badge-based) and (2) `src/hooks/usePlayerFullStats.js` `buildBreakdownItems` (player detail HISTORY/BREAKDOWN tab — uses `BREAKDOWN_LABELS` map keyed by breakdown JSONB field names). Both now include `shootout_scored`, `shootout_missed`, `shootout_saved`. `ScoringInfoModal.jsx` also updated with rule values. If a future scoring field is added to `calculate-scores`/`buildBreakdown`, **both** display paths need updating.
