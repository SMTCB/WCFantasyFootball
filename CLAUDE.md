...
- **Bucket C deferred** — BACKLOG P3 item B-04: during the next live WC 429 match, grep raw Forza event JSON for `motm`/`man_of_the_match`, `free_kick`, `corner`, `penalty_won`, `penalty_conceded`, `foul` to assess feasibility of MOTM/set-piece/penalty-won/committed scoring before building Bucket C.
