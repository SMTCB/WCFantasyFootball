...
**Next migration**: `202_` (v2 branch) / `191_` (main)

**Key pipeline facts (2026-06-24 — Tennis Module T-0 through T-3 complete, v2 branch):**
- **Tennis Module backend complete** (PRs #617–#620, migrations 197–201, v2 branch only): all DB schema, RPCs, and Edge Functions deployed. T-4 UI is the only remaining sprint.
...
- **4 deployed Tennis Edge Functions** (v2, as of 2026-06-24): `sync-tennis-players`, `score-tennis-tournament`, `score-atp-finals`, `score-f1-race`. None have crons — all admin-triggered.
- **T-4 task list** (UI only, all backend complete): 7 screens (`TennisHomeScreen`, `TennisTournamentScreen`, `TennisLeaderboardScreen`, `TennisAtpFinalsScreen`, `TennisAdminScreen`, `PlayerBoxScreen`, `TennisProfileView`) + 5 hooks (`useTennisCalendar`, `useTennisTournament`, `useTennisLeaderboard`, `usePlayerBox`, `useTennisAdmin`) + route wiring in `App.jsx`. See `SALE_READY_PROJECT_PLAN.md` Phase 2 for full T-4 task list.
- **PowerShell / GitHub API rule**: always use Python urllib for GitHub PR creation — PowerShell `Invoke-RestMethod` fails with multiline JSON bodies. See CLAUDE.md Git section for the urllib pattern.
