...
| 184 | `184_league_config_rls.sql` | Session 2026-06-19: `league_config` had RLS enabled (migration 66) but zero policies — all client operations silently blocked. Adds member SELECT policy + commissioner write policy (INSERT/UPDATE/DELETE with `role='commissioner'`). Fixes PIN QUOTE and all other `league_config` writes from CommissionerPanel. Applied directly to live DB. |

**Next migration**: `185_`

**Key pipeline facts (2026-06-19 — Forza Times polish, PRs #591–#593):**
- **`league_config` RLS** (migration 184): `league_config` had RLS enabled since migration 66 but zero policies — every client read AND write was silently blocked. All `league_config` operations (PIN QUOTE, any config key) now work. Member read + commissioner write policies added.
- **`generate-frontpage-edition` supports single-league cron trigger**: pass `{"mode":"cron","league_id":"<uuid>"}` with service-role JWT to re-process one league on demand without the 4h manual rate limit. Useful for CLI-triggered refreshes.
- **Form guide trend arrows**: only compare completed GWs. Completion is identified from `gazette_entries` with `entry_type='activity'` (written when `roundComplete=true`). In-progress rounds are excluded — prevents all arrows showing ▼ when partial GW2 scores are lower than completed GW1.
- **Transfer whispers**: guard changed to `fpEd?.transfer_rumour?.trim()` — section hidden when AI returns empty string.
- **LATEST SCORES**: when a live in-progress card exists, historical gazette score cards are suppressed. Prevents confusion of seeing "GBruschy leads GW1" alongside a live GW2 card.
- **BET TRACKER section** (`frontpageBetStats` state): fetches `bet_submissions` for all resolved `bet_instances` in the league. Per-manager accuracy bars (green ≥50%, red below) + correct/total/% + sarcastic editorial labels: "The Oracle" (100% across 2+), "The Contrarian" (0% across 2+), "The Gambler" (most entries).
- **`GazetteNews.jsx`** now queries `in('entry_type', ['breaking_news', 'classified'])` — classified posts were saved but never rendered. Commissioner reaction strip moved inside `GazetteNews` and only renders when entries exist.

**Key pipeline facts (2026-06-18 — Forza Times interactive frontpage):**
- **Edition gate**: all new frontpage UI is gated on a `frontpage_editions` row existing for today's date in the active league. If no row exists, the frontpage renders exactly as before.
- **`generate-frontpage-edition` Edge Function** (`supabase/functions/generate-frontpage-edition/index.ts`) — ✅ **DEPLOYED TO PROD** (2026-06-18). Two modes: cron (`{mode:'cron'}` + service-role JWT — loops all active leagues, skips if manual edition < 12h old; also accepts `league_id` to restrict to one league) and manual (`{league_id}` + user JWT — commissioner only, 4h rate limit). Uses Groq `llama-3.1-8b-instant` via OpenAI-compatible API with `response_format:{type:'json_object'}`. Collects: standings top5+bottom1, last 24h transfers, last 3 chat messages, upcoming fixtures 48h, gazette entries (breaking_news + classified + activity), league_config pinned quote. Output shape: `{headline, deck, hot_take, wooden_spoon, transfer_rumour}`.
...
- **`gazette_entry_type` now includes `classified`** (migration 180). Posted via CommissionerPanel → LEAGUE NEWS → CLASSIFIED AD tab. Appears in `gazette_entries` and rendered by `GazetteNews.jsx` (PR #592). `ENTRY_META` in `LeagueDetailView.jsx` does not yet register `classified` for the LEAGUE tab activity feed — deferred.
...
- **Bench scores in Recap — BUILT** (B-09, PR #588): `toggleBreakdown` in `RecapView.jsx` fetches bench player IDs from `points_breakdown.bench_players` (settled rounds) or `squad.players - starting_xi` (live rounds), queries their stats from `player_match_stats`, renders below the XI with a BENCH divider + muted styling. Critical constraint: bench pts displayed only — **never added to the manager's total**. Round 1 (429-r1) has no `bench_players` data (round settled before v28 deployed) — bench section hidden cleanly for R1, accurate from R2 onwards.
