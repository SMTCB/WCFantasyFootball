...
- **Bench scores in Recap — DESIGNED, NOT YET BUILT** (see BACKLOG B-09): the architecture is clear. Extend `toggleBreakdown` in `RecapView.jsx` to fetch bench player IDs from `points_breakdown.bench_players`, query their stats from the same `player_match_stats` source, and render them below the XI section with a divider + "BENCH" label + muted styling (same POS/PLAYER/MIN/PTS columns). Critical constraint: bench pts are displayed only — **never added to the manager's total**. If `bench_players` is absent, bench section is hidden cleanly.
