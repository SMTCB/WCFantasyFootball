...
| TEST-P2P-1 | 🟡 **PARTIALLY RESOLVED 2026-08-28** — **[TECH DEBT] The entire P2P challenge / escrow lifecycle has zero automated tests** | ~2–3 days; core lifecycle done | New `tests/unit/p2p-challenges.test.js` now covers `create_p2p_challenge`, `accept_p2p_challenge`, `decline_p2p_challenge`/`cancel_p2p_challenge` (incl. a direct regression test for the migration-204/235 double-refund bug), `resolve_p2p_challenge` (incl. a regression test for the migration-205/224 tie-double-credit bug), and the full freeform chain (`declare_freeform_result`/`confirm_freeform_result`/`dispute_freeform_result`/`arbitrate_freeform_result`) — all confirmed error codes plus payout-math assertions. Writing this suite immediately surfaced a real, previously-unknown production bug — see new row **`BUG-P2P-DISPUTE`** directly below — which is exactly the class of bug this item predicted ("a stuck or double-released stake would be... the most damaging first-cohort bug"). `submit_bet` also gained coverage this session (extended `tests/unit/bet.test.js`). **Still open**: `auto_resolve_p2p_challenges`, `expire_stale_challenges`, `admin_grant_coins` (cron-context auto-resolvers), `void_bet`, `process_auction_deadlines`, `sweep_void_auction_confirmations`, `resolve_auction_listing`, `submit_trade_proposal`/`accept`/`reject`. |
| BUG-P2P-DISPUTE | 🔴 **[BUG] `dispute_freeform_result` has never worked in production — violates a CHECK constraint on every call** | XS (~10 min fix, migration-gated) | Found 2026-08-28 while writing `TEST-P2P-1`'s freeform-chain tests: `dispute_freeform_result` (migration 239) does `INSERT INTO clubhouse_notifications (..., source_type, ...) VALUES (..., 'p2p_challenge', ...)`, but `clubhouse_notifications_source_type_check` (`supabase/schema.sql`) only allows `source_type IN ('league','paddock','box','clubhouse')` — `'p2p_challenge'` is not a permitted value. Confirmed via `Grep` across `supabase/migrations/` that no later migration ever added it. **Every real call to `dispute_freeform_result` throws `23514` and rolls back** — the freeform P2P dispute path has never functioned in production. Currently **latent, not yet hit by a real user** (`p2p_challenges` has 0 rows in prod per `TEST-P2P-1`), but it will break the very first real dispute once P2P challenges go live. 6 tests in the new `p2p-challenges.test.js` document this by asserting/skipping rather than silently treating it as expected behavior — see the file's `dispute`/`arbitrate` `describe` block. **Fix** (not yet applied — needs explicit per-item migration approval per CLAUDE.md): a new numbered migration adding `'p2p_challenge'` to the CHECK constraint's allowed list (`ALTER TABLE clubhouse_notifications DROP CONSTRAINT clubhouse_notifications_source_type_check, ADD CONSTRAINT ... CHECK (source_type = ANY (ARRAY['league','paddock','box','clubhouse','p2p_challenge']))`), with a `pg_dump` backup first per Pilot Safeguards. |
