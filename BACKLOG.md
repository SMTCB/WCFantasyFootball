...
## ✅ Docker schema-rehearsal workflow + full tennis pipeline test vs. real Wimbledon field (2026-07-31) — no PR yet (pending)
...
- **Local Docker permission bug found and fixed**: the local Supabase replica returned `42501 permission denied` from PostgREST on every `public` table (confirmed via a `leagues` control check, not tennis-specific). Root cause: an earlier-session raw-`psql` continue-on-error migration replay (used to work around a migration-filename lexicographic-sort-order bug — 100+ sorts before 90–99, so `db reset` aborts on `09_sprint1_schema.sql`'s known bug even though `10_sprint1_fixes.sql` fixes it) bypassed the step `supabase db reset` normally runs afterward to re-grant CRUD to `anon`/`authenticated`/`service_role`. Fixed locally with a direct `GRANT ALL` + `ALTER DEFAULT PRIVILEGES` + `NOTIFY pgrst, 'reload schema'`.
- **Formalized as `scripts/rehearse-schema.sh`** — tries a normal `supabase db reset` first, falls back to a continue-on-error raw-`psql` replay (logging exactly which files failed, most as ordering artifacts) only if that fails, and always finishes by restoring the CRUD grants above. Documented in a new [Schema Rehearsal Workflow](docs/deployment/DOCKER_LOCAL_DEV.md#schema-rehearsal-workflow) section.
...
- **Docs updated**: [`TENNIS_MODULE_TEST_PLAN.md`](docs/testing/TENNIS_MODULE_TEST_PLAN.md)'s real-data status table and Test Run Log; [`TRACKER.md`](docs/platform_revision/TRACKER.md)'s Phase 3B tennis smoke-test checkbox flipped to done with the real-data caveat spelled out (Module 6 UI still static-review-only; `sync-tennis-players` live call still unexercised).
