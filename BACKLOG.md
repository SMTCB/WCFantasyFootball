...
| B-07 | **[TECH DEBT] `set_lineup` mid-round deduction doesn't account for captain multiplier when benching the current captain** | 0.5–1h | Discovered 2026-06-15 while verifying B-05/B-06 (PR #545). `set_lineup`'s `fantasy_points.total` self-heal (migrations 168/173) subtracts `ROUND(raw_points)` for the benched player — correct for a non-captain, but if `p_player_out` is the squad's current captain (and their fixture already `finished`, so they'd banked a ×2/×3 bonus), the deduction should be `ROUND(raw_points) * mult` to match the breakdown (which drops the player entirely, full multiplied amount). Until the next `calculate-scores` pass or `roundComplete`, `fantasy_points.total`/leaderboard would carry a small stale surplus (the un-multiplied remainder). **Fix**: mirror migration 176's approach — compute `v_mult` for `p_player_out` (check `chips_used`/triple-captain + `v_squad.captain_id = p_player_out`) and multiply `v_deduction` accordingly, same as `set_captain`'s recompute. Low priority — narrow scenario (captain's match must already be finished AND manager then benches them mid-round, before `roundComplete`). |
