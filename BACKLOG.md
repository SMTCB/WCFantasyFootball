...
| B-13-F1 | **[FEATURE] Archive/active toggle for F1 Paddocks (parity with `B-13`'s league archive)** | ~2–3 days | Found 2026-07-31, requested alongside a cron-coverage question that surfaced the gap: F1/tennis have no cron jobs at all — `score-f1-race` is invoked manually from `F1AdminScreen.jsx:99`, by design (season is only ~24 races, doesn't justify automation the way football's every-few-days cadence did). That actually makes this **cheaper than `B-13`**, not harder: there's no background job to teach to skip archived paddocks, since none exists. **DB**: add `archived`/`archived_at` to `paddocks` (same shape as migration 244, trivial). **The real cost is UI**: unlike leagues, `PaddockLobbyScreen.jsx` has no owner-settings surface at all today (checked — only a `role === 'owner'` check for member management, no `CommissionerPanel`-style panel) — this item has to build the first one, not add a checkbox to an existing one. Also wire an `archived` filter into `F1HomeScreen.jsx`, `F1StandingsScreen.jsx`, `F1RaceBetScreen.jsx`, and the clubhouse-level pickers (`NewCompetitionFlow.jsx`, `ClubhouseScreen.jsx`) — mirroring `B-13`'s `useShowArchived` pattern rather than reinventing it. **Sequence**: do this and `B-13-TENNIS` together (near-identical shape, share the settings-panel scaffolding) before `B-13-CLUBHOUSE`, since a clean cascade design for clubhouse archiving benefits from both child tables already having the column. |
| B-13-TENNIS | **[FEATURE] Archive/active toggle for tennis Player Boxes (parity with `B-13`'s league archive)** | ~2–3 days | Found 2026-07-31, same investigation as `B-13-F1`. `score-tennis-tournament`/`score-atp-finals` are manual-only (`TennisAdminScreen.jsx:96`), so — same as F1 — there's no cron to gate, which is the main simplification versus `B-13`. **DB**: `archived`/`archived_at` on `player_boxes`. **UI**: `PlayerBoxScreen.jsx` has no owner-settings panel today either; build it (ideally as the same shared component/pattern used for `B-13-F1`'s paddock settings, not a third bespoke implementation), then filter `TennisHomeScreen.jsx`, `TennisLeaderboardScreen.jsx`, and the clubhouse-level pickers. **Recommendation**: if both this and `B-13-F1` are picked up, build one shared "competition archive toggle" component/hook parameterized by table (`leagues`/`paddocks`/`player_boxes`) rather than three near-duplicate implementations — `useShowArchived` from `B-13` is already sport-agnostic (localStorage-keyed), only the settings-panel half needs generalizing. |
| B-13-CLUBHOUSE | **[FEATURE] Archive/active toggle for whole Clubhouses (`circles`)** | ~3–4 days, contingent on a cascade decision | Found 2026-07-31 (user question: "what would archiving a whole clubhouse take"). Structurally this is a **superset** of `B-13`/`B-13-F1`/`B-13-TENNIS`, not a fourth parallel copy, because a Clubhouse (`circles`) is the container that owns leagues, paddocks *and* player boxes simultaneously (`circle_leagues`/`circle_paddocks`/`circle_player_boxes` junctions, plus `leagues.circle_id`/`paddocks.circle_id`/`player_boxes.circle_id` direct FKs — see [COMPETITION_MODEL.md](docs/architecture/COMPETITION_MODEL.md)) and also owns clubhouse-level surfaces none of the per-competition archives touch: `clubhouse_channels`/`ClubhouseChat.jsx`, `clubhouse_notifications`, `direct_messages`, `p2p_challenges` (circle-scoped), and `ClubhouseFrontpage.jsx`'s daily-generated edition (`generate-frontpage-editions` cron — the one cron job this feature *would* need to gate, unlike the F1/tennis versions). **DB**: `circles.archived`/`archived_at` — cheap on its own (~15 min). **The one genuinely open product question, and the reason effort has a range**: does archiving a Clubhouse cascade to archive its child leagues/paddocks/player boxes (clean single source of truth, but means this item is *blocked* on `B-13-F1`/`B-13-TENNIS` shipping first, since paddocks/player_boxes don't have an `archived` column yet), or is it a purely container-level flag — hide from the nav switcher and stop frontpage/notification generation — independent of whatever archived state the children already have (ships standalone, but now two flags can disagree, e.g. an "archived" clubhouse with an active league inside it)? **Recommendation: cascade.** A non-cascading flag is confusing in exactly the scenario this feature exists for — a dormant pilot clubhouse where the commissioner wants everything paused, not just the badge. **What's easy**: the owner-only `SettingsTab` in `ClubhouseScreen.jsx:358` already has the exact plumbing needed (`onUpdateSettings` patch pattern, already used for `isPublic`/`p2pEnabled` toggles) — adding an `archived` toggle there is a small, well-trodden addition, unlike the F1/tennis items which have to build their settings surface from scratch. **What's not**: filtering the clubhouse switcher (`AppLayout.jsx`), gating `generate-frontpage-editions` per-circle, and deciding whether `p2p_challenges`/chat go read-only or stay fully live in an archived clubhouse (recommend: existing P2P challenges still resolve/expire normally — don't strand real-money-adjacent state — but block *new* ones). **Sequence after `B-13-F1`/`B-13-TENNIS`** if cascading, as recommended; before them (as a standalone container-only flag) if the team wants something shippable sooner and is fine deferring the cascade question. |
