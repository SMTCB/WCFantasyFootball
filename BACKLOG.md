...
**E2E Test Suite**: full `e2e/` suite passing ✅ — 262 passed / 33 conditionally-skipped, 2026-07-17; `platform.spec.js` (36 tests × 2 browsers) passing in CI ✅ (⚠️ see B-14 below — CI's `Security` gate has been failing since ~2026-07-24, which silently skips the `E2E Tests` job on `main` merges)
...
| DATA-3 | **[TECH DEBT] Split the shared "World Cup Pilot" clubhouse back into one clubhouse per league** | ~1–2h (data migration + verification, no app-code changes) | Confirmed via DB query 2026-07-30: all 7 real WC-pilot leagues (`Draft Mundial 26`, `FIXO DRAFT MUNDIAL 26`, `Miami WC Fantasy Testers`, `Munaial '26`, `Mundial do Eder`, `Mundial Gordo Vai a Baliza`, `RANKS FC World Cup Fantasy`) still share one clubhouse (circle `b379c63e-809f-4dc7-9de1-0fff52f989b8`, name "World Cup Pilot", ~30 members, single owner `tommyazcue`). This is a migration-217 artifact, not a deliberate grouping — 217's pre-flight check (2026-06-29) found these 7 leagues orphaned (`circle_id IS NULL`) and they were bulk-parked in one placeholder clubhouse purely so the `NOT NULL` constraint could be applied without corrupting live pilot data (see `217_circle_id_not_null.sql` header comment). 6 of the 7 leagues are commissioned by `tommyazcue`, 1 (`Munaial '26`) by `SB7` — genuinely separate groups, not one community. **Why this outranks pure tidiness**: Clubhouse scope now gates real shipped functionality — Coin Challenges/P2P (migrations 235–239) and clubhouse chat/notifications are all `circle`-scoped (`is_circle_member(circle_id)`), so today any member of any of these 7 leagues can see and P2P-challenge any member of the other 6, none of whom they actually play with. Confirmed zero `p2p_challenges`/`clubhouse_notifications` rows exist yet under the shared circle (2026-07-30) — a clean split, nothing in-flight to reconcile. **Open product decision before implementation** (needs a quick user confirm, not an engineering call): one clubhouse per league (7 new clubhouses, literal reading of the ask) vs. one clubhouse per commissioner (2 clubhouses — `tommyazcue` keeps his 6 leagues together, only `Munaial '26` splits out) — cheaper and may better match how these groups actually socialize. **Implementation sketch**: per new clubhouse, `INSERT INTO circles` (owner = that league's commissioner) → `UPDATE leagues SET circle_id=...` for the league(s) moving → re-home only that league's own `league_members` into the new circle's `circle_members` (not the full shared 30-person roster) → confirm the old shared circle is left with 0 leagues (delete or repurpose, TBD). Live-DB write — SELECT-before-UPDATE + backup per Pilot Safeguards, needs explicit in-session approval before executing. |
...
| ARCH-3 | **[TECH DEBT] Provider-adapter completion** | TBD | Found 2026-07-27 (`B2B_BUYOUT_TECHNICAL_DUE_DILIGENCE.md`). Route the 4 sync functions through `adapter.listEvents()`/`getPlayerStats()` + write an adapter-conformance test suite, so a future owner can plug in a different data provider (e.g. `opta.ts`) without touching the sync functions themselves. Distinct from `ARCH-2`, which only covers the `forza_id`→`provider_key` rename. |
| CODE-7 | **[TECH DEBT] Automated RPC test harness for money/game-logic** | TBD | Found 2026-07-27. Dedicated test harness for the RPCs that move money or decide game outcomes: `execute_transfer_atomic`, `resolve_bet`, `set_lineup`, `place_bid`, `confirm_auction_win`, the coin RPCs, `calculate-scores`. `CODE-4` only covers frontend component tests — this is backend/money-logic coverage, currently untracked. |
| BUG-RB1 | **[BUG] `resolve_bet` refreshes points-type winners' `total_points` before flipping `bet_instances.status` to `'resolved'`** | XS (migration drafted, not applied) | Found 2026-07-30 by CODE-7's first real unit test (`tests/unit/bet.test.js` — "updates league_members.total_points for the winning manager", currently `it.todo` so it documents without failing CI). `aggregate_league_member_points()` sums bet rewards `WHERE bi.status = 'resolved'`, but `resolve_bet()`'s immediate points-refresh loop runs *before* the `UPDATE bet_instances SET status = 'resolved'` statement — so the first refresh silently sums 0 and a winner's leaderboard total doesn't move until some unrelated later event (next scoring pass, `set_captain`, `set_lineup`) happens to re-aggregate them. Confirmed via direct read of the live function body in `supabase/schema.sql`; low real-world impact so far because the pilot is walled and no bet has needed an isolated immediate refresh, but is a genuine correctness bug the moment betting is live again. **Fix drafted, not applied**: `supabase/migrations/242_resolve_bet_status_order.sql` — reorders the existing statements only (moves the status-flip UPDATE earlier), no new columns/logic. Needs explicit in-session "yes, run it" per CLAUDE.md before applying to the live pilot DB; once applied, flip `bet.test.js`'s `it.todo` back to a normal `it`. |
| SEC-5 | **[DOCS] Ownership-transfer runbook** | TBD | Found 2026-07-27 (`PLATFORM_VALUATION_BRIEF.md`) — not yet written. Buyer-diligence adjacent to `SEC-4`. |
| DATA-2 | **[TECH DEBT] Stamp applied-state into migration headers + reconcile DD docs** | XS | Found 2026-07-27 (`B2B_BUYOUT_TECHNICAL_DUE_DILIGENCE.md`). Adjacent to `DATA-1` but smaller/independent — migration files don't currently self-document whether they've been applied to prod. |
| CHIPS-1 | **[FEATURE] Re-enable Triple Captain / Matchday Joker chips** | Small | Currently `CHIPS_ENABLED = false` (hidden for pilot, migration-era comment said "re-enable post-pilot" with no tracking ID). The pilot has ended — now actionable. |
| INFRA-3 | **[TECH DEBT] Reconcile Dockerfile Node 20 → 24 to match `engines`** | XS | Found 2026-07-27 (`B2B_BUYOUT_TECHNICAL_DUE_DILIGENCE.md`). |
...
| CODE-6 | **[TECH DEBT] Consolidate shared UI primitives** | TBD | Includes MarketScreen's progressive-disclosure header polish (noted as "a future P3 item" in old docs, never given its own ID). |
...
| INFRA-2 | **[TECH DEBT] IaC for runtime topology** (Terraform / Supabase config-as-code) | ~2–3 wk | Found 2026-07-27 (`B2B_BUYOUT_TECHNICAL_DUE_DILIGENCE.md`). Distinct from `INFRA-1`, which is docs-only. |
...
| # | Item | Effort | Notes |
|---|------|--------|-------|
| B-14 | **[TECH DEBT] CI `Security` job failing → `E2E Tests` (`platform.spec.js`) silently not running on any merge to `main`** | ~2–3h | Found 2026-07-26 while auditing cutover status. Root cause: `npm audit --audit-level=high` finds 8 high-severity vulns (`postcss` fixable via plain `npm audit fix`; `react-router` 7.12.0–8.2.0 → `GHSA-qwww-vcr4-c8h2`, needs `npm audit fix --force`, bumps to `react-router-dom@7.11.0` — a breaking change requiring verification, same pattern as the `sharp` fix during the 2026-07-24 cutover). This alone is P2, but `.github/workflows/ci.yml:162` gates `E2E Tests` on `needs: [security, lint, build, unit-tests]` — so **`platform.spec.js` (the CI-gating E2E suite) has not actually run in CI on the last 3 pushes to `main`** (PR #764/migration-239 docs commit, the freeform-bets PR itself, and the PRs-A/B/C tracking-docs commit — confirmed via `gh run view`, `Security` fails in ~15s on the `npm audit` step, `Build`/`Lint` pass, `Unit Tests`/`E2E Tests` show as skipped). This has been silently masking the actual PR D-era regression risk — all verification since PR C has relied on locally-run `platform.spec.js` instead. Fix: apply `npm audit fix`, then evaluate the `react-router` breaking bump in isolation (check call sites for RSC-mode-specific APIs, same due-diligence the `sharp` bump got), re-run full CI once green. |
| B-11 | **[BUG] `SquadScreen › shows budget in header` E2E test flaky on both browsers** | TBD | Confirmed pre-existing (fails identically with the Coin Challenges PR D changes stashed) — not a regression from that work. Not yet root-caused; needs investigation into whether it's a timing/data-seed issue or a genuine UI regression. |
| B-12 | **[TECH DEBT] The 8 non-`platform.spec.js` E2E specs default to hitting live production Supabase, with no local Docker target wired up** | ~3–4h | Found 2026-07-26 while investigating whether "run the full E2E suite" made sense now that Docker is available. `e2e/scoring-pipeline.spec.js` (and, by the same copy-pasted pattern, the other 7 integration specs — `draft-allocation-e2e`, `draft-and-scoring`, `draft-mode-complete`, `features`, `multi-league-and-bets`, `scoring`, `autofill-draft-classic`) each hardcode `process.env.SUPABASE_URL \|\| 'https://sssmvihxtqtohisghjet.supabase.co'` (prod) as their fallback, and nothing in the repo (`.env.local`, `playwright.config.js`, no `dotenv` import anywhere in `e2e/`) sets `SUPABASE_URL` to override it — so a bare `npx playwright test` or any direct run of one of these specs writes straight to prod. This is almost certainly how the 4 orphan "E2E EPL Classic" leagues (deleted 2026-07-26 alongside migration 217) got created — timestamps match a documented 2026-07-25 incident (BACKLOG.md "Process note (PR D session)") where a bare `npx playwright test` was run and killed mid-flight; the "no live data was written" note from that session turns out to have been incomplete — 4 leagues had already been created before the kill. Separately, `.env.local`'s `VITE_SUPABASE_URL` is set to `http://127.0.0.1:54321` (local Docker), meaning the *app* these specs drive and the *direct-DB assertions* the specs make can silently target two different databases depending on which env var a given code path reads — a second footgun independent of the prod-default issue. Fix: wire a real local Docker Supabase target (`npx supabase start` + full migration replay + seed fixture data matching what these specs expect, e.g. the GW35 fixture) and point both `VITE_SUPABASE_URL`/`SUPABASE_URL` at it consistently, OR at minimum make the hardcoded fallback fail loudly instead of silently defaulting to prod. Until fixed, do not run these 8 specs without first explicitly exporting `SUPABASE_URL`/`SUPABASE_ANON_KEY` to a non-prod target. |

### P2 — MEDIUM

| # | Item | Effort | Notes |
|---|------|--------|-------|
| B-13 | **[FEATURE] League archive/active toggle (commissioner-controlled, reversible)** | ~8–10h (2 PRs) | Requested 2026-06-15. Full requirements + decisions below. |
