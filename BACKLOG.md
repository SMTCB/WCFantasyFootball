...
| CHIPS-1 | ⏸️ **ON HOLD** — **[FEATURE] Re-enable Triple Captain / Matchday Joker chips** | Small | Currently `CHIPS_ENABLED = false` (hidden for pilot, migration-era comment said "re-enable post-pilot" with no tracking ID). The pilot has ended so it's technically actionable, but **user put it on hold 2026-07-31** in favor of `B-13`/`BUG-RB1`/`ADMIN-TAB-1` — do not pick up unprompted. |
