...
## 🔍 Clubhouse RPC/schema bugs + standalone-competition UI gap (2026-08-26) — found via user-supplied console log, not yet fixed

**Context**: user pasted a raw production browser console log from loading a real Clubhouse (`c987f174-9ed1-47a1-80b6-4c2b2949950b`), flagged as possibly relevant to the still-open empty Members-tab bug (see the PR #838/#839 entry below), plus a separate explicit ask: "one thing critical to check: it should never be possible to create a standalone competition. It always needs to be assigned to a clubhouse." All five items below were diagnosed via live DB inspection (`pg_get_functiondef`, `information_schema.columns`, `pg_constraint`, and one direct RPC execution) and read-only code review — no migration, deploy, or fix applied yet, per the standing per-item-approval rule. Each needs an explicit go-ahead before any write.

- **Standalone-competition invariant — mostly enforced, one live UI hole**: at the DB layer this is solid — `leagues.circle_id`, `paddocks.circle_id`, and `player_boxes.circle_id` are all `NOT NULL` with no default, so no RPC (including a legacy 3-arg `create_league` overload with no circle concept at all, confirmed unreferenced from `src/`) can actually persist a circle-less competition; the insert fails outright. But [LeagueScreen.jsx](src/screens/LeagueScreen.jsx:825)'s "Initialize Campaign" league-creation wizard — reachable from three separate buttons (lines 1779, 1800, 1883), distinct from the newer circle-scoped `NewCompetitionFlow.jsx` — offers a **"Continue without Clubhouse"** button and tells Clubhouse-less users to "continue without linking." Taking that path always fails with a raw, user-facing `NOT NULL` constraint-violation toast rather than either succeeding (it can't) or failing gracefully. No standalone competition is ever created, but the dead-end UI path should be removed — frontend-only fix, no migration needed.
- **`get_circle_feed` RPC broken**: references `gazette_entries.created_at`, a column that has never existed (the table has always used `published_at` — confirmed since its original `02_draft_system.sql` definition). Broken since migration 188; surfaces as a 400 on every Clubhouse activity-feed load.
- **`get_circle_meta_standings` RPC broken**: `RETURNS TABLE(user_id uuid, ...)` collides with a bare `user_id` reference inside an internal `NOT EXISTS` membership-check subquery — ambiguous column, confirmed by directly executing the RPC against production (`42702` error). Breaks Clubhouse standings/meta display.
- **Empty Members tab — now root-caused** (previously investigated in the PR #838 entry below, not conclusively fixed then): `circle_members.user_id` has an FK to `auth.users(id)`, unlike every other membership table in the codebase (`league_members`, `squads`, etc.), which correctly reference `public.users(id)`. PostgREST can't resolve the `users(username)` embed in [useClubhouse.js:73](src/hooks/useClubhouse.js:73) against the wrong-schema FK, so the query 400s with `PGRST200`.
- **Clubhouse Frontpage tab fully broken for every Clubhouse**: [useClubhouseFrontpage.js](src/hooks/useClubhouseFrontpage.js) filters `frontpage_editions`/`frontpage_reactions`/`frontpage_comments` by a `circle_id` column that doesn't exist on any of those three tables — they only have `league_id` (the older, working [useFrontpageEdition.js](src/hooks/useFrontpageEdition.js) filters correctly). Looks like a half-finished piece of the Clubhouse redesign rather than a regression. Needs a product decision before a fix can be scoped: keep Frontpage league-scoped (Clubhouse view resolves via member leagues) or add a real `circle_id` column to all three tables.
- **Also noted, low priority**: the console log shows Sentry error-report delivery silently blocked by CSP — `connect-src` doesn't whitelist the Sentry ingest domain, so production errors aren't reaching monitoring. One-line CSP fix whenever convenient (location not yet confirmed — likely `vercel.json` or `middleware.js`).
