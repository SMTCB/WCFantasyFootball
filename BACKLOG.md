...
**Context**: user reported 4 issues in one message with screenshots; this entry covers the two bugs/features addressed with code this session. One other bug (empty Members tab) and two design-proposal asks (sidebar redundancy, sidebar/nav hierarchy redesign) from the same report are tracked separately below / still owed as write-ups. The third design-proposal ask (draft-mode wishlist feature) shipped as the Wishlist Draft feature — see the "✅ Wishlist Draft" entry below.
