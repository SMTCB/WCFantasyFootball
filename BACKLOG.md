...
**Last Updated**: 2026-06-11 (Fixture-label locale fix + bench club abbreviation — PR #498)  
...
| B-04 | **[FEATURE] Extend fixture-timing indicator to pitch views + mobile list** | 1–2h | PRs #497/#498 added the per-round fixture status (kickoff time / LIVE / FT score) only to `PlayerRow` (desktop LIST tab, via `PlayerCard variant="row"`). The pitch views (mobile + desktop, both render via `PitchView`'s `HybridToken`) and the mobile "squad" list tab (inline-rendered in `SquadScreen.jsx`, not via `PlayerCard`/`PlayerRow`) don't show it. To extend: add a `fixtureStatus` line to `HybridToken`'s non-compact layout (and/or compact pill) and to the mobile squad-tab player rows, reusing `formatFixtureStatus(player.fixtureInfo)` (already populated on every squad player by `SquadScreen`'s `mappedPlayers`). Also: `PlayerCard.jsx`'s "pitch" variant currently has dead fixture-status code added in PR #497 (never rendered — `PlayerCard` is only used with `variant="row"`); either wire it up via this work or remove it. |
