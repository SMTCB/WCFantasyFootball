...
**Deferred, not implemented — need user input before proceeding:**
- **CALENDAR/PICKS overlap + "don't like PICKS tab"** — an open design question, not a bug; needs a recommendation put to the user rather than a unilateral nav change.
- **"Chat is not correct, only for this clubhouse"** — root-caused to Cascais 16's circle (`f5cc62be-1ca3-4d49-a0a5-3997e7319e6c`) having zero rows in `clubhouse_channels`, unlike every other clubhouse which gets a default "General" channel at creation time. Fix is a one-row `INSERT` (`clubhouse_channels(circle_id, name, is_default, created_by) VALUES ('f5cc62be-...', 'General', true, 'd0f0cb5a-...')`) but withheld pending a fresh, explicit in-session "yes, run it" per the standing DB-write approval rule.
