...
**🟠 NOT READY FOR NEW TESTERS — 3 P0 blockers open** (changed 2026-08-01 by the pre-pilot due diligence below; was 🟢 LAUNCH READY). The *codebase* is healthy — lint/build/unit/E2E/function-drift all green, and no P0/P1 bug is open on the football game mechanics themselves. What blocks onboarding real testers is **environment and data state, not code**: `f1_seasons` ships without RLS (world-writable via the public anon key), all 22 pg_cron jobs are frozen, and there is no football competition with a single upcoming fixture for a new tester to actually play. See [🔍 Pre-Pilot Due Diligence](#-pre-pilot-due-diligence--technical--functional-2026-08-01) and the new `DD-P0-*` rows. The V2 cutover (multi-sport platform) is fully merged to `main` and the site remains deliberately walled behind `MAINTENANCE_MODE=true` — see [CUTOVER_PLAN.md](docs/platform_revision/CUTOVER_PLAN.md) status banner. Do not touch that flag without an explicit user ask in the current session.
...
- **Two backlog items re-confirmed still valid**: `SENTRY_DSN` is absent from Supabase secrets (`OPS-SENTRY` stands); the GitHub PAT is still embedded in the git remote URL (`SEC-4` stands).
...
|---|---|---|
| 1 | 🔒 `DD-P0-1` — enable RLS on `f1_seasons` | Only true security defect found. Independent of everything else, ~15 min, and the exposure is live *right now* regardless of the maintenance wall. Do it first because nothing else has to be true for it to be safe. |
...
| # | Item | Effort | Notes |
|---|------|--------|-------|
| DD-P0-1 | **[BUG] `f1_seasons` has no RLS — world-readable *and world-writable* via the public anon key** | ~15 min (1 migration) | **Confirmed empirically against production**, not inferred: an unauthenticated `GET /rest/v1/f1_seasons?select=*` with the shipped anon key returns the live 2026 row, while the RLS-protected control `round_backups` correctly returns `[]`. Root cause: migration `248_trophy_ledger_multisport.sql` created the table with no `ENABLE ROW LEVEL SECURITY` statement, and Supabase's default grants hand `anon`/`authenticated` full `SELECT/INSERT/UPDATE/DELETE/TRUNCATE` on new public tables — so RLS is the only defense and it isn't on. It is the **only** table in the whole public schema with RLS disabled. **The maintenance wall does not mitigate this**: `middleware.js`'s own header says it "blocks page loads only — it does not and cannot block direct calls to the Supabase REST/Realtime API from a browser that already has the anon key," and the anon key ships inside the client bundle. **Impact**: `f1_seasons` is read by `score-f1-race` and `award-season-trophies` for season-trophy attribution — a truncate or a tampered `starts_at`/`ends_at` silently misattributes or destroys F1 season trophies. **Fix**: new migration — `ALTER TABLE f1_seasons ENABLE ROW LEVEL SECURITY;` plus one read-only policy for `authenticated` (writes stay service-role-only, matching how `f1_races` is handled). Append-only rule applies: new file, do not edit 248. Backup first per Pilot Safeguards. |
...
| SEC-4 | **[TECH DEBT] Rotate the GitHub PAT embedded in the git remote URL; switch to SSH** | ~30 min | The PAT-in-remote-URL pattern (originally adopted for the "`gh` CLI is not installed" PR workflow in CLAUDE.md's GitHub API Fallback section) is a standing credential-hygiene risk: the token is recoverable from `git remote get-url origin` by anything with filesystem access, and it is a long-lived classic PAT with `repo`+`workflow` scope. **Still valid — re-confirmed 2026-08-01** (token still present in the remote URL). **The original justification is now gone**: `gh` *is* installed and authenticated (see `DOCS-1`), so the fallback the PAT existed to serve is no longer needed — this can be closed by simply switching the remote to SSH (or to `gh`'s own credential helper) and revoking the PAT in GitHub settings, rather than by rotating it. Sequence: add SSH key → `git remote set-url origin git@github.com:SMTCB/WCFantasyFootball.git` → verify a push → revoke the old PAT → update CLAUDE.md's fallback section (folds into `DOCS-1`). Buyer-diligence adjacent to `SEC-5`. |
...
| DOCS-1 | **[DOCS] Stale facts in CLAUDE.md — `gh` CLI note, E2E test count, and the feature flags that no longer exist** | XS (~1h) | Found 2026-07-31 (ADMIN-1 session), **extended 2026-08-01 by the DD**. Four corrections now needed, all in CLAUDE.md unless noted: (1) the **GitHub API Fallback** section states `gh` isn't installed and directs sessions to a slower Python `urllib` workaround — `gh` *is* installed (`/c/Program Files/GitHub CLI/gh`) and authenticated as `SMTCB` with `repo`/`workflow` scopes; PRs #791/#792 used `gh pr create`/`gh pr merge` directly. Correcting this also removes the original justification for the PAT-in-remote-URL pattern, so do it **with `SEC-4`**, not separately. (2) **`platform.spec.js` is described as "36 tests × 2 browsers"** in three places (Key Commands, Development Guidelines, Known Issues) — it is now **84 tests** (42 × 2), verified 2026-08-01; the same stale count appears in this file's own header and has been corrected there. (3) **CLAUDE.md's repository tree is out of date** — it lists `ChipCard.jsx`, `ChipSelectorModal.jsx` and `LiveJokerCard.jsx` under `src/components/`, none of which exist in the tracked tree, and its `supabase/migrations/` listing stops at "16_*.sql (Next migration to create)" when the real count is 271. (4) **`CHIPS_ENABLED`/`KNOCKOUT_DRAFT_ENABLED` are documented as live feature flags across `MIGRATION_LOG.md` and two archived docs but do not exist in the codebase** — see the corrected `CHIPS-1` row; fix the references so no future session acts on them. **Why this is worth an hour**: CLAUDE.md is the mandated first read every session, so each stale line costs re-derivation time in every session that follows. |
...
| DD-P3-2 | **[TECH DEBT] Untracked working files in the repo root** | XS (~10 min) | Found 2026-08-01 (pre-pilot DD). `git status` shows three untracked paths that have been sitting uncommitted: `docs/platform_revision/Pilot_Engagement_Report.html` and `.pdf` (a generated report — decide whether it belongs in the repo as a deliverable or should be gitignored alongside the other generated artefacts), and `supabase/.temp/start-secrets/` (a Supabase CLI scratch directory that **must** be gitignored — it is under a `.temp/` path and named "secrets", so it should never be committed, and its presence in `git status` is a small standing footgun for any `git add -A`). **Fix**: add `supabase/.temp/` to `.gitignore` immediately; ask the user whether the Pilot Engagement Report is a deliverable to commit or a local artefact to ignore. |
