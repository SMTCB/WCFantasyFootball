...
**Context**: user reported 4 issues in one message with screenshots; this entry covers the two bugs/features addressed with code this session. Two other bugs (empty Members tab) and three design-proposal asks (sidebar redundancy, sidebar/nav hierarchy redesign, draft-mode wishlist feature) from the same report are tracked separately below / still owed as write-ups.
