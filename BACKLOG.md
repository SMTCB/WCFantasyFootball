...
**Last Updated**: 2026-09-12 (Shipped PR #1018 — squad pitch polish: club crests on `MobilePitchField.jsx` player tokens and the player-detail bottom sheet, removed the duplicate Quick Fill/Fill buttons from the Squad screen so Market is the only place with a Fill control, and removed the Field/List toggle now that the pitch view is the only Squad view — see session entry below. Previously: full remediation of the 2026-09-11 Suárez/CODE-RACE-1 scoring-integrity incident — see session entry below. Previously: 2026-09-10 — Shipped PR #1008 — made the draft audit trail (picking order, full round-by-round pick log, and each manager's originally-submitted priority/wishlist lists) a permanent collapsible "DRAFT AUDIT" section of the Market Report tab, closing the last gap in draft transparency; also shipped PR #1000 — Clubhouse activity feed UX (30-day filter on the Home tab Activity section, expand/collapse detail rows matching the League screen pattern) plus three Frontrow/Clubhouse-edition fixes: added a classification/standings section to the generated newspaper front page, filtered `/^test/i`-prefixed accounts (e.g. `test_qa_manager`) out of all generated content in both `collectLeagueData` and `collectCircleData`, and added real transfer/market-activity collection to circle-level editions (`collectCircleData`/`buildCirclePrompt`), which previously had no transfer data at all regardless of actual league activity. Deployed `generate-frontpage-edition` + cleared a pre-existing checksum-drift backlog on `auto-open-transfer-window` (PR #993, never deployed after merge). Regenerated and verified the Mundial do Eder circle's edition only, per explicit scope instruction — confirmed all three fixes live in the output.) (Shipped PR #991 — fixed a GK/DEF conceded-goals scoring bug for tournament 1593 (migration 289, no penalty was being applied for goals conceded due to a stale legacy rules key) and added a click-to-expand points-breakdown detail panel to the League Recap screen, reusing the existing mini-dashboard breakdown logic. Shipped PR #989 — fixed the Wishlist Draft tab silently disappearing while navigating Squad/Live/Market/Digest (a lookup depended on a value that only resolves on the `/league/:id` hub route, not the flat `?leagueId=` screens), added the same "back to league" nav affordance Squad/Live/Market/Digest already have, restyled the Wishlist Save button as a filled accent CTA (was blue-on-blue, illegible), and added club crests next to player names in the Live screen's Points Log. Shipped PR #987 — Wishlist Draft pick-log transparency + formation-floor safety net, so a manager can verify no one manager is systematically favored and a round can never leave a squad unable to field a legal XI. Shipped PR #979 — added a "Pick Order — Snake Draft" infographic to the Market Report draft recap, mirroring round 1/round 2 of the lottery-set order by manager column so the fairness of the mechanism is visible, not just the outcome. Shipped PR #978 — fixed Market Report contested-pick bullets being sorted purely by wanted-by count, which scattered one manager's wins throughout the list; now grouped by winning manager, alphabetical, with wanted-by ranking preserved within each group. Shipped PR #975 — redesigned the Wishlist Draft target-pool list to match the Market screen's `PositionChip`/`StatusDot`/`PlayerStatsPanel` pattern, with expand-in-place stats and a STATS ↗ button. Previously shipped PR #966 — fixed two live pilot bugs: the commissioner draft submit-status tracker in `CommissionerPanel.jsx` was counting any `draft_submissions` row as "submitted," when auto-save/heartbeat write rows with `submitted_at: null` long before a manager actually hits Submit — now only rows with a truthy `submitted_at` count; and the Market screen's Transfer Basket bar, which predated the "Kit Light" light-theme redesign, was using CSS tokens repurposed by that redesign to mean dark-text-on-light, rendering unreadable dark-on-dark text — retokenized to the `--on-shell*`/`--shell-rule*` family already used correctly by this screen's own sticky header; see BACKLOG entry below.) Previously, redesigned the recurring Wishlist Draft screen for draft-mode leagues: fixed commissioner submission-tracker visibility, added a MARKET-adjacent nav button, filtered the player pool to unowned players only, restyled the list to match MarketScreen with expandable stats, added Save alongside Add to List with debounced auto-save + heartbeat, kept the list always-editable after submit, added a "?" info modal, and restyled the header bar; see BACKLOG entry below.) Previously, 2026-09-06 (Shipped PR #955 — fixed desktop squad-list club logos blinking on every 1s re-render and made the Live pitch league name white; see BACKLOG entry below.) Previously, 2026-09-06 (Shipped PR #953 — fixed a live pilot bug where auto-fill could produce an invalid starting XI missing a whole position, e.g. 0 FWD, by enforcing per-position formation minimums in `SquadScreen.jsx`'s self-heal; also added club logos to the Squad pitch/mobile-list and retheme'd the desktop Live pitch to match the Squad screen; see BACKLOG entry below.) Previously, 2026-09-06 (Shipped PR #949 — fixed a live pilot bug where re-clicking or re-entering a Clubhouse invite code you'd already used (e.g. re-tapping a WhatsApp share link) showed a raw `ALREADY_MEMBER` error banner instead of just opening the clubhouse; client-side fix in `useClubhouse.js` only, no migration needed; see BACKLOG entry below.) Previously, 2026-09-06 (Shipped PR #948 — Scoring v3 Phase 2: penalty goals now score a flat +3 via `penalty_scored`, no longer double-counted against the position's goal value; migration 287 applied and both `ingest-match-events`/`calculate-scores` Edge Functions deployed live, checksums synced in PR #950; see BACKLOG entry below.) Previously, 2026-09-06 (Shipped PR #943 — Scoring v3 Phase 1: appearance point, restructured minutes bonus, flat +3 assist, card no-stack rule, migration 285; see BACKLOG entry below. `calculate-scores` Edge Function deploy still pending explicit go-ahead.) Previously, 2026-09-06 (Shipped PR #939, migration 284 — fixed "Database error saving new user" signup bug live in prod, broken since migration 262; see BACKLOG entry below) (Shipped PR #937 — retheme the Squad pitch player-detail bottom sheet + swap banner from dark `--shell` tokens to the app's light `--card`/`--rule` tokens per user report; see BACKLOG entry below. Lint + build verified locally; live visual check not completed, pending user check.) Previously, 2026-09-06 (Shipped PR #935 — added self-hosted UCL club crests + hotlinked WC country flags to Squad/Market player rows via a new `club_logos` table, migration 283; see BACKLOG entry below. Visual browser verification not completed this session, pending user check.) Previously, 2026-09-04 (Shipped PR #922 — fixed misleading transfer-window banner copy and a wishlist drag/scroll UX bug. Banner: draft leagues and other no-penalty situations (pre-competition, squad still building, commissioner free-transfers toggle) were showing the generic "Free transfers available · extra buys cost points" copy even though no penalty ever applies in those cases — added `isDraftLeague`/`noPenaltyReason` props to `TransferWindowBanner` so it now reads "Unlimited transfers · draft league" or the specific reason instead. Wishlist: `touch-action: none` was applied to the entire sortable row to support drag-and-drop, which as a side effect blocked native swipe-to-scroll anywhere over the list — made a long wishlist feel broken on mobile. Moved the drag listeners and `touch-action: none` to a dedicated grip handle only, and switched the touch sensor from a 250ms hold-delay to distance-based activation (5px) to match the pointer sensor, since scroll-vs-drag ambiguity no longer exists once the listener is scoped to the handle. See [TransferWindowBanner.jsx](src/components/TransferWindowBanner.jsx) and [WishlistDraftScreen.jsx](src/screens/WishlistDraftScreen.jsx).) Previously, Fixed sync-tennis-results silently failing for every active tournament since the cron went live — a response-shape parsing bug swallowed a TypeError on every run; also shipped auto-trigger-scoring-on-champion-detection and a daily safety-net cron that force-closes stale tennis tournaments, so one can't be left "available" indefinitely again. Walked Wimbledon through the fixed pipeline end-to-end: champion recorded, 1 roster scored, tournament completed. See BACKLOG entry below, PR #923, migration 280.) Previously, 2026-09-03 (Shipped PR #917 — fixed two regressions reported immediately after PR #916 shipped: (1) the new "+ Add" competition dropdown rendered nothing on click, on both desktop and mobile — root cause was `CompetitionTopBar`'s pill row setting `overflowX: 'auto'` for horizontal scrolling, which per the CSS spec forces `overflowY` to compute to `auto` too, silently clipping the inline absolutely-positioned dropdown popping out below it; fixed by portaling the dropdown to `document.body` via `createPortal`, positioned `fixed` from the trigger button's `getBoundingClientRect()`, dismissed on outside-click/scroll/resize — same pattern already used by `ConfirmModal` and `ClubhouseInviteModal`. (2) `ClubhouseInviteModal`'s WhatsApp invite card was unreadable — white text on a white card, because it used the dark-`--shell`-sidebar-only tokens (`--shell-rule-strong`/`--shell-fill-strong`) on a white `--card` surface; swapped every affected style to the correct light-card tokens (`--card`, `--rule`, `--paper`, `--mute`, `--elev`, `--positive`). Verified in-browser at desktop viewport (dropdown opens correctly, invite modal fully legible); mobile-viewport click verification was blocked by Browser-pane tooling flakiness this session (unrelated to the fix — no viewport-conditional code exists), documented as a caveat in the PR. See [CompetitionTopBar.jsx](src/components/CompetitionTopBar.jsx) and [ClubhouseInviteModal.jsx](src/components/ClubhouseInviteModal.jsx).) Previously, 2026-09-03 (Shipped PR #916 — the Clubhouse "create a new competition" affordance was a low-contrast plain-text "+" that was hard to spot on desktop and mobile; replaced it with a clearly styled "+ Add" pill button on the competition top bar that opens a dropdown offering "New competition" and "Invite to Clubhouse" — one obvious entry point for both actions. Added `ClubhouseInviteModal`, porting the WhatsApp share pattern from `LeagueInviteCard` (present back when the app only had football leagues) to Clubhouses: `wa.me` share link, copy invite link, copy invite code. Removed the old raw invite-code box from the Clubhouse header in favor of a single "Invite" button that opens this modal — the same flow now works identically on desktop and mobile, with "Copy invite link" as the non-WhatsApp fallback. Added a `/join-clubhouse?code=X` deep-link route (`JoinClubhouseRoute` in `App.jsx`) mirroring the existing `/join?code=` league flow, so links shared via WhatsApp resolve to an auto-join on `ClubhouseScreen` with inline loading/error states. See [ClubhouseInviteModal.jsx](src/components/ClubhouseInviteModal.jsx), [CompetitionTopBar.jsx](src/components/CompetitionTopBar.jsx), and [ClubhouseScreen.jsx](src/screens/ClubhouseScreen.jsx).) Previously, 2026-09-02 (Shipped PR #914 — fixed a routing bug: clicking the SQUAD or MARKET tab in the competition nav bar dropped the `leagueId` query param, so `SquadScreen`/`MarketScreen` fell back to fetching every league across every clubhouse and rendering the legacy cross-clubhouse `SelectLeaguePicker` screen instead of staying on the already-selected league. `CompetitionScreenNav` now carries the current `leagueId` across the SQUAD and MARKET tabs (both per-league screens); LIVE and RECAP are left untouched since those are intentionally cross-league views with no `leagueId` param. See [CompetitionScreenNav.jsx](src/components/CompetitionScreenNav.jsx) and [AppLayout.jsx](src/components/AppLayout.jsx).) Previously, **Last Updated**: 2026-09-02 (Shipped PR #912 — fixed the desktop Pitch view player token cards, which had a hardcoded dark background (`rgba(15,18,24,.92)`) paired with text colors (`var(--paper)`, `var(--mute)`) that are WCAG-AA-tuned for light backgrounds, making the cards nearly unreadable. Swapped both `HybridToken` variants in `PitchView.jsx` to `var(--card)` (white), matching the already-correct bench strip pattern in `SquadScreen.jsx`, and removed the now-meaningless `backdropFilter: blur(4px)` on the opaque background. Flagged by the user via screenshot ahead of the Wishlist Draft test with Tommy Azcue. Previously, 2026-09-02 (Shipped PR #911 — fixed a live production bug in `get_wishlist_draft_status()` (migration 278): the RPC declared its tournament-id variable as `uuid`, but `leagues.tournament_id`/`fixtures.tournament_id` are `text` (Forza tournament IDs like `"429"`), so every call threw `invalid input syntax for type uuid` — silently, since `useWishlistDraft.js` discards the RPC error before checking `available`. Net effect: the recurring Wishlist Draft banner (PR #841) has never rendered for any user in any draft-mode league since it shipped. Found while seeding test data for the user + Tommy Azcue in the DRAFT_WISHLIST clubhouse; verified the fix against both the test league and a real production league (Draft Mundial 26) — both now correctly report `available: true, round_number: 9`. Also completed the test-data seeding requested for Phase D: added Tommy Azcue (existing user) as a member of the DRAFT_WISHLIST clubhouse and the `football_draft_wishlist` league, and inserted two full non-overlapping 15-player squads (one for the user, one for Tommy, `matchday_id '429-r8'`, valid 11-player starting XIs, within the €100M budget cap each) so both can now open the Wishlist Draft screen for round 9 and submit real target/drop picks. Previously, 2026-09-02 (Shipped PR #909 — added a "Wishlist Draft" control to the League Admin panel's League Controls section, for draft-mode leagues: reads `get_wishlist_draft_status` to show the current round and availability, and a button calls the existing `run-wishlist-draft` Edge Function (commissioner-JWT direct-call mode) to force-resolve the round on demand, instead of only via the automatic inline trigger or the inactive hourly cron sweep. Closes the gap flagged during recurring-Wishlist-Draft testing (PR #841 shipped the feature and allocator, but no admin-panel trigger existed). Confirmed via code/BACKLOG audit that PR #841's Wishlist Draft feature (10-target/5-drop recurring transfer wishlist, distinct from the older season-opening blind draft wishlist) has in fact shipped and is live. Next: seed test data (add existing user Tommy Azcue + run initial squad allocation) in the user's dedicated DRAFT_WISHLIST clubhouse/league so the user and Tommy can jointly test the feature — pending per-item DB-write approval.) Previously, 2026-09-02 (Shipped PR #907 — fixed `invalid input value for enum league_format: "draft"` on Draft-league creation from the Clubhouse competition-creation modal. `NewCompetitionFlow.jsx`'s format dropdown had its value/label pairing backwards: it sent the literal string `'draft'` (not a valid `league_format` enum value — the enum only has `classic`/`auction`/`noduplicate`/`hybrid`) for the "Draft" option, and used `'noduplicate'` (the real Draft-mode value, used consistently everywhere else in the app) mislabeled as "Classic" and as the default — meaning every league created via this modal with the default "Classic" selection was silently created in Draft mode instead. Checked prod impact via `SELECT`: only one league was ever created through this modal (a "Smoke Test League", 2026-07-26) — no real pilot leagues affected, no data cleanup needed. Also, same session: ran an approved one-row `UPDATE tournaments SET available_for_league_creation = true WHERE forza_id = '1593'` to flip UEFA Champions League 2026-27 into the league-creation tournament list (data/fixtures/scoring rules already fully loaded — 1,075 players, 144 fixtures, 5 scoring rules — it had simply never had this readiness flag flipped since its addition, migration `56_tournament_availability.sql`; this was the deeper cause behind PR #905's `TOURNAMENT_NOT_FOUND` on UCL 26/27, on top of the `forza_id`/`id` bug that PR fixed). Previously, 2026-09-02 (Shipped PR #905 — fixed three clubhouse/competition-creation bugs: league creation was passing `tournaments.id` instead of `tournaments.forza_id` to the `create_league` RPC, causing `TOURNAMENT_NOT_FOUND` on UCL 26/27 (and any other tournament) — `NewCompetitionFlow.jsx` now fetches/selects `forza_id` filtered to `available_for_league_creation = true`; added a "+" button to the Home dashboard header that opens/scrolls to the existing create-Clubhouse form; fixed `ClubhouseScreen`'s empty-state "CREATE A COMPETITION" button, which navigated to `/league` instead of opening the shared `NewCompetitionFlow` modal — lifted the modal's open/close state into `useClubhouse()` so both `AppLayout`'s top-bar "+" and this button trigger the same modal, scoped to the active Clubhouse. Pure frontend change, no migration or Edge Function deploy needed. Verified via clean build+lint and 84/84 `platform.spec.js` E2E passing; local demo-mode auth (`VITE_AUTH_ENABLED=false`) prevented a full backend E2E check of bugs #1 and #3, verified via code tracing instead. Previously, 2026-09-01 (Shipped PR #895 — PR 4 of the approved 8-item P2P social/chat plan (`synchronous-stirring-pretzel.md`), the final item: quick-bet button in Clubhouse chat + a live bet-activity feed rendered inline in chat. New migration `271_clubhouse_bet_activity_messages.sql`: adds `kind`/`bet_ref` columns to `clubhouse_messages` (CHECK `kind IN ('text','bet_activity')`) and a `post_bet_activity_message()` helper (best-effort, wrapped in `EXCEPTION WHEN OTHERS THEN NULL` so a messaging failure never blocks the caller), wired into `create_p2p_bet`, `create_p2p_challenge`, `arbitrate_bet_outcome`, `finalize_declared_bets`, `auto_void_stale_bet_disputes`, `resolve_p2p_challenge`, `confirm_freeform_result`, and `arbitrate_freeform_result` to post a feed card on bet create/resolve only (not join, to avoid channel spam). `useClubhouseChat.js` now selects/forwards `kind`/`bet_ref`; `ClubhouseChat.jsx` renders `kind==='bet_activity'` rows as a compact `BetActivityCard` (question, stake, a VIEW link into the Bets tab) instead of a plain bubble, and adds a 🎲 quick-bet button next to the message composer that routes to `/challenges?tab=bets&new=1` — `ChallengeScreen.jsx`'s existing unified `NewBetModal` now auto-opens from that `new=1` param, pre-scoped to the correct circle via its own existing context, rather than duplicating/exporting its ~1300 lines of modal code into the chat component (resolves the plan's explicitly-flagged open prop-threading-vs-routing question). This closes the full 8-item plan (PRs #887, #889, #891, #893, #895). Found and fixed along the way: the CI "Schema Rehearsal" job (added in PR #856) expects `supabase/schema.sql` to stay at its pre-migration state within a PR, so the job can replay a new migration file cleanly on top — regenerating `schema.sql` from prod in the same PR after applying the migration breaks that job with a duplicate-column error; reverted the regen, `schema.sql` will pick up 271's columns on its next post-merge refresh. Known gap: manual two-account live-browser verification of the real-time feed card was not performed this session (no live two-account test setup available) — all 12 CI checks passed (lint, build, security, unit, E2E, Tier 3 full-stack E2E, schema rehearsal, Android/iOS builds, Vercel). Previous entry (2026-08-31): Shipped PR #893 — PR 3 of the approved 8-item P2P social/chat plan (`synchronous-stirring-pretzel.md`): lifetime win-streak badge for P2P betting. New migration `266_p2p_streak.sql`: `get_my_p2p_streak()` RPC unions resolved outcomes from `p2p_challenges` (`winner_id`) and `p2p_bets`/`p2p_bet_participants` (`is_winner`), oldest-first, deliberately no 30-day cutoff (unlike `get_my_challenges()`, which is fine for an active-bets list but wrong for a lifetime streak). New `src/hooks/useP2PStreak.js` reduces the returned rows into a current win-streak count, reusing the same reduce logic already used client-side in `H2HSheet.jsx` for fantasy H2H streaks (reused the algorithm, not the data source). Renders as a "🔥 N-win streak" pill next to "Season record" in `ChallengeScreen.jsx`'s sidebar, hidden below 2 wins. **This PR also absorbed a P0 production incident found while getting its CI green, plus a real compliance-guard trip — both fixed and applied to production this session, each with fresh in-session "yes, run it" approval before any SQL was written:** (1) a repeated PostgreSQL function-overload ambiguity bug — `CREATE OR REPLACE FUNCTION` only replaces on an exact signature match, so a change in parameter count/defaults creates a second overload instead of replacing the original unless the old signature is explicitly `DROP`ped first — had silently affected `credit_coins`, `debit_coins_to_escrow`, and `release_escrow` (new-user signup coin grants and P2P escrow/refund paths were intermittently resolving to the wrong overload, functionally broken for roughly a day since these functions were last touched) and `seed_cup_clubs` (broken silently for roughly 3 months, zero downstream effect since nothing had called it in that window); fixed via three migrations — `267_fix_credit_coins_debit_escrow_overload.sql`, `268_fix_release_escrow_overload.sql`, `269_fix_seed_cup_clubs_overload.sql` — each re-creating the affected function on its single correct signature and dropping the stray overload; (2) a genuine LEGAL-1 compliance-guard trip: `finalize_bet_payout()` (added by migration 262, `p2p_group_bets`) tripped `tests/unit/coins.test.js`'s "no cash_out/withdraw/payout RPC" schema guard purely on its name — it's an internal-only, already-`REVOKE`d helper that settles virtual-coin wins/losses between escrow and balance within our own wallet ledger, not an external payout of any kind, but the guard doesn't distinguish intent from name; fixed via `270_rename_finalize_bet_payout.sql`, a pure rename to `settle_bet_coins()` (identical body, identical callers — `arbitrate_bet_outcome`/`finalize_declared_bets`/`auto_void_stale_bet_disputes` — re-created on their exact original signatures to avoid reproducing the overload-ambiguity bug above). Two further Unit Tests failures surfaced alongside these were pure pre-existing test-file bugs with zero production impact, fixed without needing migration approval: `coins.test.js`'s insufficient-balance test used a debit amount (9999) that tripped the unrelated `DAILY_STAKE_LIMIT_EXCEEDED` check before reaching the intended `INSUFFICIENT_BALANCE` path (fixed to 250 — over-balance but under the 1000/24h cap); `p2p-challenges.test.js` asserted a stale error string (`DAILY_STAKE_CAP_EXCEEDED`) that didn't match what the shipped RPC actually raises (`DAILY_STAKE_LIMIT_EXCEEDED`). Also closed an audit-trail gap found mid-session: migration 268's file had been applied to and verified against production in an earlier segment of this same session but never committed to git — backfilled in its own commit, explicitly noted as prod-already-live/git-catch-up-only. `supabase/schema.sql` regenerated twice this session to track migrations 267-270. All four migrations backed up first per Pilot Safeguards (e.g. `backups/pre_migration_270_20260831_222347.sql`) and verified read-only against live `pg_proc` post-apply (no duplicate signatures, target functions present/absent as expected). PR #893 finished 12/12 CI checks green (Android/iOS/Web builds, Lint, Security, Schema Rehearsal, Unit/E2E/Tier-3 tests, Vercel). **Not completed this session**: live interactive streak verification — same pre-existing demo-session limitation noted on PRs #887/#889/#891 (no real circle membership to generate an actual streak locally); recommend resolving a few test bets/challenges for a `TEST_`-prefixed account on the live pilot to confirm the pill's count by hand. Next in this plan, not yet started, not yet approved where a migration is needed: PR 4 (quick bet from chat + live bet feed in chat, needs a migration, largest/riskiest, planned last). Previous entry (2026-08-31): Shipped PR #891 — PR 1.5 of the approved 8-item P2P social/chat plan (`synchronous-stirring-pretzel.md`): Open Challenges, closing the "join this bet" gap for 1:1/Football GW challenges by letting a challenger post one without naming an opponent — any Clubhouse member can then claim it (first tap wins), after which it behaves exactly like a normal 1:1 challenge (existing accept/decline/resolve/auto-resolve RPCs untouched). Migration `265_open_challenges.sql` (applied to production with fresh per-item approval in the prior session segment, `supabase/schema.sql` synced) makes `p2p_challenges.opponent_id` nullable, relaxes the `no_self_challenge` CHECK to tolerate NULL, and adds `claim_p2p_challenge(p_challenge_id)` (circle-member-only; fails `ALREADY_CLAIMED`/`CHALLENGE_NOT_PENDING`/`CHALLENGE_EXPIRED`/`CANNOT_CLAIM_OWN_CHALLENGE`/`NOT_CIRCLE_MEMBER`/`NOT_LEAGUE_MEMBER`) and `get_open_challenges(p_circle_id)` (circle-wide, mirrors `get_clubhouse_bets`). This session shipped the frontend: `CreateChallengeModal` (`ChallengeScreen.jsx`) gains a third target-picker row, "🎲 Open — first to accept" (mutually exclusive with the existing "one person" row via paired state-clearing `onClick`s), submitting with `opponentId: null`; a new `OpenChallengeCard` (dashed gold border, single "Claim" button, no Decline) renders in the unified "Incoming" list via a three-way `kind` dispatch (`'challenge' | 'open_challenge' | 'bet'`) added to `useAllBets.js`; `OutgoingCard` now labels a still-unclaimed post "🎲 Open — awaiting claim" instead of a blank opponent name (no RPC change needed — `get_my_challenges` already returns the challenger's own open rows). `useChallenges.js` gained `openChallenges` state + `fetchOpenChallenges`/`claimChallenge`, with the existing `p2p_challenges` Realtime subscription extended to refetch both on any change. Verified: `npm run lint` 0 errors, `npm run build` clean (no TDZ regression — `OpenChallengeCard` added inline in `ChallengeScreen.jsx`, not a new cross-module import), full CI green including a dedicated Schema Rehearsal check re-validating migration 265 (12/12 checks) on PR #891. **Not completed this session**: live interactive claim-flow testing — same pre-existing demo-session limitation as PR #887/#889 (`CreateChallengeModal` correctly shows its "No Clubhouse selected" blocked state; no real circle membership to test against locally); verification relied on diff review + lint/build/CI. No new migration, DB write, or Edge Function touched this session (265 was applied and closed out in the prior session segment). Next in this plan, not yet started, not yet approved where a migration is needed: PR 3 (win-streak badge, needs a migration), PR 4 (quick bet from chat + live bet feed in chat, needs a migration, largest/riskiest, planned last). Previous entry (2026-08-31): Shipped PR #889 — PR 2 of the approved 8-item P2P social/chat plan (`synchronous-stirring-pretzel.md`): bet templates, smart-default targeting, and one-tap rematch, all pure application-layer changes inside `ChallengeScreen.jsx`, no migration. Three one-tap template chips ("Who scores more this GW?"/"Loser buys the round"/"Bench boost gamble") added to `CreateGroupBetModal`'s composer, pre-filling question+stake and leaving the target picker for the user; hidden automatically during a rematch pre-fill. The target-mode picker (One person/Select people/Whole clubhouse) now defaults to the user's last-used choice per Clubhouse via a `bet_target_pref:${circleId}:${userId}` localStorage key, read on open and written on successful submit — a rematch's explicit `initialTargetMode` always takes precedence over the stored default, since a rematch should mirror the original bet, not the user's general preference. A "↻ Rematch" button was added to resolved challenge/bet history cards (`BetCard`'s history section and `HistoryItem`); tapping it reopens the existing create modal pre-filled with opponent/targets/stake/question rather than silently resubmitting the old payload directly — a deliberate design call, since a Football GW rematch needs a fresh gameweek selection (the old `matchdayId` would be stale/already resolved), so `forcedBetType` is still passed but `gw` is left for the user to reselect via the modal's own `useGameweekOptions` hook. For `selected_users` group bets, whose original target list isn't returned by the list RPCs, the rematch handler fetches it on demand from `p2p_bet_targets` only when tapped (mirrors the established on-demand `fetchParticipants` pattern; a `whole_clubhouse` rematch needs no such fetch). Verified: `npm run lint` 0 errors (no new warnings), `npm run build` clean (no TDZ regression), full CI green (lint/security/build/E2E/schema-rehearsal/unit/Tier-3/iOS/Android/Vercel) on PR #889. **Not completed this session**: live interactive verification of the three features — the local demo session has no backing circle membership (`get_my_p2p_bets` returned `permission denied`, the clubhouse switcher listed zero clubhouses for the demo account), the same pre-existing "no live two-account session" limitation noted on PR #887/#880; verification relied on a careful line-by-line diff review against the plan spec instead. No migration, DB write, or Edge Function touched this session. Next in this plan, not yet started, not yet approved where a migration is needed: PR 1.5 "Open Challenges" (needs a migration — fresh per-item approval required before writing any SQL), PR 3 (win-streak badge, needs a migration), PR 4 (quick bet from chat + live bet feed in chat, needs a migration, largest/riskiest, planned last). Previous entry (2026-08-31): Shipped PR #887 — unified the Coin Challenges (`p2p_challenges`, 1:1) and Group Bets (`p2p_bets`) UIs into a single "Bets" tab on `ChallengeScreen.jsx`, per the approved 8-item P2P social/chat plan (see `docs/platform_revision/` plan history / `synchronous-stirring-pretzel.md`) — this is PR 1 of that plan. Root finding driving the design: `p2p_bets`'s `whole_clubhouse` target mode already lets any circle member join server-side (`join_p2p_bet`/`get_clubhouse_bets` were never actually gated) — the "can other people join a bet" gap was a UI-visibility problem, not a backend one, so PR 1 fixes it by putting both bet kinds in one list rather than adding new join logic. New `src/hooks/useAllBets.js` — a pure client-side merge of `useChallenges`/`useGroupBets` output into kind-tagged (`'challenge'|'bet'`) status buckets (incoming/sent/live/awaitingOutcome/disputedBets/ownerArbitration/history); no new RPC, no change to either hook's mutators. `ChallengeScreen.jsx`'s tab bar collapsed from `Challenges | Group Bets | Wallet` to `Bets | Wallet`; the two separate "New Challenge"/"New Group Bet" entry points replaced by one "New Bet" flow (`NewBetModal`/`BetKindPicker`, defined inline to avoid the Vite/Rolldown TDZ same-module-two-depths pattern) that routes Football GW bets to the existing `CreateChallengeModal` (`p2p_challenge`, the only schema with auto-resolve) and Freeform bets of any target shape (one person/select people/whole clubhouse) to `CreateGroupBetModal` (`p2p_bet`) — except when a circle has `p2p_betting_enabled=false`, where Freeform falls back to the legacy challenge modal since `create_p2p_bet` has no server-side flag enforcement; this fallback was directly confirmed working in manual browser testing. `CreateGroupBetModal` gained a new "One Person" target-mode option (single-select) alongside the existing two. `ClubhouseScreen.jsx`'s `NotificationBell` routing simplified to a single `/challenges` destination for both `p2p_bet` and `p2p_challenge` source types (the old `?tab=bets` distinction no longer applies with one tab). Verified: `npm run lint` 0 errors (no new warnings), `npm run build` clean (no TDZ regression, checked mid-edit and final), full required CI green (`gh pr view` confirmed `MERGEABLE`/`CLEAN` before merge; the non-required Tier-3 local full-stack job was still pending at merge time, consistent with its documented manual/non-CI status), manual browser smoke test (desktop unified tab bar/CTA/copy, mobile 375px single action bar, the `betsEnabled=false` fallback path live-confirmed). **Not completed this session**: full two-account manual verification (creating/joining a bet of each target shape, confirming a second real account sees and can join a whole-clubhouse bet from the unified list) — local dev has no real authenticated session to test with, and Claude does not sign in as a live user; flagged in the PR body as a recommended live-pilot follow-up. No migration, DB write, or Edge Function touched this session. Next steps per the approved plan, not yet started, not yet approved where a migration is needed: PR 1.5 "Open Challenges" (join-a-1:1-challenge, needs a migration — fresh per-item approval required before writing any SQL), PR 2 (bet templates/smart default targeting/one-tap rematch, no migration), PR 3 (win-streak badge, needs a migration), PR 4 (quick bet from chat + live bet feed in chat, needs a migration, largest/riskiest, planned last). Previous entry (2026-08-30): Shipped PR #880 — P2P Group Bets UI (Phases 2-4), the frontend for the P2P Group Bets schema/RPCs shipped in migration 262 (already live in production, PR #879). New `src/hooks/useGroupBets.js` hook modeled on `useChallenges.js` (fetch + realtime resubscribe on `p2p_bets`/`p2p_bet_participants` + derived status lists + RPC mutators). New "Group Bets" tab on `ChallengeScreen.jsx` (`/challenges?tab=bets`), gated on `circles.p2p_betting_enabled` (default `false`, currently enabled only on the `TEST_SB_1` test clubhouse for QA), covering the full lifecycle: create a bet (freeform text or multiple choice, whole-clubhouse or selected-members targeting, 10-500 coin stake), join/decline, submit/resubmit an answer, close betting (creator), declare an outcome (creator, opens a 48h dispute window), dispute (participant, within window), and arbitrate (circle owner, on disputed bets — pays out immediately). All new UI (`CreateGroupBetModal`, `SubmitAnswerModal`, `DeclareOutcomeModal`/`ArbitrateOutcomeModal`, `BetCard`, `BetSection`) added as local functions inside `ChallengeScreen.jsx` rather than new child-module imports, avoiding the documented Vite/Rolldown same-module-two-depths TDZ crash pattern. `ClubhouseScreen.jsx`'s `NotificationBell` routing extended one line: `p2p_bet` dispute notifications now deep-link to `/challenges?tab=bets` (previously fell through to the generic `/challenges` route with no tab context). Known accepted gap, deliberately not closed with a new migration this pass: `get_clubhouse_bets` doesn't return per-participant answer detail or `target_user_ids`, so the hook falls back to direct `p2p_bet_participants`/`p2p_bet_participant_answers` table reads (RLS-scoped) for the declare/arbitrate modals and answer pre-fill — a future `get_bet_detail(p_bet_id)` RPC is a plausible follow-up if that proves inadequate live, not built here. Verified: `npm run lint` 0 errors, `npm run build` clean (no TDZ regression), full CI green (lint/security/build/E2E/schema-rehearsal/unit/Tier-3/iOS/Android/Vercel) on PR #880. **Not completed this session**: a real interactive click-through (join/answer/close/declare/dispute/arbitrate, wallet balance checks) — local dev runs in demo mode (`VITE_AUTH_ENABLED=false`, no real Supabase session, every RLS-protected request returns `401`), and a real pass requires signing in as a live user, which Claude does not do (won't enter a password even with explicit permission). Recommend a manual pass against `TEST_SB_1` (flag enabled, session owner is the clubhouse owner) now that it's live. One DB write this session, explicit per-item "yes, run it" approval given in-chat: `UPDATE circles SET p2p_betting_enabled = true WHERE id = 'df0975b3-a20e-41b8-ba76-d4ee48bf8085'` (`TEST_SB_1`) to unblock that manual QA pass — no migration, no other write. Previous entry (2026-08-30): Fixed a user-reported `TOURNAMENT_NOT_FOUND` error when entering a tennis competition from a Clubhouse — screenshot showed "🏠 Clubhouse · TEST_Wimbledon_Box" followed by the error. Root cause: `get_clubhouse_competitions` returns `player_boxes.id` for tennis entries (a Player Box is a *container* of tournaments, sport-flavored per `COMPETITION_MODEL.md`, not a single tournament), but three frontend call sites — `ClubhouseScreen.jsx`'s `AllCompetitions.onEnter`, `HomeDashboardScreen.jsx`'s `enterCompetition`, and `NewCompetitionFlow.jsx`'s `handleJoin` tennis branch — incorrectly passed that player-box id straight into `/tennis/tournament/:id`, which does a strict `tennis_tournaments.id` lookup via `get_tennis_tournament_for_user` and throws `TOURNAMENT_NOT_FOUND` when the ids don't coincidentally collide. Fixed all three to instead call `setActivePlayerBoxId(id)` + `navigate('/tennis')`, matching the already-correct, already-proven F1 `enterPaddock` pattern (`setActivePaddockId` + `navigate('/f1/:id')`) and the existing `PlayerBoxScreen.jsx` `enterBox` helper. Deliberately did not add a new `useSport` import to `NewCompetitionFlow.jsx` (a child of `ClubhouseScreen.jsx`, which already imports `useSport` directly) to avoid the documented Vite/Rolldown TDZ same-module-two-depths crash pattern — the fix needed only a route-target change there, no new import. Two call sites confirmed already-correct and left untouched: `TennisHomeScreen.jsx` and `TennisProfileView.jsx` both navigate using genuine `tennis_tournaments.id` values from real joins, not player-box ids. Verified: `npm run lint` 0 errors (same 677 pre-existing warnings), `npm run build` clean (no TDZ regression), `npx playwright test e2e/platform.spec.js` 84/84 passing. Live browser click-through was not possible this session — local dev runs in unauthenticated "Demo Manager" mode with zero clubhouses for the demo user, so real Player Box/Clubhouse data (like production's `TEST_Wimbledon_Box`) can't be reproduced locally without a real Supabase login; verification relied on lint/build/E2E plus a careful manual trace of the RPC return shapes and route definitions against the already-proven F1 pattern. No migration, DB write, or Edge Function touched this session. Previous entry (2026-08-30): Corrected stale maintenance-wall documentation across `CLAUDE.md`, `CUTOVER_PLAN.md`, and this file's status banner — the user reported `MAINTENANCE_MODE` came down 2026-08-30 (corroborated by `vercel env ls production`'s recent-update timestamp on the var and a live screenshot of the fully-functional app, not the `503` maintenance page); all three docs previously asserted the site was still walled behind it, which was stale, so their standing-instruction language was updated to reflect the wall is down while preserving the underlying rule that re-enabling `MAINTENANCE_MODE` still needs an explicit user ask in a current session. Flagged along the way that cron-job re-enable status is separate and unconfirmed — `DD-P0-2` previously recorded 0/22 active and nothing in this session's evidence shows that changed, so do not assume scoring/sync crons are running without checking `cron.job` first. Also fixed a UI bug from the same user report, a live screenshot of the Home screen: `CompetitionTopBar` (the "🏠 Clubhouse · <competition>" pill row) was rendering unconditionally in `AppLayout.jsx` for every route, including the cross-clubhouse `/home` dashboard, where it has no single active competition to represent — gated it to skip rendering when `location.pathname === '/home'`. `CompetitionScreenNav` (the sport-screen sub-nav) already self-gates on pathname and needed no change. No migration, DB write, or Edge Function touched this session. Previous entry (2026-08-30): Shipped PR #870 — fixed a user-reported 404 when opening a tennis competition from the Clubhouse "all competitions" grid: `ClubhouseScreen.jsx`'s `AllCompetitions.onEnter` tennis branch navigated to `/tennis/${id}`, which matches no registered route (correct path, confirmed as the only registered tennis-tournament route, is `/tennis/tournament/${id}`) — this was a previously-known, deliberately-deferred dead link flagged in the PR #866 BACKLOG entry, now actually hit by a real user and fixed. Also changed the app's root-path redirect (`App.jsx`) from `/clubhouse` to `/home` per explicit user request ("when I open the platform it travels to the home page"), and updated `platform.spec.js`'s "404 redirect to home" test to match the new default. Investigated the user's third ask, "ensure the bottom bar is always available" — live-browser-verified (375×812) that the mobile bottom bar already renders unconditionally on every `AppLayout`-wrapped route, including the 404 page itself; it's a sibling of the scrollable main-content div with no ancestor `transform`/`overflow` that could break its `position: fixed`, and is only hidden by design at desktop widths ≥1024px (`lg:hidden`) where the sidebar nav takes over — concluded no code change was needed there. Verified: `npm run lint` 0 errors (same 677 pre-existing warnings), `npm run build` clean, `npx playwright test e2e/platform.spec.js` 84/84 passing, full CI green (lint/security/build/E2E/schema-rehearsal/unit/Tier-3/iOS/Android/Vercel). No migration, DB write, or Edge Function touched this session. Previous entry (2026-08-30): Shipped PR #868 — mobile-viewport follow-up feedback on PR #866's Clubhouse/Home work: `HubTopbar`/`HubLeagueHeader` (`src/components/league/HubShared.jsx`) was vertically flush-to-top, not centered — fixed via `alignItems: 'center'`; its cup-phase/DRAFT·H2H badge text used `--gold` (tuned for light/paper surfaces), low-contrast against the dark `--accent` bar — swapped to `--on-shell-gold` (the existing WCAG-AA gold token for dark shell surfaces); confirmed the bar's blue is indeed the canonical brand accent (`--accent`/`--brand-accent: #1A6FA8`) per `docs/brand/tokens.css`, per the user's question. Bigger change: the mobile bottom bar was redesigned from sport-scoped tabs (Live/Squad/Market/etc., different per sport, duplicated across football/F1/tennis) to clubhouse-level tabs (Home/Club/Trophy/Coins) — root cause was that sport-scoped screens already live in `CompetitionScreenNav` at the top of every screen on both mobile and desktop (confirmed no responsive hide class), making the old bottom bar fully redundant with it; 3 new nav icons added (`NavIconHome`/`NavIconTrophy`/`NavIconChallenges` in `NavIcons.jsx`) matching the existing hand-authored schematic icon style. `platform.spec.js`'s bottom-nav navigation test updated to assert the new Home/Trophy tabs instead of the removed Market/Squad. Also investigated and answered two open questions from the user without a code change: confirmed the new mobile Home dashboard (`HomeDashboardScreen.jsx`, shipped in PR #866) already renders cleanly at 375px, live-verified in the browser preview; and recommended keeping the existing `ClubhouseSwitcher` desktop-only (it already never rendered on mobile — `hidden lg:flex` — and the new mobile Home tab now covers the same "jump between clubhouses" job). **Not resolved this session**: a reported "loose"/doesn't-fill-the-window feel on iPhone 17 Pro specifically — `index.html`'s viewport meta (`viewport-fit=cover`) and `index.css`'s safe-area handling were inspected and are already correct, so the root cause remains unidentified; needs either a live-device screenshot or Capacitor/WKWebView-specific diagnosis not yet performed. Verified: `npm run lint` 0 errors (same 677 pre-existing warnings), `npm run build` clean, `npx playwright test e2e/platform.spec.js` 84/84 passing (test updated to match the new nav), full CI green. Live-browser verification (375×812) confirmed the new bottom bar and Home dashboard on real rendering; the header alignment/gold-text fix was verified by code only — the session's demo account has no populated leagues to render `HubTopbar`/`HubLeagueHeader` against. No migration, DB write, or Edge Function touched this session. Previous entry (2026-08-30): Shipped PR #866 — Clubhouse UX bug-fix batch + new cross-clubhouse Home dashboard, see the new "✅ Clubhouse UX bugs + cross-clubhouse Home dashboard" entry directly below. Previous entry (2026-08-30): Documentation close-out following PRs #863/#864: un-skipped the 6 `dispute`/`arbitrate` tests in `tests/unit/p2p-challenges.test.js` that had been `.skip()`-marked citing `BUG-P2P-DISPUTE` — now that migration `261_fix_p2p_dispute_notification_check.sql` is live, those tests run for real; reloaded the local Tier 1 test DB (`bootstrap.sql` → `schema.sql` → `seed.sql`, after clearing a stale `auth.users` table left over from before the fix) and verified all 190 unit tests pass, 0 skipped, 0 failed. Fixed a stale cross-reference in `docs/testing/TESTING_STRATEGY.md` that still described `BUG-P2P-DISPUTE` as active/unresolved. Audited `MIGRATION_LOG.md`, `TRACKER.md`, and `DOCS_INDEX.html` against CLAUDE.md's update triggers — none needed changes (the first two are selective/historical by design, the changes here don't touch any `DOCS_INDEX.html` trigger category). See the updated `TEST-P2P-1` row below. No migration, DB write, or Edge Function touched this session. Previous entry (2026-08-28): Shipped P2P-challenge, direct-bet, and Stripe-free mock-payment test coverage — the user asked for a plan given the upcoming pilot runs with no Stripe, fake coins only, then approved it. Closed three confirmed zero-coverage gaps: (1) new `tests/unit/p2p-challenges.test.js` covers the full P2P challenge lifecycle (`create`/`accept`/`decline`/`cancel`/`resolve_p2p_challenge` plus the freeform `declare/confirm/dispute/arbitrate` chain), including direct regression tests for two real bugs this code has shipped before (migration-204/235's double-refund on decline/cancel, migration-205/224's tie double-credit on resolve); (2) `tests/unit/bet.test.js` extended with `submit_bet` RPC coverage (happy path, re-submit resets `is_correct`, the migration-98/AUDIT-57-02 `UNAUTHORIZED` ownership regression, `BET_NOT_FOUND`/`BET_CLOSED`/`DEADLINE_PASSED`); (3) two new Tier 3 specs — `e2e/bet-lifecycle.spec.js` (proves the commissioner-only RLS policy on `bet_instances` actually holds, both the positive and negative case, plus a full create→submit→resolve→points-update cycle) and `e2e/p2p-challenge-lifecycle.spec.js` (a live create→accept→declare→confirm cycle with service-role escrow assertions) — plus a `MOCK_PAYMENTS=true` mock-purchase test added to `e2e/wallet-screen.spec.js` (Stripe-free `purchase-coins` path, gitignored `supabase/functions/.env` for the local Edge Runtime). Writing `p2p-challenges.test.js` surfaced a real, previously-unknown, currently-unfixed production bug — **`BUG-P2P-DISPUTE`**: `dispute_freeform_result` inserts `source_type='p2p_challenge'` into `clubhouse_notifications`, but that table's CHECK constraint only allows `'league'|'paddock'|'box'|'clubhouse'`, so every real dispute call throws `23514` and rolls back. Latent (zero `p2p_challenges` rows in prod today) but will break the first real dispute. A fix migration was drafted but **deliberately not applied** — out of scope for this PR and gated behind CLAUDE.md's per-session migration-approval rule; see the new `BUG-P2P-DISPUTE` row in the P1 table. Also fixed, as production code needed to make `e2e/bet-lifecycle.spec.js` correctly assert real signed-in-session behavior: `src/screens/LeagueScreen.jsx` gained a `resolveUserId()` helper (same established pattern as `F1RacePickForm.jsx`/`TennisAdminScreen.jsx`) so its squad/draft/listing lookups use the real authenticated session id, not `useAuth()`'s possibly-frozen demo identity. Full-suite verification also caught and fixed a second, unrelated pre-existing flake in `e2e/tennis-screens.spec.js`: SUITEs 2/3/5 reset-then-mutate rows keyed to a single shared `USER_A` identity, and `fullyParallel:false` does not stop `desktop-chrome`/`mobile-chrome` running the same spec concurrently in separate workers against the one shared local DB — the same race class already fixed once this session in `wallet-screen.spec.js`'s balance test — fixed by restricting all three tests to `desktop-chrome` only via `test.skip(testInfo.project.name !== 'desktop-chrome', ...)`. A second, unrelated real bug surfaced by that same full-suite pass under heavier concurrent load: `F1RacePickForm.jsx`'s mount-time fetch of a user's existing picks applied unconditionally in its `.then()` callback, so a late-resolving fetch could silently overwrite picks the user (or a concurrent test) had already started entering — fixed with an `editedRef` interaction-guard set by every onChange in the form and checked before the fetch applies its data. `supabase/seed.sql` extended: a `coin_wallets` row for `USER_B` (P2P-challenge opponent) and one `coin_packs` row (mock-payment pack). No migration applied to production, no Edge Function deployed, no `db query --linked` write — every change is a test file, local-only gitignored `.env`, or docs, except the one in-scope `LeagueScreen.jsx` production fix. See the new "✅ P2P challenge, direct-bet, and mock-payment test coverage" entry below. Previous entry (2026-08-28): Shipped Tier 3 CI wiring + 5 new screen smoke specs + `BUG-DRAFT-SVC` fix — closing the three gaps named when the user asked "what can we tackle in terms of tests now": (1) a new non-blocking `e2e-tier3` CI job now runs `npm run test:e2e:local` on every PR via the pinned Supabase CLI + Playwright's real `chrome` channel, informational only until its real-world timing/stability is observed across a few runs; (2) 5 new Tier 3 smoke specs (`wallet-screen`, `challenge-screen`, `trophy-cabinet-screen`, `settings-screen`, `auth-screen`) close every screen gap except Draft, whose actual gap turned out to be `BUG-DRAFT-SVC` blocking its existing spec rather than missing coverage; (3) `BUG-DRAFT-SVC` fixed test-side only (no Edge Function/migration change, per explicit user scope) — corrected root cause is that `run-draft-lottery` was never designed to accept a service-role caller, not a regression, so both draft specs now sign in as a real commissioner instead. Surfaced and documented a demo-mode split-identity quirk along the way: RPC-derived identity (`get_my_wallet()`) correctly reflects an injected real session, but client-side `useAuth().user.id` comparisons (Challenge/Trophy Cabinet/Settings) always see the frozen demo identity regardless — shaped both seed design and assertion strategy for the new specs. Auth coverage is deliberately smoke-only per explicit user scope (no real `signInWithPassword` round-trip, which would need a second `VITE_AUTH_ENABLED=true` build). No migration applied to production, no Edge Function deployed, no `db query --linked` write. See the new "✅ Tier 3 CI wiring + 5 new screen smoke specs + BUG-DRAFT-SVC fix" entry below. Previous entry (2026-08-28): Shipped Tier 3 F1/tennis E2E coverage — `L-10`/`TEST-E2E-1`: all 14 F1 + tennis screens now covered by two new spec files, `e2e/f1-screens.spec.js` and `e2e/tennis-screens.spec.js`, run against Tier 3's local full-stack harness (`npm run test:e2e:local`). Coverage: smoke assertions on all 14 screens; real click-driven interaction specs for race-pick submission, paddock join by invite code, tennis roster + QF-captain submission, and player-box join by invite code; admin/scoring specs that invoke the real `score-f1-race`/`score-tennis-tournament` Edge Functions against seeded data and assert both the scores table and an `event_win` row landing in `trophy_ledger`. `supabase/seed.sql` extended with an F1 paddock + 3 races (upcoming/unlocked, finished-unscored, finished-and-scored) and a tennis player_box + 4 tournaments (`roster_open`, `qf_captain_open`, `completed`, group-stage-only `atp_finals`). Deliberately out of scope, called out rather than dropped silently: the ATP Finals knockout phase (group-stage smoke only) and the F1 Season Bets "locked" placeholder view. A new migration, `260_fix_paddock_playerbox_members_rls_recursion.sql` (fixes a `42P17` infinite-recursion bug in the `paddock_members`/`player_box_members` RLS policies, needed for the join-by-invite-code specs), ships in this PR wired into the local harness only — **not applied to production**, pending its own separate approval. Two real bugs were found and fixed along the way: a tennis player-box-join desktop-only flake caused by a `waitForURL` regex that matched its own starting URL (resolved the wait instantly instead of waiting for the real post-join navigation, letting the test assert on the DB before the `join_player_box_by_code` RPC had committed — fixed by narrowing the regex to match only the true post-join URL); and a scope leak where wiring a service-role key into the shared `SUPABASE_SERVICE_ROLE_KEY` env var (needed for F1/tennis idempotency resets) accidentally un-skipped two old, unrelated, already-broken `draft-mode-complete.spec.js`/`draft-allocation-e2e.spec.js` tests — fixed by giving the new harness its own distinct env var name (`E2E_LOCAL_SERVICE_ROLE_KEY`), and the two underlying bugs those tests exposed (`run-draft-lottery` rejects a service-role caller; a `draft_submissions` upsert targets the wrong `onConflict` columns) are logged as a new deferred item, `BUG-DRAFT-SVC`, rather than fixed in this pass. Full verification: `npm run test:e2e:local` **265 passed / 31 skipped / 0 failed / 0 flaky** across both `desktop-chrome` and `mobile-chrome` (up from the prior baseline's 225 passed with F1/tennis entirely uncovered), `npm run lint` 0 errors, `npm run build` clean (no Rolldown TDZ regression). No migration applied to production, no Edge Function deployed, no `db query --linked` write this session — everything above is confined to local test-harness files and an ephemeral local Postgres stack. `docs/testing/TESTING_STRATEGY.md` and this file's `L-10`/`TEST-E2E-1` rows updated to match. Previous entry (2026-08-27): Shipped Multi-Sport Test Coverage Pass 1 — 4 new Tier 1 unit suites closing the "football is well-tested, everything else is nearly untested" gap flagged by the user: F1 bet-scoring (`score-f1-race/scoring-logic.ts` extracted verbatim, 14 tests), draft allocation (`_shared/snakeDraft.ts` — already a pure, isolated module with zero tests before this pass — 15 tests, no extraction needed), cup elimination (`sync_cup_eliminations`/`eliminate_cup_club` SQL RPCs, pg-client pattern against real Postgres, 10 tests), and tennis tournament scoring (`score-tennis-tournament/scoring-logic.ts` — the one genuine refactor in this pass: `scoreRoster()` lifted out of an inline per-roster loop inside the `Deno.serve` handler into a named, testable function, verified both by the new suite and a before/after manual trace against a seeded 17-pt roster to confirm identical output — 23 tests). Brings `tests/unit/` to 129 tests / 37 suites, all green; `npm run lint` 0 errors, `npm run build` clean (no Rolldown TDZ regression). Two scope decisions came directly from the user: the relaxation-formula suite was dropped entirely from this plan (the feature itself may be retired — no point testing something that might not survive; revisit as a cheap zero-refactor addition later if it's kept), and tennis was pulled forward into this pass ahead of the schedule despite needing a refactor first, in preference to closing four gaps for cheap and leaving tennis for a later pass. `docs/testing/TESTING_STRATEGY.md` updated: Tier 1 "current coverage" now lists all 4 new suites, the Tier 1 gap list narrowed to just the relaxation formula (on hold) and the two Pass-2 items below, and the Tier 3 e2e gap list updated to reflect that F1/tennis/draft *scoring/allocation math* now has Tier 1 coverage even though their e2e/UI flows remain untested. **Explicitly deferred, tracked as follow-up, nothing silently dropped**: "Pass 2" — ATP Finals scoring extraction (`score-atp-finals/index.ts:129-158`, no pure function exists yet) and `run-reverse-standings-draft`'s own duplicated (not shared with `snakeDraft.ts`) allocation/tiebreak logic, both flagged as needing the same kind of careful refactor tennis just got, not a zero-refactor addition like this pass; and a "Tier 3 multi-sport" e2e entry (F1/tennis/chat/coins/draft-timing flows) to pick up once Tier 3 CI wiring lands (see the Tier 3 Foundation entry below — CI wiring for `test:e2e:local` is still unbuilt). No migration, DB write, or Edge Function deploy this session — all new/existing tests continue to run against the disposable local Postgres test harness, not the linked pilot project. PR pending. Previous entry (2026-08-27): Shipped Tier 3 "Foundation" — build-order steps 2–3 from the testing-strategy close-out below: `supabase/seed.sql` (synthetic multi-sport dataset — 2 deterministic auth accounts, 1 circle, EPL+WC player pools, a hand-worked deterministic scoring scenario, 2 leagues), the 3 previously hard-blocked specs (`autofill-draft-classic.spec.js`, `draft-allocation-e2e.spec.js`, `scoring-pipeline.spec.js`) rewritten onto it, and `scripts/e2e-local.mjs` (`npm run test:e2e:local`) to bootstrap-and-run all 8 gated specs against a local `npx supabase start` stack. Along the way, verification surfaced and fixed a real bug worth flagging on its own: `playwright.config.js`'s `webServer` builds the app via `.env.local`-sourced `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` unless overridden, which point at **production** — every UI-driven spec run locally before this fix was silently built against prod, not the local stack, even though a read-only check afterward confirmed no writes actually landed (RLS/FK/JWT-signature mismatches blocked every attempt). `e2e-local.mjs` now explicitly overrides both vars. Also root-caused and fixed a non-obvious demo-mode/RLS interaction (documented in `TESTING_STRATEGY.md`'s Tier 3 section for future specs) and a local `edge_runtime`-container resource-contention timeout under full-suite concurrency (`autofill-draft-classic.spec.js` test 1.3, fixed via a per-test `test.setTimeout`, not a weakened assertion). Full verification: `npm run test:e2e:local` 225 passed / 31 skipped / 0 failed, `npm run lint` 0 errors, `npm run build` clean (no Rolldown TDZ regression). CI wiring for this tier and Tier 3 step 4 (new F1/tennis/chat/coins/draft-timing/cron/trophy coverage) remain explicitly open — this was "Foundation only" per the user's earlier scope choice. See the new "✅ Tier 3 Foundation" entry below. Previous entry (2026-08-27): Closed out the 4-tier testing strategy per explicit user instruction ("close the test strategy... before marking as closed let's do a full run"): added `tests/unit/admin.test.js` (9 tests, `competition_admins` RPC family, zero prior coverage), landed Tier 3's `supabase/config.toml` groundwork (full local-stack sections — `[db.migrations]`, `[auth]`, etc. — though the seed script/spec-retargeting steps remain open, see the entry below for two newly-discovered scoping complications: a never-existent `e2e-setup` Edge Function and a narrow 4-table RLS demo-user carve-out), and formalized Tier 4 as a new committed, read-only `scripts/TEST_pilot_gate.js` (6 checks against prod — cron failure streaks, inactive crons, stale fixtures, unpriced players, edge-function errors, league hygiene), run once and returning `GATE: PASS`. Ran the full required gate before closing: lint 0 errors, build clean, 67/67 unit tests green. Updated `docs/testing/TESTING_STRATEGY.md` throughout to match actual landed state. See the updated "✅ Testing Strategy" entry below. Previous entry (2026-08-27): Decided and began landing a 4-tier testing strategy (`docs/testing/TESTING_STRATEGY.md`, rewritten — the previous version was stale/self-contradictory) after the user asked for a deliberate, non-duplicative test approach spanning Docker/local and CI ("on git"): Tier 1 unit/RPC (existing, `tests/unit/`, CI-required), Tier 2 schema rehearsal (existing script, promoted into a new CI job `schema-rehearsal` gated on `supabase/migrations/**` changes so a bad migration is caught automatically instead of only when someone remembers to run the script by hand), Tier 3 local full-stack E2E via `npx supabase start` (new — the actual gap-closer, since Auth/RLS/Realtime/multi-user behavior has only ever been testable in production; not yet built, tracked as follow-up), and Tier 4 narrowed live-platform verification (formalizing this session's ad hoc `TEST_QA_Manager` script into a committed pre-launch gate — not yet built). A feature/coverage audit (grep-verified, not assumed) found F1 and tennis have **zero** automated test coverage of any kind, plus confirmed gaps in Realtime chat, admin RPCs, coins/P2P beyond RPC level, draft lottery/wishlist timing, cron-driven scoring orchestration, and trophy-ledger emission — all slated for Tier 3 once built. This session shipped only the lowest-risk foundation piece (this entry): the new `schema-rehearsal` CI job and the rewritten strategy doc; expanding `supabase/config.toml` for the full local stack, a committed multi-sport seed script, retargeting the 8 currently-prod-only E2E specs, and new coverage for every confirmed gap are queued as separate follow-up PRs, not done yet. No migration, DB write, or Edge Function touched this session. Previous entry (2026-08-27): Shipped PR #854 — Phase A UI Polish Pass: 8 cross-sport chrome/UX fixes (per-entry League Activity feed collapse/expand, a new shared `CompetitionRankBadge` component replacing 3 inconsistent rank treatments, a mono-color polish pass on the football League screen, F1's next-race row switched from a left-accent border to a full uniform border, Clubhouse/League header alignment, a new shared `NotificationBell` component replacing the deleted `NotificationPanel.jsx`, a Clubhouse-switch scroll-reset fix in `AppLayout.jsx`, and removal of the duplicate League-level "+ INVITE" button) — plus, ahead of merging, built a reusable passwordless `TEST_QA_Manager` QA account and joined it into two real pilot circles ("Mundial do Eder" football league, "Cascais 16 F1" paddock) plus a newly-created `TEST_Wimbledon_Box` tennis competition, then used it to live-verify all 8 tasks in the Browser pane against real authenticated production data rather than demo mode or code inspection alone. Verified: lint 0 errors, build clean (no Rolldown TDZ regression), 84/84 E2E green, plus a full live-browser confirmation pass of every task. See the new "✅ Phase A UI Polish Pass" entry below. Previous entry (2026-08-27): Shipped PR #852 — fixed 4 of 5 issues from a third round of "Cascais 16 F1" / Clubhouse pilot feedback: a stale 2024-vintage F1 `DRIVERS`/`TEAMS` roster in `f1-data.js` (missing real 2026 drivers/teams like Antonelli, Hadjar, Bortoleto, Lindblad, and the new Cadillac team, plus accent/formatting mismatches) was causing stored picks referencing the missing values to render as blank `<select>`s — replaced with the DB-verified real 2026 grid; a duplicate race-winner badge shown in both the collapsed header row and the expanded RACE RESULT podium box on `F1HomeScreen.jsx` — now suppressed on the header row once expanded; `ClubhouseScreen.jsx`'s Activity-feed click handler was passing a raw `league_id` string to `enterLeague(league)`, which expects a `{id}` object, producing a broken `/league/undefined` navigation — fixed the call shape; and `LeagueScreen.jsx`'s "My Leagues" list was showing every league the user belongs to across all Clubhouses instead of just the active one — now scoped via `circle_leagues`. Also fixed a related root cause in `useClubhouse.js`: `fetchCircleData` only reset Clubhouse-scoped state when switching to no Clubhouse (not between two different Clubhouses) and had no race-condition guard, so switching from an active-Clubhouse with data to one without could briefly leak the previous Clubhouse's activity/standings — now resets state unconditionally on every call and discards stale out-of-order responses via a request-id guard. The 5th item — Clubhouse-level Activity feed appearing to duplicate Recap screen's "My Digest" — was a product question, not a bug (the two feeds have materially different scope: Clubhouse Activity is everyone's activity across every league linked to that Clubhouse, My Digest is your own activity across all your Clubhouses via RLS); put to the user, who chose to keep both feeds as-is. Verified: lint 0 errors, build clean (no Rolldown TDZ regression from the new `useClubhouseContext` import in `LeagueScreen.jsx`), 84/84 E2E green in CI. Live-browser verification limited to items that didn't require Clubhouse membership (F1 roster/winner-badge fixes confirmed live); the Clubhouse-scoped fixes (`ClubhouseScreen.jsx`/`LeagueScreen.jsx`/`useClubhouse.js`) were verified by code inspection only — the session's demo account has no Clubhouse membership, and creating/modifying real users' Clubhouse membership just to test was judged out of scope without explicit approval. See the new "✅ F1 picks roster + Clubhouse activity nav fixes" entry below. Previous entry (2026-08-26): Shipped PR #850 — folded the F1 PICKS tab into CALENDAR (race rows expand in place to reveal the pick form), closing out the last of the 8-item "Cascais 16 F1" paddock feedback batch; also confirmed the Cascais 16 chat-channel `INSERT` deferred from PR #848 was run with in-session approval and is live. See the new "✅ F1 PICKS folded into CALENDAR" entry below. Previous entry (2026-08-26): Shipped PR #848 — 6 of 8 items from a second round of "Cascais 16 F1" paddock feedback: moved the PADDOCKS shortcuts grid into the CALENDAR tab and dropped the now-redundant separator; PICKS screen's lock banner now shows "🏁 Race finished" for scored races (previously misleadingly showed the same "picks open" state as a genuinely-open future race, and separately a stale "picks locked — qualifying has started" for finished races even after the fact); Report screen's per-category score breakdown was rendering "+0" for every stored round because it only read the current `p1/p2/p3` schema keys while all real stored rows use an older lump-sum schema (`podium_p1_pts`/`wrong_spot_pts`/etc.) — added a `normalizeBreakdown()` client-side translation layer plus a new "🎯 Podium (right driver, wrong spot)" breakdown row, verified against real production scoring data; top bar gained a 12px desktop-only spacer above the competition tab strip (was flush against the viewport edge); sidebar's "The FrontRow" nav item had a stray smaller bullet from a leftover `sub` prop, removed for parity with the other four nav items. Verified live against real production data (not demo mode): lint 0 errors, build clean (no Rolldown TDZ), `platform.spec.js` 84/84 green. Two items intentionally deferred, pending user input: (1) the CALENDAR/PICKS-overlap design question and "don't like PICKS tab" feedback — needs a recommendation put to the user before any nav restructuring, not implemented; (2) "the chat is not correct, only for this clubhouse" — root-caused to Cascais 16's circle having zero rows in `clubhouse_channels` (every other clubhouse has a default "General" channel auto-created at circle-creation time; this one predates that trigger or fell through a gap), fix is a single-row `INSERT` but withheld pending fresh in-session "yes, run it" approval per the standing DB-write rule — not yet asked. See the new "✅ F1 paddock feedback round 2" entry below. Previous entry (2026-08-26): Shipped PR #846 — fixed 7 issues reported by the "Cascais 16 F1" paddock creator during pilot testing: admin access denied to the creator (stale `users.is_admin` gate replaced with `is_competition_admin` RPC), Bahrain/Saudi Arabian GP shown as "UPCOMING" despite being cancelled (deleted from `f1_races`, rounds 4-5, after confirming zero dependent bets/scores and dumping a backup — explicit per-item approval given in-session), Dutch GP shown as "UPCOMING" despite already having happened (past unscored races now show "RESULTS PENDING"), a tab-underline artifact staying lit on the home tab found in *two* places (`CompetitionScreenNav.jsx` desktop nav and `AppLayout.jsx` mobile bottom nav — same root cause, same fix, the second instance found and fixed proactively during verification), the Report screen only showing the logged-in user's bets instead of all paddock members' (rewritten to reuse the existing `get_paddock_leaderboard` RPC for membership, no new migration), season standings not flagging points as preliminary (banner added, keyed off `f1_year_results.is_final`), and a request to combine the hero/leaderboard/calendar into one landing view (folded the existing leaderboard preview into the CALENDAR tab). Verified live against the real paddock and its real (non-creator) member data, not just demo mode. Lint/build/84-E2E all green. See the new "✅ F1 Paddock pilot feedback batch" entry below. Previous entry (2026-08-26): Resolved all 5 Clubhouse issues from the user-supplied production console log — PR #844 + migrations 194–196/256–259, all applied to production with explicit per-item approval: standalone-competition UI hole closed (frontend redirect + DB-level `CIRCLE_REQUIRED`/`NOT_CIRCLE_MEMBER` failsafe), `get_circle_feed`/`get_circle_meta_standings` column bugs fixed, Members tab wrong-schema FK fixed, and the Clubhouse-level Frontpage tab unblocked — root-caused as a pure deployment gap (migrations 194–196 were fully built and committed but never applied to prod since the 2026-07-24 cutover) rather than a design gap, confirmed via the user's explicit direction that Frontpage must be genuinely Clubhouse-level, not league-scoped. `schema.sql` regenerated to capture all of it. See the "✅ Clubhouse RPC/schema bugs" entry below. Previous entry (2026-08-26): Shipped PR #841 — Wishlist Draft, a recurring asynchronous transfer-window allocation for draft-mode leagues addressing the cross-timezone fairness gap flagged in the 2026-08-26 user report (see the "✅ Wishlist Draft" entry below for full detail). Migrations 252-255 applied to production, 3 Edge Functions deployed (`run-wishlist-draft` new, `auto-open-transfer-window` and `run-draft-lottery` modified). Rolled out to all draft-mode leagues from day one per explicit user instruction, no single-league pilot. Previous entry (2026-08-26): Shipped PR #839 — sport info popover on Create Competition + collapsible League Activity panel on the Leaderboard screen, and PR #838 — fixed the Trophy Cabinet crash (`TrophyCabinetScreen.jsx` was calling `useClubhouse()` directly instead of the shared `useClubhouseContext()`, creating a second hook instance whose `competitions` state could receive `get_clubhouse_competitions`'s in-band `{error:'NOT_MEMBER'}` sentinel shape and crash on unguarded `.length` access) plus a defensive hardening of `useClubhouse.js`'s `fetchCircleData` against that same sentinel across all 5 of its parallel queries, with `console.error` added on any query failure for future diagnosis. Also investigated a reported empty Clubhouse "Members" tab for a real WC-pilot league — backend/RLS confirmed fully correct (11 real members verified in `circle_members`, `is_circle_member()` RLS policy verified correct, `users` table read policy wide open) but the client-side trigger could not be conclusively reproduced this session; the hardening above is a plausible mitigation, not a confirmed fix — flagged as open, ask to check browser console (now instrumented) if it recurs. See the new "✅ Trophy Cabinet crash + Create Competition/Activity panel UX" entry below. Three open design questions raised in the same user report — sidebar "Clubhouse"/"The FrontRow" label redundancy, a full sidebar/navigation hierarchy redesign, and a draft-mode "wishlist pre-draft" feature to address cross-timezone transfer-window unfairness — have since all shipped: the first two via PR #840 (see below), the third via PR #841 (Wishlist Draft, see above). Only the empty Clubhouse Members-tab bug from that same report remains open, unconfirmed. Previous entry (2026-08-26): Resolved `DATA-5` — restored the League-level Bets tab, dropped by a June 2026 CH-5 regression that stripped it from `LeagueScreen.jsx` nav on the mistaken assumption it'd be rebuilt at Clubhouse level like Chat/Frontpage were. Lint/build/84-E2E all green; live-browser click-through not performed — see entry below for why. Previous entry (2026-08-23): Shipped Design Audit B1 + B2 — white-alpha token sweep and the 7-step type-ramp migration, see the "✅ Design Audit B1/B2" entry directly below. Previous entry (2026-08-23): Ran a live-browser verification pass of the 2026-08 design/UX audit across 8 signed-in screens under local demo mode (`VITE_AUTH_ENABLED=false`) — all 14 original source-derived findings held up against real rendering, and 7 further contrast failures surfaced that source review alone couldn't catch (findings 15–21, appended to `docs/design/DESIGN_AUDIT_2026-08.html`'s ledger): 4 critical — a bottom-nav active-tab label at 3.02:1 and a header "Beta" badge at 3.49–4.24:1 (both global chrome, present on every screen), a red status colour (LIVE badge / position-count chips) at 2.33–2.53:1, and a near-invisible 1.22:1 empty-state caption on the Live screen — plus 3 high-severity residual gaps in the just-shipped A2 fix: `--mute` still fails AA against `--elev` (4.26:1, carries every Settings field label) and against `--shell` directly in at least one un-swept spot (Market screen, 3.19:1), and the old un-migrated `--gold` used as a solid button fill (League's "Join →", 3.85:1). Not yet triaged into BACKLOG priority tiers — see `docs/design/DESIGN_AUDIT_2026-08.html` ledger rows 15–21 and the new "✅ Design Audit — Live Verification Pass" section below for full detail. Docs-only change, no code fixed this session. Previous entry (2026-08-23): Shipped the three P0 actions from the 2026-08 design/UX audit (`docs/design/DESIGN_AUDIT_2026-08.html`, PR #824), clearing findings 1, 2, 3, 7, 14 — PR #827. **A1**: rebuilt `AuthScreen.jsx` sign-in fields — fixed a near-invisible ~1.00:1 fill/border contrast, restored keyboard `:focus-visible` state, corrected label font (JetBrains Mono, not Archivo Black at 9px), raised touch targets to 44px, aligned radius to the system's 6px. **A2**: darkened the global `--mute` token (`#8A97A8` → `#626F80`) for WCAG AA on light surfaces — preceded by sweeping 18 sites across `AppLayout.jsx`, `DraftRecoveryScreen.jsx`, `DraftScreen.jsx`, `SquadScreen.jsx`, `LiveScreen.jsx` where `var(--mute)` was misused directly on the dark `--shell` surface, swapping those to the existing `var(--on-shell-dim)` token first so the global darkening didn't regress contrast there. **A3**: fixed four failing `.ffl-btn` states — `primary:hover`, `gold` rest, `gold:hover`, `danger` text color — plus two invisible white-tinted hover fills on `secondary`/`icon`; `--gold` itself was left untouched (296 other usages) in favor of a scoped `.ffl-btn--gold` override. Verified: lint clean, build clean (no Rolldown TDZ regression), 84/84 `platform.spec.js` passing, live-browser confirmation of computed styles, real `Tab`-key focus-visible state, and parsed CSSOM hover-rule values. Previous entry (2026-08-22): Resolved `CLUB-SWITCH-1` — the Clubhouse sidebar switcher was rendering 60+ duplicate, cryptically-labeled badges (e.g. dozens of identical "WC" tiles). Root cause: `fetchMyCircles()` in `useClubhouse.js` queried `circle_members` with no `user_id` filter, relying entirely on RLS — but that table's RLS policy is scoped per-circle (any member can read every member row for a circle they belong to), not per-user, so the query returned one row per fellow member of each circle, each embedding the same Clubhouse. Fixed by adding `.eq('user_id', user.id)`; also replaced the initial-badge grid with a name dropdown per user request. PR #822, no DB change. Follow-up same day: the open dropdown panel rendered white-on-white (used `var(--ink-2, #14181f)` assuming the variable was undefined, but `src/index.css` defines `--ink-2: #FFFFFF`, a light token — so the dark fallback never applied), fixed in PR #825 with a hardcoded dark shade matching the sidebar's `--shell`. See entry below. Previous entry (2026-08-21): **Post-vacation re-assessment → [🚀 Launch Plan — Pilot Relaunch](#-launch-plan--pilot-relaunch-living-plan-target-2026-09-04)**, the new living plan section directly below this header. Nothing had moved in the 21 days since the DD; all gates re-verified green today (lint, build, 56/56 unit, 84/84 E2E, function-drift, `madge --circular`). The three 2026-08-01 P0s are unchanged in substance — `DD-P0-1` stays resolved, `DD-P0-2` (22 frozen crons) and `DD-P0-3` (no upcoming football fixture) still block onboarding. User set launch scope: **all three sports** (football + F1 + tennis) and **coins/P2P in scope for testers** (no Stripe — the 500-coin signup grant is the only faucet). Plan sequences 12 steps across two weeks to a 2026-09-04 reopen decision, with `L-8` (cold-start funnel walk) as the go/no-go gate. Five new items added to the existing priority tables rather than to new documents: `TEST-P2P-1` + `TEST-E2E-1` (P1), `TEST-SEED-1` + `PAY-1` (P2), `DOCS-2` + `DOCS-3` + `REPO-1` (P3) — of which `DOCS-2` was fixed the same session, because `BACKLOG.html` was rendering a green "LAUNCH READY" badge over a document that says the opposite. Also corrected two stale claims in this file: migration 251 is **applied to prod** (verified live today), not "written, not yet applied". Previous entry (2026-08-01): **Pre-pilot technical + functional due diligence** ahead of onboarding real testers — audit only, no code changed. Status downgraded 🟢 LAUNCH READY → 🟠 NOT READY FOR NEW TESTERS: the codebase passes every gate (lint/build/56 unit tests/84 E2E/function-drift/secret hygiene, zero runtime errors logged since the cutover), but three **environment-and-data** P0s block onboarding — `f1_seasons` shipped without RLS and is world-writable via the public anon key (confirmed empirically against prod, and the maintenance wall explicitly cannot mitigate it); all 22 pg_cron jobs are frozen so nothing scores or syncs; and there is no football competition with a single upcoming fixture, while all 8 leagues point at a finished tournament. Added `DD-P0-1/2/3`, `DD-P1-1/2`, `DD-P2-1/2/3`, `DD-P3-1/2` and a new P0 — BLOCKER table, plus a recommended pre-tester sequence. Also fleshed out all 31 previously one-line open items with scope, effort and a still-makes-sense verdict — of which four changed materially: **`CHIPS-1`'s premise was factually wrong** (`CHIPS_ENABLED` does not exist in the codebase; chips are fully wired and live, just never once used — `chips_used` has 0 rows), `CODE-2` is recommended for rescoping to a typed-boundary slice or closure rather than a 4–6 week TypeScript migration, `INFRA-1` has no subject as worded (the runtime is single-region) and should become a topology/data-residency doc, and `SEC-4` can now be *closed* rather than rotated since `gh` is installed and the PAT's justification is gone. See the [🔍 Pre-Pilot Due Diligence](#-pre-pilot-due-diligence--technical--functional-2026-08-01) entry below. Previous entry (2026-08-01): Corrected the Docker schema-rehearsal workflow after the user asked "does docker schema match prod, did you double-check?" — an actual structural diff found local Docker had genuinely drifted from prod (75/271 migration files fail on a from-scratch replay; root cause an uncommitted prod-only fix to `27_auction_listings.sql`'s FK type). Rewrote `scripts/rehearse-schema.sh` to build the local schema from `supabase/schema.sql` (a verified prod snapshot) instead of replaying migration history; re-verified zero drift across all schema object types. See entry below. Previous entry (2026-07-31): Formalized a Docker schema-rehearsal workflow (`scripts/rehearse-schema.sh`) and used it to re-verify the full tennis scoring pipeline against the real 128-player Wimbledon 2026 field (not synthetic fixture data); found and fixed a local-only PostgREST permission bug along the way, and refreshed `supabase/schema.sql` + repaired `tests/unit/seed.sql`'s drift from it (56/56 unit tests passing) — see entry below. Previous entry (2026-07-31): Resolved `ARCH-1` — trophy emission wired end-to-end: PR #807, migrations 248–249 (sport-polymorphic `trophy_ledger` + `award_trophy()` helper, `f1_seasons` table, `award-season-trophies` function+cron for `season_win`), plus `event_win` wiring in `score-f1-race`/`score-tennis-tournament` and a `TrophyCabinetScreen.jsx` read-column fix; all three functions deployed post-cron-freeze. Also added `docs/architecture/COMPETITION_MODEL.md`, the canonical reference that League/Paddock/Player Box are the same concept under a sport-flavored name (each its own circle-scoped container with real membership + invite flow) — see entry below and the resolved `ARCH-1` row in the P2 table. Previous entry (2026-07-31): Resolved `DATA-3` — split the shared "World Cup Pilot" clubhouse into 7 per-league clubhouses via a live data fix (no PR, no app-code change); see entry below and the resolved `DATA-3` row in the P2 table. Previous entry (2026-07-31): Resolved `CODE-7` — PR #801, extracted `calculate-scores`'s pure scoring logic into `scoring-logic.js` + 24 unit tests, see entry below. Also froze all 21 pg_cron jobs (`active=false`) — the site has been walled behind `MAINTENANCE_MODE=true` since the 2026-07-24 cutover with no live competition, so scoring/sync crons were running against nothing; reversible at Phase 4 reopen, see `CUTOVER_PLAN.md` §Phase 4 step 1. Previous entry (2026-07-31): Resolved `ADMIN-TAB-1` — added the "Other / write-in" affordance to the commissioner manual bet-resolution UI (`ResolvePendingBets` in `CommissionerPanel.jsx`), per the open TODO at `docs/platform_revision/design/design_handoffs/admin_tab/LOGIC.md:106`. No backend change needed: `resolve_bet(p_instance_id, p_answers text[])` already accepts arbitrary text, not just predefined option keys — the gap was purely that the UI only exposed the predefined chips when `options` existed. Added a "+ OTHER / WRITE-IN" chip (shown only when predefined options exist) that reveals a text input; the typed value is added to the same `betResolutionAnswers` array via the existing `toggleBetResolutionAnswer` helper, so it composes naturally with chip picks (a write-in with no matching submission behaves as a de-facto void; a write-in alongside a chip pick acts as a split payout, matching the backlog's "custom void/split-payout" framing) and flows through the unchanged `resolveBet`/`_doResolve` path. Write-ins render as removable "✓ {value} · WRITE-IN" tags. Lint and production build (Rolldown TDZ check) both clean. Not exercised in a live browser session — no pilot login credentials available this session (single-environment app, no dev/demo split); verification was lint + build + code-path reuse review (same hooks/state already used by the existing chip-toggle and free-text-fallback paths). Previous entry (2026-07-31): Resolved `BUG-RB1` — turned out the live pilot DB was never actually broken; migration `232_fix_resolve_bet_points_ordering.sql` had already fixed this in prod, but `supabase/schema.sql` was never regenerated afterward so it still carried the old buggy order, which is what fooled CODE-7's new unit test and the draft migration 242 into thinking it was unresolved. Corrected `schema.sql` directly, deleted the redundant migration 242 draft, flipped `bet.test.js`'s `it.todo` back to `it`. Verified via full local `npm run test:unit` run (32/32 green) — no live DB write, no migration approval needed. Previous entry (2026-07-31): Resolved `ADMIN-1` — Clubhouse/competition admin ownership model shipped: PR #791, migration 243 applied to live pilot DB. New `competition_admins` table + `is_competition_admin()` helper + owner-gated `get_circle_competition_admins`/`set_competition_admin`/`remove_competition_admin` RPCs for `leagues`/`paddocks`/`player_boxes`; new "Competition Admins" section in Clubhouse Settings. Also unblocks `TENNIS-ADMIN-GAP`: the 9 tennis admin RPCs (migration 200) now accept `users.is_admin` platform admins (not just `service_role`), mirroring `F1AdminScreen.jsx`'s existing gate, and `TennisAdminScreen.jsx` got the matching client-side lock screen. `tennis_tournaments` intentionally stays on the platform-admin model, not the new per-Clubhouse one, since it's shared global data with no owning circle. Note: the assign/remove round-trip in the new Settings UI has not been exercised end-to-end in a live browser session (no pilot login credentials available this session) — logic mirrors the already-shipped "Linked Leagues" pattern and passed lint/build/E2E, but flagging this as an open verification gap. Previous entry (2026-07-30): Resolved `B-11` — flaky `SquadScreen › shows budget in header` E2E test root-caused (timing, not a UI regression) and fixed via PR #787; `platform.spec.js` now 12/12 SquadScreen tests green on both browsers. Also resolved `B-12` — the 8 E2E integration specs no longer silently default to production Supabase; PR #785 adds a `supabase-target.js` guard that fails loudly instead. Full local-Docker-target option still open. Also resolved `B-15` (the `B-12` follow-up flagged for `platform.spec.js` itself) — PR #789 drops its hardcoded prod Supabase URL+key fallback in favor of the already-CI-provided `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY`, throws a clear error if none of the four env vars are set, and along the way fixed an unrelated pre-existing `bodyText.toMatch is not a function` bug in the `MarketScreen` test that the hardcoded fallback had been masking. Also resolved `B-14` — CI `Security`/`Unit Tests` gates fixed, `E2E Tests` confirmed running in CI again on PR #784; added `BUG-RB1`, a `resolve_bet` statement-order bug found along the way, fix drafted as migration 242 but not yet applied. Also added `DATA-3` — legacy clubhouse cleanup for the 7 WC-pilot leagues left sharing one clubhouse post-cutover, confirmed via live DB query. See P1/P2 — platform tables. Original 2026-07-27 consolidation note: this file is once again the sole source of truth for all open items, including platform-revision/v2 work previously tracked in `docs/platform_revision/TRACKER.md`. See the [🌐 Platform Revision — Consolidated Open Items](#-platform-revision--consolidated-open-items-merged-2026-07-27) section below.)
 Shipped PR #981 — added a round-by-round draft pick log (audit trail: who picked what, when, and how far down their own wishlist) to the Market Report draft recap and backfilled it onto the existing Draft Champions 26/27 gazette entry; added a quick-add "+" button and explicit "Save Wishlist" button to the Wishlist Draft screen. Redeployed all 25 _shared-importing Edge Functions (function-drift check flagged them after editing snakeDraft.ts) and updated checksums. Also ran a data-only UCL (tournament 1593) price-list sync: inserted 60 new arrivals with prices, deactivated 59 of 60 departed players (1 excluded — rostered on a live squad).
...
**🟠 NOT READY FOR NEW TESTERS — 2 P0 blockers open** (changed 2026-08-01 by the pre-pilot due diligence below; was 🟢 LAUNCH READY; `DD-P0-1` resolved 2026-07-31, see row for detail). The *codebase* is healthy — lint/build/unit/E2E/function-drift all green, and no P0/P1 bug is open on the football game mechanics themselves. What blocks onboarding real testers is **environment and data state, not code**: cron jobs were last confirmed frozen (unclear if re-enabled alongside the maintenance-wall removal, see `DD-P0-2`), and there is no football competition with a single upcoming fixture for a new tester to actually play. See [🔍 Pre-Pilot Due Diligence](#-pre-pilot-due-diligence--technical--functional-2026-08-01) and the `DD-P0-*` rows. **`MAINTENANCE_MODE` came down 2026-08-30** — the V2 cutover (multi-sport platform) is fully merged to `main` and the site is now live and publicly reachable — see [CUTOVER_PLAN.md](docs/platform_revision/CUTOVER_PLAN.md) status banner. Do not re-enable that flag without an explicit user ask in the current session.
**Live App**: https://wc-fantasy-football.vercel.app (live — maintenance wall removed 2026-08-30)
...
- **Club crest badges**: added to `MobilePitchField.jsx` player tokens (top-right corner, next to the captain armband) and to `SquadScreen.jsx`'s player-detail bottom sheet (next to the club name). `ClubCrest` is passed into `MobilePitchField` as a **prop**, not imported directly — `SquadScreen.jsx` already imports it at depth 1, so a second import at depth 2 would trip the Rolldown TDZ crash pattern documented above (see Vite v8/Rolldown section).
- **Fill-button consolidation**: removed the `useAutoFill` hook and all three of its call-sites from `SquadScreen.jsx` — mobile Pitch tab's "Quick Fill", mobile List sub-view's "Fill", and desktop List tab's "Quick Fill". `MarketScreen.jsx` keeps its own independent `useAutoFill` call/button — now the only Fill control in the app, exactly where the user wanted it.
- **Removed the Field/List toggle**: deleted `mobilePitchMode` state and the list-fallback rendering block from `SquadScreen.jsx`; `<MobilePitchField>` now renders unconditionally. This also killed the confusing double-list problem (list view + toggle, on top of the top nav's own list) the user called out.

Verified: `npm run lint` (0 new errors), `npm run build` (clean, zero Rolldown TDZ errors — confirms the `ClubCrest`-as-prop fix), `npx playwright test e2e/platform.spec.js` (84/84 passed, desktop + mobile Chrome). Full manual browser verification with live squad data was not possible in demo mode (`VITE_AUTH_ENABLED=false` — no real Supabase Auth session, so RLS blocks the writes needed to seed a test squad) and was correctly not pushed further once it required entering credentials into a signup form; CI's E2E suite was the verification path instead.
...
**Bug 1 — Suárez/Tresoldi/Guirassy goal+penalty double-subtract** (`scoring-logic.js`): a scoring bug caused certain goal+penalty combinations to be double-subtracted for these three players' `player_match_stats` rows. Root cause fixed and covered by `tests/unit/scoring-logic.test.js` in PR #1009 (merged this session).

**Bug 2 — CODE-RACE-1** (migration 291, PR #1010, merged this session): a race condition where a round-freeze pass (`rollupSquads()` in `calculate-scores/index.js`) could read `player_match_stats` before every fixture in the round had finished writing, permanently undercounting `total` for any squad caught mid-write. Verified via merge-base diff analysis (not just GitHub's "clean merge" status) that merging #1010 could not silently revert #1009's fix — #1010's own branch never touched `scoring-logic.js` or its test file, so the 3-way merge was provably conflict-free on those files.

**Full-DB audit + correction**: ran a generic (non-hardcoded) discrepancy check across every frozen squad+matchday in the database, not just Champions League. Result: 18 UCL squads across 3 real leagues (tournament 1593, round r1) had CODE-RACE-1 undercounts — corrected via SELECT-then-UPDATE per Pilot Safeguards (backup taken first). `CHAMPS_2627_TEST` was excluded from the correction per explicit user instruction (test league, not a real one) — its 1 known residual discrepancy is expected and unchanged.

**World Cup (tournament 429) discrepancies found, left untouched per explicit user instruction** ("Leave the World Cup"): 44 squad-matchday rows across 7 leagues, rounds r1 and r3–r8, showing two patterns — round r3 has CODE-RACE-1-shaped undercounts (−14 to −1); rounds r1/r4–r8 show small ±1/±2 diffs consistent with late `player_match_stats` corrections that were never reconciled. Not corrected this session — revisit only if the user raises it again.

**Closing the gap between "merged" and "actually protects against recurrence"**: discovered via `scripts/check-function-drift.js` that production's `calculate-scores` Edge Function was still running stale code — neither #1009's nor #1010's fix was actually live, despite both being merged to `main` (Edge Functions are not auto-deployed by Vercel). Deployed `calculate-scores` and independently reconstructed its checksum at several recent commits to confirm production now exactly matches `main`.

**New standing safety net — migration 292 + `reconcile-scores` (PR #1011, merged and deployed this session)**: `get_score_discrepancies()` (migration 292, applied to live DB) recomputes each frozen squad+matchday's `total` from its frozen `effective_xi`/captain/joker/penalty metadata against *current* `player_match_stats`, and returns any row that disagrees with the stored total — catching both this bug class (a late stats correction never reaching an already-frozen `total`) and future CODE-RACE-1-shaped undercounts automatically instead of relying on a user bug report. `reconcile-scores` Edge Function (deployed) calls it and logs any discrepancy via the existing `edge_function_errors`/Sentry path (same as `check-cron-health`); read-only, never writes to `fantasy_points`/`league_members` — a human still reviews and corrects manually per Pilot Safeguards. Not yet wired to a schedule (code-only, same as `check-cron-health` before its cron was approved) — needs a separate cron-creation approval if the user wants it running automatically.

**Verification**: `.function-checksums.json` regenerated and committed (PR #1012, merged) — `node scripts/check-function-drift.js` reports all 26 Edge Functions match deployed code. Fresh DB backups taken before both the correction UPDATE and the migration 292 apply.

**Still open / deferred**: World Cup (tournament 429) discrepancies, left alone per user instruction above. Whether Tresoldi's fix should be revisited was never explicitly re-confirmed with the user this session (PR #1009's description states it was already corrected directly in a prior session alongside Guirassy/Suárez).

---

## ✅ Draft audit trail made permanent + initial priority lists surfaced (2026-09-10) — PR #1008

User audit request: does the Market Report tab's draft audit already cover (1) each manager's full originally-submitted list, (2) the picking order, and (3) complete picking-order/pick-log detail? Investigation found (2) and (3) were already built in `GazetteDraftReport.jsx` but presented as an ephemeral-feeling "Draft Edition — {date}" news card wrapped in a conditional render — plus a real durability bug: the `wishlist_draft_report` query was capped at `.limit(12)`, so older rounds could silently drop out of view as a league accumulated rounds. (1) — each manager's full submitted list — was a genuine gap: the data existed (`draft_submissions.player_ids` for the season draft; `full_data.submissions[]` on each wishlist gazette entry) but nothing rendered it.

**Fix**: [MarketReportView.jsx](src/components/league/MarketReportView.jsx) now renders a permanent "DRAFT AUDIT" header (always visible whenever any draft data exists for the league) with a SHOW/HIDE toggle — the picking order + full pick log content only mounts when expanded, so it doesn't crowd the screen by default but is never gone. [GazetteDraftReport.jsx](src/components/GazetteDraftReport.jsx): removed the `.limit(12)` cap on wishlist-round history; added `InitialListsTable` (season draft — each manager's full ranked `player_ids` list, cross-referenced against `pick_log` to mark each entry won / lost-to-another-manager / still-open) and `WishlistListsTable` (per wishlist round — each manager's full `target_ids`/`drop_ids` submission, same won/lost/open cross-reference for targets). Both fetch and render as their own collapsible sub-sections inside the existing draft/wishlist report cards.

Verified via `npm run lint` (0 errors, pre-existing style warnings only) and `npm run build` (clean, no Rolldown TDZ errors). Could not visually verify against the real "Draft Champions 26/27" league this session — no pilot login credentials available, only an unauthenticated demo account with no leagues joined.
...
**Activity feed — 30-day filter**: [useClubhouse.js](src/hooks/useClubhouse.js) filters `get_circle_feed` results to entries under 30 days old immediately after the RPC response lands in state.

**Activity feed — expand/collapse**: added per-entry local `useState` to `FeedEntry` in [ClubhouseScreen.jsx](src/screens/ClubhouseScreen.jsx), matching the show/hide-details pattern already used on the League screen's activity rail.

**Frontrow classification section**: new `ClassificationSection` component in [ClubhouseFrontpage.jsx](src/components/ClubhouseFrontpage.jsx) renders per-league standings (top 3) from `edition.raw_input.leagues`, inserted above the existing Hot Take/Wooden Spoon/Transfer Desk grid.

**Frontrow test-user exclusion**: no `is_test` DB flag exists, so all 5 known test accounts (`TestComm`, `TestMgr2/3/4`, `test_qa_manager`) are matched by a `/^test/i` username-prefix filter. Applied in [generate-frontpage-edition/index.ts](supabase/functions/generate-frontpage-edition/index.ts)'s `collectLeagueData` (members/transfers/chat/squads) and `collectCircleData` (per-league standings) — query limits widened (league members 7→15, circle standings 3→10) so filtering still leaves enough real managers to display.

**Frontrow market/transfer content**: `collectCircleData` previously collected zero transfer data — circle-level editions structurally could never reference the market regardless of actual activity. Added a `squad_events`-based transfer collection block (last 7 days, all linked leagues, test-users excluded) mirroring the existing league-level pipeline, and wired the result through `buildCirclePrompt` as a new "MARKET ACTIVITY" prompt section feeding `transfer_rumour`.

**Verification**: `npm run lint` (0 errors), `npm run build` (clean, no Rolldown TDZ issues), `npx playwright test e2e/platform.spec.js` (84/84 passed). Real-circle UI verification wasn't possible locally (dev server runs with `VITE_AUTH_ENABLED=false` demo mode, no real Supabase session) — instead deployed `generate-frontpage-edition` and invoked it in cron mode scoped to just `circle_id=c987f174-9ed1-47a1-80b6-4c2b2949950b` (Mundial do Eder), per the user's explicit "no need to regenerate all leagues" instruction. Confirmed in the resulting row: `test_qa_manager` no longer appears anywhere, `transfer_rumour`/`raw_input.transfers` contain 25 real transfer lines, and `raw_input.leagues[].standings` is populated for both linked leagues (feeds the new classification section).

**CI note**: PR #1000's "Security" check (a function-drift audit, not a vulnerability scan) also caught `auto-open-transfer-window` as undeployed since PR #993 merged — a pre-existing gap unrelated to this session's changes. Deployed it alongside `generate-frontpage-edition` and committed updated `.function-checksums.json` to clear CI.
...
**Bug fix**: Kobel (GK) conceded 2 goals in a GW1 match with no penalty applied, and the same was true for defenders (e.g. Waldemar Anton) — traced to tournament 1593's GK/DEF `scoring_rules` rows still carrying the legacy unused `conceded_per_goal: 0` key instead of `conceded_2plus_penalty`, so [scoring-logic.js](supabase/functions/calculate-scores/scoring-logic.js)'s `rules.conceded_2plus_penalty ?? 0` silently applied no penalty regardless of scoreline. Migration `289_scoring_v3_conceded_penalty_1593.sql` brought tournament 1593 into parity with tournament 429 (fixed there by migration 175) via surgical `jsonb_set`/`-` operator, applied live; GW1 scores recalculated and confirmed correct against real data.

**Feature**: added a points-breakdown detail panel to the League Recap screen — the recap already had the player "timeline" mini-infograph (cards, goals, etc.) but was missing the full points breakdown already available in the Squad screen's mini-dashboard. Implemented in [RecapView.jsx](src/components/league/RecapView.jsx): each player row in an expanded manager's squad is now click-to-expand, revealing the same color-coded label/points breakdown as `PlayerStatsDashboard.jsx`'s mini-dashboard (reusing `buildBreakdownItems()` from [usePlayerFullStats.js](src/hooks/usePlayerFullStats.js) as-is), plus a captain/triple-captain multiplier row when applicable. Chose click-to-expand-per-player over always-visible breakdowns for all ~15 squad players, to keep the already-dense Recap view uncluttered — user explicitly invited a better layout than their mockup.

**Verification**: `npm run lint`, `npx eslint` on the touched file, and `npm run build` all passed clean; full CI green (build, lint, security, schema rehearsal, unit, E2E, Tier 3 full-stack, Android/iOS builds). Full browser click-through against real league data wasn't possible this session — local dev only has an unauthenticated demo account with no leagues — so data-plumbing was instead verified via a read-only `db query --linked` cross-check against Kobel's real `player_match_stats.breakdown`/`bonus_points` row, confirming the shape matches what `buildBreakdownItems()` expects and the summed values match the stored `fantasy_points` exactly. Worth a quick manual check in a live league's Recap tab post-deploy.

## ✅ Wishlist Draft back-nav, disappearing tab, Save contrast, Live club crests (2026-09-08) — PR #989

Four small UX fixes reported by a pilot user in one session, all frontend-only:

1. **Wishlist tab disappearing while navigating Squad/Live/Market/Digest** — root-caused, not flaky. `CompetitionNav.jsx`'s draft-format lookup (`isDraftLeague`) resolved the league via `activeComp`, which is only set when `extractActiveCompId(pathname)` matches `/league/:id`-shaped paths. The four flat `?leagueId=` screens (Squad/Live/Market/Digest) never match that regex, so `activeComp` was always `undefined` there and the Wishlist tab silently dropped every time the user left the league hub — deterministic, not intermittent. Fixed by resolving the league via the existing, more robust `footballLeagueId` fallback chain (already used elsewhere in the same file) instead of `activeComp`.
2. **Wishlist screen missing "back to league"** — added the same mobile "← Back" top bar + desktop "← Back to League" bar that Squad/Live/Market/Digest already get from `AppLayout.jsx`, by extending its `isLeagueScopedScreen`/`backLeagueId` logic to also match the Wishlist screen's `/league/:id/wishlist` path shape (path param, not query string). Removed `WishlistDraftScreen.jsx`'s own redundant in-header back arrow (an earlier one-off workaround) now that `AppLayout` covers it consistently — avoids a double-back-arrow UI.
3. **Wishlist Save button hard to see** — was `color: var(--cyan)` text directly on the `--shell` header background; both tokens are in the same blue family, so contrast was very low. Restyled as a filled accent CTA (`background: var(--accent)`, white text, rounded), matching the filled-button pattern already used on Tennis/F1 screens.
4. **Club logos in Live screen's Points Log** — added the existing `ClubCrest` component next to player names in both the desktop and mobile Points Log rows (it was already imported in this file and used elsewhere). Kept it on mobile as well as desktop: the component is small (14–16px) and fails gracefully to a dashed ghost ring rather than a broken image if a club isn't mapped in `club_logos`.

Verified: `npm run lint` (0 errors, pre-existing warnings only, none in touched files), full CI green before merge. Local dev has no seeded/authenticated session, so visual confirmation with live data (real Points Log rows, a draft-mode league) was not possible locally — recommend spot-checking the live app.

## ✅ Wishlist Draft pick-log transparency + formation-floor safety net (2026-09-08) — PR #987

Extended the pick-log transparency shipped for the season draft (PR #981, below) to the recurring Wishlist Draft, and closed a formation-safety gap the user flagged: "we don't run the risk of subbing out a MID and subbing in a FWD because of the wishlist the manager submitted and other managers['] wishlist."

Three changes, all in `_shared/wishlistDraft.ts` and `_shared/snakeDraft.ts`:
1. **Pick log**: `runSnakeDraft()` already computed a `pickLog` (used by the season draft) but `wishlistDraft.ts` discarded it — now captured and written into `gazette_entries.full_data.pick_log`, same shape as the season draft's.
2. **In-loop formation guard**: `runSnakeDraft()` gained an opt-in `minFloor` parameter (only `wishlistDraft.ts` passes it — `run-draft-lottery` is unaffected, provably, since it never sets the option). While a manager still has an unmet position deficit against `leagues.min_formation` (live default `{GK:1,DEF:1,MID:1,FWD:1}`), their last few remaining squad slots are reserved for that deficit rather than spent on whatever their wishlist ranks next — so a manager can no longer end a round unable to field a legal XI because their (or another manager's) wishlist happened to be lopsided.
3. **Post-allocation safety net**: `backfillFormationFloor()` — a last-resort backstop for the rare case a manager's own wishlist contains no eligible replacement even with slot reservation. Pulls the cheapest eligible untaken player of the needed position from the full tournament pool (not just wishlist targets), respecting budget and club-cap; if the squad is already at full size, first frees a slot by releasing the manager's most expendable player (priciest player from whichever position sits furthest above its own floor). Every action is logged into `gazette_entries.full_data.formation_safety_net` (`added`/`released` per player) — nothing happens silently. Budget-cap and max-per-club logic were explicitly left untouched per the user's direction ("I leave up to you... what we need to ensure is [the formation-legality risk]").

`GazetteDraftReport.jsx`'s existing season-draft components (`DraftOrderList`, `DraftPickLogTable`) were generalized with optional `title`/`note`/`firstRoundLabel` props (defaults unchanged, so season-draft rendering is byte-identical) and reused inside a rewritten `WishlistRoundReport`, which now renders pick order, the full pick log, and — only when non-empty — an amber "Formation Safety Net" callout explaining what was swapped and why. Old gazette rows without `pick_log`/`formation_safety_net` still render fine (those fields default to empty).

Same `_shared/*.ts` drift-check consequence as PR #981: editing `snakeDraft.ts` and `wishlistDraft.ts` flagged all 25 functions that bundle `_shared`. Redeployed all 25 and committed updated `.function-checksums.json`.

Verified: `npm run lint` (0 errors, pre-existing warnings only), `npm run build` (clean), full CI green before merge. Visual verification of the new report UI is still pending — no wishlist round has run through the new code path in production yet; the deficit reservation and safety-net logic were verified by manual trace instead (no `deno` CLI available locally to type-check the Edge Function TypeScript).

## ✅ Draft audit trail — round-by-round pick log with wishlist rank (2026-09-08) — PR #981

The Draft Order Board (PR #979, above) showed the snake-draft *rotation pattern* but not an actual pick-by-pick record — a manager who missed several top wishlist targets had no way to see whether that was because someone else picked first that round, or because the player was gone before their turn came around. User feedback verbatim: "what was the list (by order) of each manager; what was allocated to each manager in each round; what was the picking order in each round... so that I understand why, for example, none of my top 5 picks were assigned to me."

Fix: `_shared/snakeDraft.ts`'s `runSnakeDraft` now returns a `pickLog` — one entry per successful pick (`round`, `order_index`, `user_id`, `player_id`, `wishlist_rank`) — threaded through `run-draft-lottery/index.js` into `gazette_entries.full_data.pick_log`. `GazetteDraftReport.jsx` renders a new collapsible `DraftPickLogTable`, grouped by round with each pick showing the manager, player, and how far down that manager's own wishlist the pick came from (`wishlist_rank`) — the direct answer to "why didn't I get my top picks." The old `DraftOrderBoard` illustration remains as a fallback for historical entries that predate this log.

Backfilled the existing "Draft Champions 26/27" gazette entry (id `c7a83e40-a079-4d24-bfca-3717acfcb207`) by deterministically replaying the snake-draft algorithm against the stored `draft_submissions` — verified the replay reproduces the exact stored final rosters letter for letter before writing, confirming the reconstructed 110-entry pick log is a faithful account, not a guess.

Editing the shared `_shared/snakeDraft.ts` module changed its combined hash and flagged all 25 functions that import from `_shared/` as drift in CI's function-drift check (by design — the script can't tell which functions actually use the changed file, only that `_shared` changed). Redeployed all 25 and committed updated `.function-checksums.json`.

Verified: `npm run lint` (0 errors, pre-existing warnings only), `npm run build` (clean), full CI green (Security, Lint, Build, Unit Tests, Schema Rehearsal, iOS/Android builds, Tier 3 E2E, E2E Tests all passed) before merge.

## ✅ Wishlist Draft quick-add button + explicit Save Wishlist button (2026-09-08) — PR #981

Two UX gaps on the Wishlist Draft screen, per user feedback verbatim: (1) "i need to 'open' the stats details to add the player to the wishlist... can you but that button - add to list - immediately in the player line? Maybe change to a button with a + symbol"; (2) "can you add a button with 'Save wishlist'... it's not obvious if it auto-saves or not."

Fix: added a compact "+" quick-add button directly on each target-pool player row (reuses the existing `addTarget` handler with `e.stopPropagation()` so it doesn't also open the stats detail sheet) — avoids the "huge list of add-to-cart buttons" the user flagged as the risk of a full-text button per row. Relabeled the existing save action to "Save Wishlist" so it reads as an explicit, necessary action rather than implying autosave.

Not fully interactively verified in-browser: the demo account has no joined league with an active Wishlist Draft to navigate to, so verification was limited to a clean dev-server compile/HMR reload with no console errors — flagged as an open limitation rather than claimed as tested.

## ✅ UCL price sync — new arrivals priced, departed players deactivated (2026-09-08) — no PR (data-only)

Processed the user-supplied UEFA Champions League price-list extraction (`SyncPricesUpdated2.xlsx`): (1) filled `price` only for tournament 1593 players that had none set — 0 rows needed it, already handled in a prior session; (2) inserted 60 new UCL arrivals not previously in the `players` table, priced from the user's suggested prices; (3) soft-deleted (`is_active = false`) 59 of 60 players present in the DB but absent from the latest Forza extraction (transferred out) — the 60th, Matias Fernandez-Pardo (`fp-1214193263-1593`), was excluded from deactivation because he's rostered on a live squad, per the user's explicit instruction to leave squad-held players as-is until a manager subs them out.

Followed the pilot SELECT-before-write safeguard throughout: checked all 60 new-arrival IDs for collisions (none), checked all 60 deactivation-target IDs' current `is_active` state, and cross-referenced the deactivation list against every squad's `players` array — which is what caught the Fernandez-Pardo case, since the source list only carried two known exclusions (Rodrigo Becão, Jordan Lotomba) from a prior session and missed this one. Verified post-write: 60/60 new arrivals present, 59/60 target players deactivated, Fernandez-Pardo confirmed still `is_active = true`.

## ✅ Draft Order Board — snake-draft pick-order infographic (2026-09-08) — PR #979

Follow-up to the manager-grouped contested-picks fix below: managers reading the Market Report draft recap could see the outcome of the draft (who won what) but had no visual for the mechanism itself — that round 1's order was set by a random lottery and every round after reverses direction, so no one manager is structurally favored.

Fix: added `DraftOrderBoard` to `GazetteDraftReport.jsx`'s `SeasonDraftReport`, rendered from `fullData.snake_order` (already stored on the gazette entry, no new data needed). Shows round 1 and round 2 as two rows of numbered pick chips, one per manager, with round 2's pick numbers and arrow direction mirrored to visually demonstrate the reversal — plus a legend mapping pick-order position to manager name. Deliberately scoped to showing the *rotation pattern*, not a full pick-by-pick history — no such log exists server-side to render from.

Caught via visual verification (built a static HTML mock from real production `snake_order` data and screenshotted it locally): an initial version combined a JS array `.reverse()` with a CSS `flex-row-reverse` on round 2, which canceled out and left round 2 rendering ascending (9→16) instead of mirrored (16→9), and also broke manager column-alignment across rounds. Fixed by keeping both rounds iterating the same unreversed array and only flipping the displayed pick-number formula and arrow glyph per round — managers now stay in the same visual column between round 1 and round 2, with pick numbers correctly mirrored.

Verified: `npm run lint` (0 errors), `npm run build` (clean), and a local static-HTML visual check against real league data (8-manager league, picks 1-8 then 16-9).

## ✅ Market Report contested-pick bullets grouped by manager (2026-09-08) — PR #978

The Market Report draft recap's "contested picks" bullets were sorted purely by wanted-by count, which scattered a single manager's several wins throughout the list instead of showing them together — made it hard to scan "who won the most contested battles."

Fix: contested-pick bullets are now grouped by winning manager (alphabetical), with the existing wanted-by-count ranking preserved as the sort order within each manager's group.
...
Fix: target-pool rows now reuse `PositionChip`, `StatusDot`, and the same `PlayerStatsPanel` component Market uses for its expand-in-place last-5-GW stats table + season totals + "STATS ↗" button — passed `isLocked` to suppress `PlayerStatsPanel`'s own buy/sell action, since Wishlist's action is "Add to List," not buy/sell. `useWishlistDraft` now attaches fitness intel to the player pool via the same `players` + `player_status` join Market uses, so the availability dot has data to render. Deliberately scoped out: fixture-timing display (needs nationality + active-fixtures data not yet available to the wishlist hook) and the four other dense drag/drop/checklist rows elsewhere on the screen that also use inline position badges (`SortableRow`, squad-release list, finalized-submission summary, drag overlay) — those aren't the "browsable player list" being compared to Market.

Verified: `npm run lint` (0 errors) and `npm run build` (clean, no Rolldown TDZ error — screen is a standalone lazy route, not nested under `LeagueScreen`, so no cross-depth-import risk). Live-account visual verification not done this session (requires the real pilot login, not the local demo session).
...
- **Commissioner draft submit-status false positive**: `CommissionerPanel.jsx`'s LifecycleOps built its
  "who has submitted" Set from *any* row in `draft_submissions` for the league, but `DraftScreen.jsx`'s
  3-second debounced auto-save and 30-second heartbeat both `upsert` rows with `submitted_at: null` long
  before a manager clicks the explicit Submit button — only that click sets `submitted_at`. Result: a
  manager who had merely started (and auto-saved) their list, without ever submitting, showed as
  ✓ SUBMITTED to the commissioner. Confirmed live against "Draft Champions 26/27" (one of the league's
  rows had `submitted_at: null` and was being displayed as submitted). Fixed by filtering the Set-building
  logic to only include rows where `submitted_at` is truthy. The parallel Wishlist Draft tracker (added in
  PR #964, table `wishlist_draft_submissions`) does not share this bug — that table's `submitted_at` is
  `NOT NULL DEFAULT NOW()` by design, since wishlist rounds stay always-editable and every save legitimately
  counts as a submission.
- **Transfer Basket unreadable color scheme**: the Market screen's bottom "Transfer Basket" bar (shown when
  players are queued for transfer) predates the app's "Kit Light" light-theme redesign. It still used
  `--paper`/`--mute`/`--rule`/`--positive`/`--danger`/`--cyan` — tokens the redesign repurposed to mean
  dark-text-on-light — against the bar's own hardcoded near-black background (`rgba(13,17,23,0.98)`),
  producing effectively invisible dark-on-dark text (player names, prices, tags all unreadable). Fixed by
  retokenizing the entire bar to the `--on-shell*`/`--shell-rule*` family (`--shell`, `--on-shell`,
  `--on-shell-dim`, `--on-shell-mid`, `--on-shell-accent`, `--on-shell-danger`, `--on-shell-gold`,
  `--shell-rule`, `--shell-rule-strong`) — the same WCAG-AA-tuned dark-panel token set this same screen's
  own sticky header already uses correctly for the identical "dark panel over light app" scenario. The
  positive net-change color (no `--on-shell` positive/green token exists) uses the same raw
  `rgba(24,201,107,0.95)` literal already established in `FormStrip.jsx` for bright-green-on-dark-shell text.

**Verification**: `npm run lint` (0 errors, 706 pre-existing warnings unrelated to these files) and
`npm run build` (clean, no Rolldown TDZ errors) both pass. Live visual confirmation of the basket-bar fix
was not captured — the demo account in the dev preview has no clubhouse/squad data to populate a transfer
basket — but the change is a direct token substitution mirroring an already-verified in-file pattern, and
the new token values were confirmed in `src/index.css` to resolve to white/near-white against the dark
`--shell` background.

## ✅ Wishlist Draft redesign — commissioner visibility, ownership filter, Market-style UI (2026-09-07) — PR #964

**Nine-item overhaul** of the recurring per-round Wishlist Draft feature (draft-mode leagues only) — the pick list managers use each round to rank incoming targets and squad players they'd release, resolved automatically via a rotating snake order before the market opens.

- **Commissioner visibility fix**: `CommissionerPanel.jsx`'s LifecycleOps now fetches `get_wishlist_draft_status` + `wishlist_draft_submissions` for the currently open round and renders a submitted/not-submitted tracker (reusing the existing `DraftSubmissionTracker`, same pattern as the one-time season draft above it) — previously the commissioner had no way to see who still needed to submit.
- Added a persistent MARKET-adjacent nav button in `CompetitionNav.jsx`/`NavIcons.jsx`, shown only for draft-mode (`noduplicate`) leagues.
- `useWishlistDraft.js`'s player pool is now filtered to unowned players only (excludes anyone already on a roster anywhere in the league) — previously showed the full player database.
- Restyled the player list in `WishlistDraftScreen.jsx` to match `MarketScreen`: club crest, price, an expandable row revealing priority rank, season-stats line, a "Stats ↗" link, and "Add to List".
- Added a "Save" button alongside "Add to List", a 3s debounced auto-save on target/drop-list changes, and a 30s dirty-state heartbeat save — mirrors the season draft screen's existing auto-save pattern.
- The list stays always-editable: "Submit" no longer locks it server-side: managers can keep adjusting targets/releases until the round actually resolves.
- Added `WishlistInfoModal.jsx`, a "?" info sheet covering what the wishlist draft is, why it exists (draft leagues have no free-buy market), how it resolves (rotating snake order), when it runs (no fixed deadline, resolves automatically pre-market), and how editing works.
- Restyled the wishlist's top header bar to match the rest of the app.
- **Verification**: live-tested against the real `football_draft_wishlist` test league (real Supabase data) — temporarily added the dev demo account to `league_members` for that league, confirmed ownership filtering against hundreds of real unowned players, row expand/collapse, target add/reorder/remove, and the info modal's content all render and function correctly, then deleted the temporary membership row afterward. The Save button's RPC call failed in this demo-mode session — traced to `submit_wishlist_draft` (migration 252) requiring `auth.uid()` with `EXECUTE` revoked from `anon`/granted only to `authenticated`; the dev demo account has no real Supabase Auth session (see `AuthContext.jsx`), so this is an expected demo-mode limitation mirroring the existing `knockout_keep_submissions` RPC (migration 143), not a regression — a real authenticated user's save/submit/auto-save was not itself broken by this change.
- `npm run lint` (0 errors), `npm run build` (clean, no Rolldown TDZ), `npx playwright test platform.spec.js` (83/84 passed, 1 unrelated flaky timeout that passed on retry) all green before merge.

## ✅ Fix desktop squad-list logo blink + white Live pitch league name (2026-09-06) — PR #955

**User report**: "the club logos in the desktop in list view seem to be blinking" + "on the live view can you also change the league name (to the right) to white font."

- Root cause of the blink: `SquadScreen.jsx`'s desktop LIST tab renders a `PlayerList` helper defined as `const PlayerList = () => {...}` **inside** the component body, then invoked as `<PlayerList />` (JSX component syntax). Redefining it every render gives it a new identity each render, so React treats it as a new component type and unmounts/remounts the whole list subtree — including every `ClubCrest` inside `PlayerRow` — which resets each crest's local image-load state back to its ghost-ring placeholder before flipping back to the cached image. This screen also runs a `setInterval(tick, 1000)` countdown that ticks state once a second, so the list (and its logos) visibly blinked roughly once per second.
- Fix: call `PlayerList()` as a plain function (`{PlayerList()}`) instead of rendering it as JSX (`<PlayerList />`), so its output is inlined into the parent's own element tree and diffed normally by key instead of torn down and rebuilt every render.
- `LiveScreen.jsx`'s `MiniPitch` header league name now uses `--on-shell` (white) instead of the league's own accent tone.
- Verification note: could not exercise live in the browser — the local demo account has no real league/squad data to load these screens with, and creating some would mean writing rows into the live pilot DB. Verified via careful code-path tracing (`PlayerCard variant="row"` → `PlayerRow` → `ClubCrest`, plus the 1s interval as the render trigger) and clean `lint`/`build`.

## ✅ Squad formation self-heal + club logos on Squad/Live pitches (2026-09-06) — PR #953

**User report with screenshot**: after auto-fill, "Champions Eder 26/27" squad `79e12342...` had a starting XI of 1 GK / 5 DEF / 5 MID / 0 FWD (all showing 0 PTS), with bench holding 3 FWD + 1 GK. Also requested: club logos on the Squad pitch view (desktop+mobile) and the mobile Squad LIST tab (desktop LIST already had them), and a retheme of the desktop Live pitch to match the Squad screen's styling (was flat dark navy) plus club logos there too.

- Root cause: `SquadScreen.jsx`'s `fetchSquad` self-heal only guaranteed "11 players, exactly 1 GK" — nothing enforced `useLeagueConfig`'s per-position `minFormation` (`{GK:1, DEF:1, MID:1, FWD:1}`), so an XI could satisfy both existing checks while still having zero players at an outfield position.
- Fix: `fetchSquad` now also enforces per-position minimums, swapping a bench player of the missing position in for a pitch player from whichever outfield position currently has the largest surplus above its own minimum, and persists the corrected `starting_xi`. Verified against the real broken row via a read-only `SELECT` (DEF=5, MID=5, FWD=0) — the fix pulls in a bench FWD and benches a DEF, landing on a valid 4 DEF/1 FWD/1 GK/5 MID XI.
- Added `ClubCrest` club logos to the Squad pitch view (desktop + mobile, in [PitchView.jsx](src/components/PitchView.jsx) and [SquadScreen.jsx](src/screens/SquadScreen.jsx)) and the mobile Squad LIST tab.
- Retheme `LiveScreen.jsx`'s desktop `MiniPitch` from flat navy to the same green gradient/guide-lines/centre-circle used by `PitchView`, fixed `POS_TONE` to import the canonical palette from `lib/formations` (was a local duplicate with the wrong GK color), and added club logos to the mini player tokens.
- Verification note: could not exercise the fix live in a browser — the production-preview build has no real demo league/squad data, and creating some would have written real rows into the live pilot DB. Verified instead via a direct read-only query against the actual broken squad row, plus clean `lint`/`build`.

## ✅ Clubhouse invite-link "already a member" error banner (2026-09-06) — PR #949

**User report**: tapping a WhatsApp Clubhouse invite link for the "Mundial do Eder" clubhouse — already joined — showed a raw red `ALREADY_MEMBER` error banner instead of just landing the member in the clubhouse.

- Root cause: `join_circle_by_code`'s `ALREADY_MEMBER` response (migration 286, see entry below) never includes a `circle_id`, so `joinCircleByCode` in [useClubhouse.js](src/hooks/useClubhouse.js) had nothing to navigate to and just re-threw the raw error — which `ClubhouseScreen.jsx`'s auto-join-via-link effect (and its manual code-entry form) surfaced verbatim as an error banner.
- Fix: client-side only, no migration. `joinCircleByCode` now catches the `ALREADY_MEMBER` case and resolves the circle itself from the caller's own `circle_members` rows (matched by `invite_code`, same RLS-permitted query shape as `fetchMyCircles`), returning that `circle_id` instead of throwing. A member re-clicking or re-entering an invite code they've already used is now silently navigated straight into the clubhouse — nothing shown, matching the requested behavior. New members joining for the first time are unaffected (still go through the normal insert path).
- Verified: repo-wide search confirmed `join_circle_by_code`'s `ALREADY_MEMBER` branch has only this one caller.
...
## ✅ Scoring v3 Phase 3 — time-on-pitch clean sheet & goals-conceded (2026-09-06) — PR #957, checksums PR #958

Final phase of the Sept 2026 scoring revision (see Phase 1 entry below for the approved plan/scope). Clean sheet and goals-conceded no longer key off the final match score — PATH A (`ingest-match-events`) now builds each player's actual on-pitch interval `[start, end]` from lineup/substitution data and only counts goals conceded while that player was on the pitch, with the doc's explicit exception that a red card does NOT shorten the player's exposure window (their interval still runs to full time, since the team stays a player down for the rest of the match). A self-consistency safety net (`useTimeOnPitch`) cross-checks the new per-player tally against the authoritative final-score conceded count for both teams before trusting it, falling back to the prior final-score-based behavior whenever lineup/period data is missing or the counts disagree — guards against own-goal mis-attribution or incomplete event data. PATH B (`calculate-scores` manual/mock pipeline) has no lineup/roster source at all, so it intentionally keeps today's final-score-based behavior rather than guessing, per the plan's own "skip gracefully when the data isn't there" rule. `ScoringInfoModal.jsx` updated with a caption explaining the mechanic. No migration was needed — this is a pure logic change, `scoring_rules` point values are untouched. Verified via a standalone hand-trace script covering substitution, red-card, and unused-bench-player scenarios before deploy. This completes all three phases of Scoring Revision v3 (Phases 1–3 all shipped) — chips, MVP bonus, and backfill of already-played fixtures remain explicitly out of scope. Open follow-up: a manual spot-check of a real live fixture's `goals_conceded`/`clean_sheet` values once a match with a substitution or red card occurs post-deploy.

## ✅ Scoring v3 Phase 2 — penalty goals as their own event, no double-count (2026-09-06) — PR #948, migration 287, checksums PR #950

Phase 2 of the Sept 2026 scoring revision (see Phase 1 entry below for the approved plan/scope; Phase 3 — time-on-pitch clean sheet/conceded mechanic — is next). Chips, MVP bonus, and backfill of already-played fixtures remain out of scope.

- Fixed a confirmed double-count bug in [ingest-match-events/index.js](supabase/functions/ingest-match-events/index.js): a penalty conversion previously incremented both the general `goalsMap` tally and `penaltyScoredMap`. Penalty-detail goals now populate only `penaltyScoredMap`.
- Checked PATH B (`calculate-scores/index.js`'s manual/mock event loop) and confirmed it has no penalty-goal tracking at all — no double-count bug present there, so no logic fix was needed on that path.
- `penalty_scored` scores a flat +3, position-agnostic (already correct in [scoring-logic.js](supabase/functions/calculate-scores/scoring-logic.js); verified via a standalone Node script against fabricated stats — a penalty-only goal scores exactly the flat value, never also the position's per-goal rate).
- Migration `287_scoring_v3_penalty_goals.sql` applied live to `scoring_rules` for tournaments 429 (World Cup) and 1593 (Champions League) via surgical `jsonb_set` — verified before/after (0 → 3 on all 8 GK/DEF/MID/FWD rows), backed up first per Pilot Safeguards.
- `FALLBACK_POINTS.penalty_scored` in [calculate-scores/index.js](supabase/functions/calculate-scores/index.js) set to 3 for all four positions (was 0), so a tournament with no `scoring_rules` row doesn't inherit the old ruleset.
- [ScoringInfoModal.jsx](src/components/ScoringInfoModal.jsx) updated: new "Penalty Scored +3 (flat, not the goal value)" row and an explanatory caption.
- Both `ingest-match-events` and `calculate-scores` Edge Functions deployed live; `.function-checksums.json` synced in follow-up PR #950.
- Lint/build/`platform.spec.js` (84/84) all green before merge.
...
- New UNIVERSAL terms `appearance` (+1, any minutes played) and `minutes_60_bonus` (+1, at 60+ minutes) replace the old per-90-minute rate, in [scoring-logic.js](supabase/functions/calculate-scores/scoring-logic.js).
- Assist bumped to a flat +3 across DEF/MID/FWD (GK was already +3).
- Card Accumulation Rule: a red card always supersedes any yellow(s) in the same match — capped at −3 total, never stacked (Forza's match-event data can't distinguish a two-yellow send-off from a separate straight red).
- `penalty_missed` aligned to −2 everywhere.
- Migration `285_scoring_v3_foundational.sql` applied live to `scoring_rules` for tournaments 429 (World Cup) and 1593 (Champions League) via surgical `jsonb_set`, backed up first per Pilot Safeguards.
- `FALLBACK_POINTS`/`FALLBACK_UNIVERSAL` in [calculate-scores/index.js](supabase/functions/calculate-scores/index.js) updated to match, so a tournament with no `scoring_rules` row doesn't inherit the old ruleset.
- [ScoringInfoModal.jsx](src/components/ScoringInfoModal.jsx) updated: new Appearance/60+ Min Bonus rows, +3 assist rows, card-accumulation caption, v3 tab label.
- Verified via a standalone Node script exercising `scorePlayer`/`buildBreakdown` directly (11/11 assertions), plus lint/build/`platform.spec.js` all green.
- **Pending**: `calculate-scores` Edge Function has not been deployed yet — Vercel doesn't auto-deploy functions, and this needs its own explicit go-ahead separate from the migration confirmation already given.

## 🟡 P3 — Fallback conceded-penalty is silently always 0 due to a field-name mismatch — open, no PR yet (2026-09-06, found during Scoring v3 Phase 1)

`FALLBACK_POINTS` in [calculate-scores/index.js](supabase/functions/calculate-scores/index.js) names the goals-conceded-penalty field `conceded_per_goal`, but [scoring-logic.js](supabase/functions/calculate-scores/scoring-logic.js)'s `scorePlayer`/`buildBreakdown` reads `rules.conceded_2plus_penalty` — so any tournament falling back to `FALLBACK_POINTS` (no `scoring_rules` DB row, e.g. a future EPL/426 tournament) silently never applies the conceded-goals penalty for GK/DEF, with no error. Pre-existing bug, not introduced by Phase 1 — found while auditing the fallback constants for that PR, deliberately left out of its scope. Fix: rename `conceded_per_goal` → `conceded_2plus_penalty` in `FALLBACK_POINTS` (both GK and DEF rows currently have it at value `0` anyway, so no scoring change today — but the field name needs to match before that value is ever made non-zero, or before a tournament without a DB row goes live).
...
**Fix**: `_create_user_wallet()` now creates the `coin_wallets` row itself (idempotent `ON CONFLICT (user_id) DO NOTHING`) before crediting the welcome bonus, instead of relying on `credit_coins()`'s auto-create behavior. `credit_coins()` itself is untouched — scoped narrowly to the signup path in case other call sites (e.g. P2P bets) now rely on its stricter behavior.

**Verified live** (per Pilot Safeguards): dumped a full backup (`backups/pre_migration_20260906_151334.sql`) before applying; applied migration 284 to the live DB; created a real `TEST_`-prefixed signup through the actual Create Account UI (`TEST_signup_bugfix`) — succeeded, landed in onboarding, no error; confirmed via SQL that `auth.users`/`public.users`/`coin_wallets` all created correctly with the 500-coin welcome bonus; deleted the test account and its rows afterward.

**Side finding (not fixed, separate issue)**: while cleaning up the test account, discovered `guard_coin_columns()` (the trigger guarding direct writes to `coin_wallets`) has a pre-existing bug — for a `BEFORE DELETE` trigger it does `RETURN NEW`, but `NEW` is always `NULL` in a DELETE context, so it silently cancels **every** delete on `coin_wallets` regardless of role, leaving orphaned wallet rows behind whenever a user is deleted. Worked around it for this one cleanup (disabled/re-enabled the trigger around the single DELETE); the trigger itself still needs a proper fix — tracked as a new open item below.

---

## ✅ Light-theme the Squad pitch player-detail sheet (2026-09-06) — PR #937

**Context**: User reported that selecting a player on the Squad screen's Pitch tab opens a bottom sheet still styled in the old dark navy color scheme, inconsistent with the rest of the app's "Kit Light" theme (screenshot showed a MID/France player card in dark colors — form squares, "VIEW FULL STATS DASHBOARD", action buttons all on a dark surface).

**What was built**: Retheme of `SquadScreen.jsx`'s player-action bottom sheet and swap-mode banner from the dark `--shell`/`--on-shell-*` token family to the light `--card`/`--rule`/`--elev`/`--mute`/`--gold-text` tokens already used by the app's canonical light sheet (`src/components/shared/BottomSheet.jsx`). Changed: sheet background/border/shadow, drag handle, position badge, club/captain-pill row text and CAPTAIN pill colors, close button, FORM/NEXT FIXTURE divider borders, and the swap-mode banner background/label/cancel button. Confirmed via repo-wide grep that the dark `--shell` sticky header used on every screen (Squad, Market, Live, etc.) is a deliberate, unrelated app-wide chrome convention — out of scope and left untouched, along with `--cyan`/`--gold`/`--danger` action-button accents which were already correct.

**Verification**: `npm run lint` (0 errors, pre-existing warning count unchanged) and `npm run build` (clean, no Rolldown TDZ regression) confirmed locally. A live browser check in demo mode wasn't possible — the demo account has no clubhouse/league/squad membership, and faking its id client-side doesn't work around Supabase RLS (`auth.uid()` comes from the real Auth JWT, which demo mode never creates) — so this shipped on code review + the established `BottomSheet.jsx` token convention rather than a visual screenshot; flagged to the user for a live check.

---

## ✅ Club crests + country flags on player rows (2026-09-06) — PR #935, migration 283

**Context**: Player rows (Squad screen, Market screen) showed club/country only as plain text, no visual badge. User approved self-hosting UCL club crests (36 clubs) and hotlinking World Cup national flags (48 countries, via flagcdn.com) into a new `club_logos` name→URL lookup table, choosing flags over federation crests for WC and scoping to both tournaments.

**What was built**: Migration 283 creates `public.club_logos` (name, type, logo_url) plus a public `club-logos` Storage bucket with RLS read policy. 36 club crest SVGs downloaded and uploaded to that bucket; all 84 rows (36 clubs + 48 countries) inserted after diffing the generated name list byte-for-byte against a live `SELECT DISTINCT club FROM players` query across both tournaments (zero discrepancies, including special characters — Atlético, Fenerbahçe, Bodø/Glimt, Türkiye, Côte d'Ivoire, Curaçao). New `useClubLogos` hook fetches the table once per session (shared cache). New `ClubCrest` component renders a 20px circular badge, falling back to a dashed ghost ring — never a broken image — when a name isn't mapped or the image fails to load. Wired into [PlayerRow.jsx](src/components/PlayerRow.jsx) (Squad + any other row-based list) and [MarketScreen.jsx](src/screens/MarketScreen.jsx)'s own inline row markup, between the position chip and player name. Not yet wired into `PlayerCard.jsx`'s compact "pitch" nameplate variant or `MatchCard.jsx`'s existing `TeamCrest` placeholder — both explicitly out of scope for this pass.

**CI fix included**: the PR's own "Schema Rehearsal" check failed on the new migration — `storage.buckets`/`storage.objects` don't exist on the plain `postgres:15-alpine` container that job rehearses against (a managed-Supabase-only schema, same class of gap already tolerated for `cron`/`net`/`http`/`vault`/`realtime`, first hit here by a migration touching Storage). Extended the job's `TOLERABLE` error regex in `.github/workflows/ci.yml` to include `storage`.

**Verification**: `npm run lint` 0 errors, `npm run build` clean (no Rolldown TDZ regression), full PR CI green (Lint/Build/Security/Schema Rehearsal/Unit/E2E/Tier 3/iOS/Android). Visual verification in a running browser (real crests/flags rendering, not the ghost fallback) was **not completed this session** — blocked by a Browser-pane permission denial (`navigate`/reload/key-press all refused by the run-mode classifier) with no real league visible under the local demo-mode account. User will check the live badges post-merge and report back.
...
**What was built**: `TransferWindowBanner` gained `isDraftLeague` and `noPenaltyReason` props — `MarketScreen` passes the specific reason (pre-competition, squad still building, or a commissioner's free-transfers toggle) so the banner surfaces the real reason instead of the generic penalty copy; `SquadScreen` passes `isDraftLeague` only (does not yet have the other exemption context locally, left as a known follow-up). In `WishlistDraftScreen`, moved the `dnd-kit` drag listeners and `touch-action: none` off the row and onto a dedicated ⚿ grip handle, and switched `TouchSensor` from a 250ms hold-delay to distance-based activation (matching `PointerSensor`) since scoping the listener to the handle removes the scroll-vs-drag ambiguity that the hold-delay existed to resolve. Also enlarged the ▲/▼ reorder button tap targets.

**Verification**: Lint, build, and both mobile CI builds passed. The PR's "Security" CI check failed twice in a row with an identical `npm warn audit 503 Service Unavailable — POST https://registry.npmjs.org/-/npm/v1/security/advisories/bulk` — confirmed as an npm registry-side outage (not a new vulnerability tied to this diff, which touches only `.jsx` files with no dependency changes) and merged past it. Full interactive drag/scroll verification on a real draft-league wishlist was not performed in-session — doing so would have required creating real Clubhouse/league data in the production database outside the existing `TEST_`-prefixed fixtures, which was avoided per the pilot data-safety rule; the fix itself follows the standard, well-understood pattern for this class of bug (scoping `touch-action: none` to the drag handle instead of the whole scrollable row).

---

## ✅ Tennis stale-tournament fix + daily safety-net sweep (2026-09-04) — PR #923, migration 280

**Context**: User noticed Wimbledon still showing as "available" in the app long after the real tournament ended. Investigation found two stacked bugs, not one.

**Bug 1 — Wimbledon was never started**: `tennis_tournaments.status` was still `roster_open` — no admin had ever called `admin_start_tournament`, so the entire sync/scoring pipeline (which only looks at `in_progress`/`qf_captain_open` tournaments) had never had a reason to touch it.

**Bug 2 — `sync-tennis-results` was silently broken for every active tournament**: the results endpoint (`/atp/tournament/results/{id}`) returns `{ data: { singles: [...] } }`, not a bare array under `data` like the fixtures/draw endpoint `sync-tennis-players` uses. Every cron run (3×/day since migration 272) was throwing `rawMatches.filter is not a function`, caught, and returned as a 200 with the error swallowed into the JSON body — meaning US Open's live results sync had likely been silently dead the whole time too, not just Wimbledon's.

**What was built**: (1) Fixed the response-shape parsing in `sync-tennis-results/index.ts` with explicit `Array.isArray()` guards per fallback level. (2) Added auto-trigger: once a champion is detected, `sync-tennis-results` now calls `score-tennis-tournament` itself instead of waiting for an admin to click "Trigger Scoring". (3) Migration 280: new daily cron `tennis-stale-tournament-sweep` (06:00 UTC) — re-invokes scoring for any tournament stuck `in_progress`/`qf_captain_open` more than 3 days past its `end_date`, and force-completes anything that never left `roster_open`/`upcoming` by the same threshold. This is the actual fix for "stuck open indefinitely" — Bug 1 can still happen (an admin forgetting to open a tournament), but now it self-heals within a day of the real event ending instead of sitting open forever.

**Executed live, 2026-09-04** (approved per-item, per CLAUDE.md): found Wimbledon had 1 real submitted roster (not empty, as first assumed) — user chose "score it properly" over force-completing with no score. Ran `admin_start_tournament`, triggered the fixed `sync-tennis-results` (pulled the real Wimbledon final: Sinner def. Zverev), which auto-triggered scoring. Verified end-to-end: `tennis_tournament_players` shows Sinner as `round_reached = 'champion'` (7 rounds won), the 1 roster (a `test_qa_manager`-labelled test account, not a real pilot user) scored 49 points, a `tennis_result` gazette entry was written, and `tennis_tournaments.status = 'completed'`. US Open correctly remains `in_progress` (real end date 2026-09-13). Migration 280 applied directly and confirmed active in `cron.job`.

---

## ✅ F1 race-result sync automated via cron — replaces manual OpenF1 fetch (2026-09-01) — PR #903, migrations 276–277

**Context**: User flagged that neither Tennis nor F1 should require an admin to manually click a "fetch from provider" button — results should sync via cron the same way the football tournaments already do. Tennis was already cron-driven (`sync-tennis-results`, migration 272). Auditing `F1AdminScreen.jsx` found the opposite: a "⚡ FETCH FROM OPENF1" button calling client-side `src/lib/f1/openf1.js`, one click per race weekend — the "one-off at season start" assumption in the original ask was wrong; it was actually a recurring per-race manual step.

**What was built**: `supabase/functions/sync-f1-race-results/index.ts` — new cron-only Edge Function (`requireServiceRole` guard, same shape as `sync-tennis-results`) that polls `f1_races` for any row where `race_at` is 2+ hours past and `result_p1 IS NULL` (bounded to a 14-day lookback), queries OpenF1 (`api.openf1.org` — free, unauthenticated, no daily-budget throttle unlike tennis's paid RapidAPI provider) for that round's `Race` session, and fills `result_p1`/`result_p2`/`result_p3` + flips `status` to `'finished'`. **Scope is deliberately narrow**: DNF drivers, team-most-points, and the special-category answer have no OpenF1 equivalent and stay fully manual on `F1AdminScreen`, as does the final SCORE RACE click (`is_scored` is never touched by this function) — an admin still opens the race, fills those 3 fields, and scores it once P1–P3 are already populated. `migrations/276_schedule_sync_f1_race_results.sql` schedules it every 30 minutes via the standard `net.http_post` + `<ADMIN_TRIGGER_KEY>`-placeholder pattern (migrations 272–274).

**Cleanup**: removed the manual button, its handler, and its loading state from `F1AdminScreen.jsx` (replaced with a one-line hint about the new auto-fill behavior); deleted the now-dead `src/lib/f1/openf1.js` entirely (confirmed via grep it had no other importers) — its fetch logic was reimplemented server-side in the new function rather than reused as-is.

**Docs**: new cookbook [ADDING_A_NEW_F1_SEASON.md](docs/deployment/ADDING_A_NEW_F1_SEASON.md), mirroring `ADDING_A_NEW_TENNIS_TOURNAMENT.md`'s structure — season seeding is a one-off direct `INSERT` (no admin UI, no external-ID lookup needed), results now auto-fill, and Step 2b documents the manual P1/P2/P3 fallback (never clobbered by the cron, which only ever fills a `NULL` result). Added a matching card to `DOCS_INDEX.html`. With this PR, all four sports the user asked about (EPL/WC football, Tennis, F1) have both cron automation and a cookbook — football's is `ADDING_A_NEW_TOURNAMENT.md`, already existing.

**Also investigated this session (no action taken)**: user asked to look into unfreezing the 12 dormant football-adjacent pg_cron jobs (`DD-P0-2`) while working nearby. Confirmed both of `DD-P0-2`'s gating prerequisites still stand unmet as of 2026-09-01 — no `status='active'` tournament has a future fixture (`DD-P0-3`; see the row above on stale-tournament cleanup, which removed 2 of the 4 but did not add new forward-looking fixtures to the remaining `1593`/`429`), and no `SENTRY_DSN`-equivalent backend secret is configured (`OPS-SENTRY`). Reactivation was deliberately not attempted — see the `DD-P0-2` row in the Open Items section below for the updated note.

**Unplanned mid-ship blocker → migration 277**: applying migration 276 required embedding the live `ADMIN_TRIGGER_KEY` value in place of its `<ADMIN_TRIGGER_KEY>` placeholder. That value (rotated the same day by migrations 272–275) was not recoverable in-session — `npx supabase secrets list` only ever returns name + hash digest, never plaintext. Asked the user to choose between pasting the old value or generating a fresh one; they chose the latter. Generated a new 32-byte hex key via `openssl rand -hex 32`, set it live as the `ADMIN_TRIGGER_KEY` secret, and wrote `supabase/migrations/277_rotate_admin_trigger_key.sql` to repoint every dependent cron/function in one pass: all 13 pre-existing `net.http_post`-invoking `cron.job` rows (12 frozen `active=false` per `DD-P0-2`, plus `sync-tennis-results` which stays `active=true`) via `cron.alter_job()` — chosen over migration 274's unschedule+reschedule pattern specifically because `alter_job` leaves `active` untouched when omitted, avoiding a repeat of the exact bug migration 275 had to fix — plus `cron_sync_active_tournaments()`, a plpgsql function (backing the 3 `sync-active-tournaments-*` jobs) that carries its own hardcoded bearer inside the function body rather than in `cron.job.command`. Three retired WC-hardcoded functions (`trigger_wc2026_sync`, `sync_wc_players`, `load_wc_teams`) were confirmed dead — no `cron.job` row references them — and deliberately left unrotated as out of scope.

**Executed live, 2026-09-01** (both approved per-item, per CLAUDE.md): took a pre-migration backup (`backups/pre_migration_20260901_232446.sql`), applied migration 276 then 277 via `npx supabase db query --linked --file`, verified every job/function now carries the new key and that `active` states are exactly unchanged (12×`false`, `sync-tennis-results` + new `sync-f1-race-results` both `true`), deployed `sync-f1-race-results` (`npx supabase functions deploy`), and ran `npm run update:checksums` to clear the expected function-drift CI failure. Real secret material never touched git — the committed migration files keep the `<ADMIN_TRIGGER_KEY>` placeholder; substitution happened only in gitignored scratchpad files, deleted immediately after the live queries succeeded.

**Old `ADMIN_TRIGGER_KEY` value — closed, no separate revocation step exists or is needed.** Checked `supabase/functions/_shared/auth.ts` Path C: `Deno.env.get('ADMIN_TRIGGER_KEY')` is read fresh on every request and compared with exact string equality against whatever the Supabase secret currently holds — there is no allowlist, grace period, or cache of prior values. The instant the secret was set to the new key (this session, before migration 277 even ran), the pre-2026-09-01 value stopped authenticating anywhere, permanently. Unlike the GitHub PAT below, there is no dashboard, list, or API where the old value persists as a live credential to go delete — it simply isn't accepted anywhere anymore. Nothing further to do here.

**Old classic GitHub PAT (`SEC-4`) — still needs one manual step, and it will stay that way.** This is a real credential change on the user's GitHub account, which falls under "modifying system or security settings" — outside what Claude will perform even with explicit approval (see `CLAUDE.md`'s operating rules). It also cannot be done via `gh` CLI or the GitHub API: classic PATs (`ghp_...`) have no self-service revocation endpoint, only the web UI. **Action needed from the user**: visit https://github.com/settings/tokens, find the old classic token (the one that used to be embedded in this repo's git remote URL, retired 2026-07-31 — see `SEC-4`'s row below for identifying detail), and click Delete. Re-confirmed 2026-09-01 that this is still outstanding — see updated `SEC-4` row.
...
**What was done this session (manual, one-off fix — not a platform fix)**: confirmed the season boundary was clean (old season's last kickoff 2026-05-30, new season's first kickoff 2026-07-07, no overlap), backed up the affected rows to `backups/ucl_1593_*_20260901_121817.json` (Docker was down, so used the CLAUDE.md-documented SELECT-and-save fallback instead of `supabase db dump`), then deleted the 281 old-season fixtures, ~1,418 players tied to the 47 old-season-only clubs (targeted delete, not delete-all, to preserve `price` valuations already set on current-season players), and 7 stale `matchday_deadlines` rows, and updated the `tournaments` row's `name`/`slug`/`starts_at`/`ends_at` to the 2026-27 season. Verified clean afterward: 234 fixtures, rounds 1–8 each with exactly 18 fixtures (no collisions), 2,395 players across 80 clubs, and World Cup (`tournament_id='429'`) confirmed untouched throughout (104 fixtures, never referenced by any query this session).

**The open problem**: this was done by hand, tournament-by-tournament, and nothing prevents the same collision from recurring — for UCL next season, and for any other tournament that reuses its forza_id year over year (continental/knockout competitions are the likely repeat offenders; single-season domestic leagues like a given EPL season may or may not follow the same pattern — not yet confirmed either way). There is no season-aware cleanup step in [docs/deployment/ADDING_A_NEW_TOURNAMENT.md](docs/deployment/ADDING_A_NEW_TOURNAMENT.md)'s onboarding cookbook, and no code path detects "this forza_id's fixtures just changed season out from under us."
...
## ✅ Chat quick-bet + live bet-activity feed — final item of the P2P social/chat plan (2026-09-01) — PR #895

**Context**: PR 4 (last item) of the approved 8-item P2P social/chat plan (`synchronous-stirring-pretzel.md`). Confirmed before building: the live Clubhouse chat is `clubhouse_messages` via `useClubhouseChat.js`/`ClubhouseChat.jsx` (`chat_messages`/`ChatView.jsx`/`useChatMessages.js` are dead code, not built on).

**What shipped**: Migration `271_clubhouse_bet_activity_messages.sql` (applied to production with fresh per-item approval this session, backed up first per Pilot Safeguards) adds `kind text NOT NULL DEFAULT 'text'` and `bet_ref jsonb` to `clubhouse_messages` (CHECK `kind IN ('text','bet_activity')`), plus a `post_bet_activity_message()` helper wrapped in `EXCEPTION WHEN OTHERS THEN NULL` so a chat-post failure never blocks the caller. Wired into `create_p2p_bet`, `create_p2p_challenge`, `arbitrate_bet_outcome`, `finalize_declared_bets`, `auto_void_stale_bet_disputes`, `resolve_p2p_challenge`, `confirm_freeform_result`, and `arbitrate_freeform_result` — posts a feed card on create and resolve only (join is intentionally silent to avoid spam). `useClubhouseChat.js` now selects/forwards `kind`/`bet_ref` in both the initial fetch and the realtime INSERT handler (same subscription, no second channel needed). `ClubhouseChat.jsx` renders `kind==='bet_activity'` messages as a compact `BetActivityCard` (question, stake, VIEW link) instead of a plain bubble, and adds a 🎲 quick-bet button next to the composer.

**Routing decision** (resolves the plan's explicitly-flagged open question): rather than exporting/duplicating the ~1300-line `NewBetModal` tree out of `ChallengeScreen.jsx`, the quick-bet button navigates to `/challenges?tab=bets&new=1`; `ChallengeScreen.jsx`'s `showCreate` state now initializes from that `new` param, auto-opening its existing unified create-bet modal pre-scoped to the circle via context it already owns. Same navigate-with-query-param precedent as `MyBetsScreen.jsx`.

**CI fix found along the way**: the "Schema Rehearsal" job (PR #856) loads `supabase/schema.sql` then replays any migration files new in the PR on top of it — it expects `schema.sql` to represent the *pre-migration* state within a PR. Regenerating `schema.sql` from prod in the same PR after applying migration 271 baked the new columns in already, so the replay failed with "column already exists." Fixed by reverting the regenerated `schema.sql` back to `main`'s version for this PR; it'll pick up 271's columns on its next post-merge refresh.

**Verification gap**: same class as prior P2P PRs — full CI green (12/12 checks), but no live two-account browser test of the real-time feed card (no live multi-account test setup available this session). Recommend a manual two-account pass on the live pilot post-deploy: create a bet via the chat 🎲 button and confirm a feed card appears for a second account in real time, then resolve it and confirm a second card appears.

**Plan closed**: this was the final item of the 8-item P2P social/chat plan. All shipped: PR 1 (#887), PR 1.5 (#891), PR 2 (#889), PR 3 (#893, win-streak badge), PR 4 (#895).

## ✅ Open Challenges — post a 1:1/Football GW challenge without naming an opponent (2026-08-31) — PR #891

**Context**: PR 1.5 of the approved 8-item P2P social/chat plan (`synchronous-stirring-pretzel.md`), the migration item slotted between PR 1 and PR 2. Closes the "join this bet" gap for 1:1/Football GW challenges — `p2p_bets` already let any circle member join a whole-clubhouse bet, but `p2p_challenges.opponent_id` was a required column with a `CHECK (challenger_id <> opponent_id)` constraint assuming exactly one fixed opponent from creation, so there was no way for a third party to claim an open slot.

**What shipped**: Migration `265_open_challenges.sql` (applied to production with fresh per-item approval in the prior session segment, `supabase/schema.sql` synced) makes `opponent_id` nullable, relaxes the self-challenge CHECK to tolerate `NULL`, and adds `claim_p2p_challenge(p_challenge_id)` (circle-member-only; errors `ALREADY_CLAIMED`/`CHALLENGE_NOT_PENDING`/`CHALLENGE_EXPIRED`/`CANNOT_CLAIM_OWN_CHALLENGE`/`NOT_CIRCLE_MEMBER`/`NOT_LEAGUE_MEMBER`) and `get_open_challenges(p_circle_id)` (circle-wide, mirrors `get_clubhouse_bets`). This session shipped the frontend: `CreateChallengeModal` gains a third target-picker row, "🎲 Open — first to accept," mutually exclusive with picking a named opponent; a new `OpenChallengeCard` (dashed gold border, single "Claim" button) renders in the unified Incoming list via a three-way `kind` dispatch (`'challenge' | 'open_challenge' | 'bet'`) added to `useAllBets.js`; `OutgoingCard` labels a still-unclaimed post "🎲 Open — awaiting claim" (no RPC change needed — `get_my_challenges` already returns the challenger's own open rows). `useChallenges.js` gained `openChallenges` state, `fetchOpenChallenges`/`claimChallenge`, and its existing Realtime subscription now refetches both on any `p2p_challenges` change. Once claimed, a challenge behaves exactly like any other 1:1 challenge — existing accept/decline/resolve/auto-resolve RPCs are untouched.

**Verification gap**: same class as PR #887/#889 — full CI green (12/12 checks on PR #891, including a dedicated Schema Rehearsal check re-validating migration 265), but no live interactive claim-flow test (local demo session has no backing circle membership, so `CreateChallengeModal` correctly shows its "No Clubhouse selected" blocked state). Verified via diff review + lint/build + a clean no-crash browser smoke test. Recommend a manual claim-flow pass on the live pilot post-deploy.

**Plan status**: all 8 items now shipped — PR 3 (win-streak badge, #893) and PR 4 (chat quick-bet + live bet feed, #895) closed it out. See PR #895's section above.

## ✅ Bet templates, smart-default targeting, one-tap rematch (2026-08-31) — PR #889

**Context**: PR 2 of the approved 8-item P2P social/chat plan (`synchronous-stirring-pretzel.md`), built directly on the unified "Bets" tab from PR #887. No migration — pure application-layer changes inside `ChallengeScreen.jsx`.

**What shipped**: one-tap bet-template chips in `CreateGroupBetModal`'s composer (question + stake pre-fill, target picker left to the user); a `localStorage`-backed smart default for the target-mode picker, keyed per Clubhouse + user; a "↻ Rematch" button on resolved history cards (both 1:1 challenges and group bets) that reopens the create modal pre-filled — Football GW rematches still force a fresh gameweek pick, and `selected_users` group-bet rematches fetch the original target list on demand from `p2p_bet_targets`.

**Verification gap**: same class as PR #887/#880 — full CI green, but no live interactive click-through (local demo session has no backing circle membership: `get_my_p2p_bets` returned `permission denied`, clubhouse dropdown empty). Verified instead via a full line-by-line diff review against the plan spec. Recommend a manual pass on the live pilot post-deploy, same as prior P2P PRs.

**Plan status**: all 8 items now shipped (see PR #895's section above for the full closure note).

## ✅ Unified Bets Flow — Coin Challenges + Group Bets merged into one tab (2026-08-31) — PR #887

**Context**: PR 1 of the approved 8-item P2P social/chat plan (plan file: `synchronous-stirring-pretzel.md`). The user's own complaint drove this — they'd been using the always-visible 1:1 "New Challenge" flow (single-opponent by design) and asked "can't we have the bets all in the same place... this is confusing," not realizing the separate Group Bets tab already supported multi-select/whole-clubhouse joining. Investigation also answered a second question in the same plan ("what's the effort of a 'join this bet' option") — for Group Bets it's already ≈0 effort, since `join_p2p_bet`'s `whole_clubhouse` path already only checks circle membership; the real gap was that it lived on an easy-to-miss separate tab.

**What shipped**: new `src/hooks/useAllBets.js` (client-side merge of `useChallenges`/`useGroupBets` into kind-tagged status buckets, no new RPC); `ChallengeScreen.jsx` tab bar collapsed `Challenges | Group Bets | Wallet` → `Bets | Wallet`; one "New Bet" entry point (`NewBetModal`/`BetKindPicker`) routing Football GW → `p2p_challenge` (only schema with auto-resolve) and Freeform (any target shape) → `p2p_bet`, with a `betsEnabled=false` fallback to the legacy challenge modal; `CreateGroupBetModal` gained a "One Person" target option; `ClubhouseScreen.jsx` notification routing simplified to one destination.

**Verification gap**: same class as prior P2P sessions — full CI green, manual smoke test in demo mode confirmed the UI and the fallback-routing logic live, but no real two-account join test (no live Supabase session available locally, and Claude does not sign in as a live user). Recommend a manual pass on the live pilot post-deploy.

**Plan status**: all 8 items now shipped (see PR #895's section above for the full closure note).

## ✅ P2P Group Bets UI — Phases 2-4 (2026-08-30) — PR #880

**Context**: migration 262 (P2P Group Bets schema + RPCs — multi-participant coin bets within a Clubhouse, distinct from the existing 1:1 `p2p_challenges`) shipped live in production via PR #879 with no frontend. User approved building the full UI in one PR, ship-only, no new migrations.

**What shipped**: `src/hooks/useGroupBets.js` (fetch + realtime + derived status lists + RPC mutators, modeled on `useChallenges.js`), and a new "Group Bets" tab on `ChallengeScreen.jsx` (`/challenges?tab=bets`, gated on `circles.p2p_betting_enabled`) covering create → join/decline → answer → close → declare → dispute → arbitrate, plus a history section. `ClubhouseScreen.jsx`'s notification bell now deep-links `p2p_bet` dispute alerts to the new tab. All new components live inside `ChallengeScreen.jsx` (no new cross-depth imports — avoids the Vite/Rolldown TDZ bug).

**Verification gap**: full CI green (lint/build/security/unit/E2E/Tier-3/schema-rehearsal/iOS/Android/Vercel), but no live interactive click-through — local dev's demo-mode auth can't hold a real Supabase session, and Claude does not sign in as a live user. `TEST_SB_1` (a `TEST_`-prefixed clubhouse) had `p2p_betting_enabled` turned on for manual QA, with explicit per-item approval. Recommend a manual pass there to confirm wallet balances move correctly at each lifecycle step before wider rollout.

See the "Last Updated" banner above for full technical detail.

## ✅ Maintenance-wall doc correction + Home-screen CompetitionTopBar fix (2026-08-30)

**Context**: user posted a screenshot of the live production Home screen and reported two things: (1) the maintenance wall had already been removed, so the standing "site is walled" documentation was stale, and (2) the "🏠 Clubhouse · TEST_Wimbledon_Box" competition pill bar was incorrectly showing on the cross-clubhouse Home dashboard — it should only appear inside a clubhouse.

**Doc correction**: `MAINTENANCE_MODE` came down 2026-08-30, confirmed by the user's screenshot (live app rendering, not the `503` page) and `vercel env ls production` showing a recent update to the var. Updated `CLAUDE.md` (the SINGLE BRANCH standing-instruction list and the Session Start Checklist's PR-workflow note), `docs/platform_revision/CUTOVER_PLAN.md` (status banner + Phase 4 step 1), and this file's status banner + the Platform Revision section's "Site reopen" blockquote — all previously asserted the site was still walled. Preserved the underlying safety rule in every case: re-enabling `MAINTENANCE_MODE` still requires an explicit user ask in a current session, same as before. Also flagged, not fixed: cron-job re-enable status is unconfirmed — `DD-P0-2` last recorded all pg_cron jobs frozen, and nothing in this session's evidence shows that changed alongside the wall removal.

**UI fix**: [AppLayout.jsx](src/components/AppLayout.jsx) rendered `CompetitionTopBar` unconditionally inside `#main-content` for every route, including `/home` (the cross-clubhouse `HomeDashboardScreen`), which has no single active competition to represent. Gated it behind `location.pathname !== '/home'`. `CompetitionScreenNav` (the sport-screen sub-nav underneath it) already self-gates on pathname — returns `null` unless the route is a football/F1/tennis screen — so it needed no change.

Verified live in the browser preview: `/home` no longer shows the competition pill bar; clubhouse/league/F1/tennis screens still show it as before. `npm run lint` 0 errors, `npm run build` clean, `npx playwright test e2e/platform.spec.js` green. No migration, DB write, or Edge Function touched.

## ✅ Tennis Clubhouse 404 + default landing route (2026-08-30) — PR #870

**Context**: user reported a 404 when trying to open "the tennis test league" from the app, plus asked to (1) ensure the mobile bottom bar is always available and (2) make the app open to the Home page.

**Fix**: `ClubhouseScreen.jsx`'s `AllCompetitions.onEnter` tennis branch navigated to `/tennis/${item.id}`, which matches no registered route — corrected to `/tennis/tournament/${item.id}`, matching the registered `/tennis/tournament/:id` route. Confirmed via codebase-wide grep this was the only incorrect call site (`HomeDashboardScreen.jsx`, `TennisProfileView.jsx`, `TennisHomeScreen.jsx`, `CompetitionTopBar.jsx`, `NewCompetitionFlow.jsx` all already used the correct path). This exact dead-link was flagged but deliberately deferred as out-of-scope in the PR #866 BACKLOG entry — now fixed as a real follow-up.

**Default landing route**: `App.jsx`'s root path (`/`) now redirects to `/home` instead of `/clubhouse`, per explicit user request.

**Bottom bar — investigated, no fix needed**: live-verified in the browser (375×812) that the mobile bottom bar already renders unconditionally on every route inside `AppLayout`, including the 404 catch-all — it's a sibling of the scrolling main-content div, `position: fixed`, no ancestor transform/overflow to break it. It's only hidden by design (`lg:hidden`) at desktop widths ≥1024px, where the sidebar nav takes over.

Verified: `npm run lint` 0 errors, `npm run build` clean (no Rolldown TDZ regression), `npx playwright test e2e/platform.spec.js` 84/84 passing (test updated to expect `/home`), full CI green. No migration, DB write, or Edge Function touched.

## ✅ Mobile header/nav polish — header centering, gold-text contrast, clubhouse-oriented bottom bar (2026-08-30) — PR #868

**Context**: user follow-up feedback on PR #866's Clubhouse/Home work, from mobile screenshots — two bugs in the league header bar, plus a request to reassess the mobile bottom nav and the mobile Home screen.

**Fixes**: (1) `HubTopbar`/`HubLeagueHeader` (`src/components/league/HubShared.jsx`) was vertically flush-to-top instead of centered — `alignItems: 'flex-start'` → `'center'`; (2) the bar's cup-phase and DRAFT·H2H badge text used `--gold` (tuned for light/paper surfaces) against the dark `--accent` bar, low contrast — swapped to `--on-shell-gold`, the existing WCAG-AA gold token for dark shell surfaces. Confirmed the bar's blue is indeed the canonical brand accent (`--accent`/`--brand-accent: #1A6FA8`), answering the user's "is this our blue?" question.

**Bottom nav redesign**: mobile bottom bar (`AppLayout.jsx`) changed from sport-scoped tabs (Live/Squad/Market/etc., different per sport) to clubhouse-level tabs (Home/Club/Trophy/Coins) — sport-scoped screens already live in `CompetitionScreenNav` at the top of every screen on both mobile and desktop, so the old bottom bar duplicated it. 3 new icons added to `NavIcons.jsx` (`NavIconHome`/`NavIconTrophy`/`NavIconChallenges`) matching the existing schematic icon style. `platform.spec.js`'s bottom-nav test updated to assert Home/Trophy instead of the removed Market/Squad tabs.

**Investigated, no code change needed**: the mobile Home dashboard (`HomeDashboardScreen.jsx`, shipped in PR #866) already renders cleanly at 375px — live-verified in browser preview. The desktop-only `ClubhouseSwitcher` dropdown was confirmed to already never render on mobile (`hidden lg:flex`); recommended keeping it as-is since the new mobile Home tab now covers the same "switch clubhouse" job.

**Not resolved — flagged open**: a reported "loose"/doesn't-fill-the-window feel on iPhone 17 Pro specifically. `index.html`'s viewport meta (`viewport-fit=cover`) and `index.css`'s safe-area CSS were inspected and are already correct, so the root cause is not yet identified — needs a live-device screenshot or Capacitor/WKWebView-specific diagnosis.

Verified: `npm run lint` 0 errors, `npm run build` clean (no Rolldown TDZ regression), `npx playwright test e2e/platform.spec.js` 84/84 passing, full CI green (lint/security/build/E2E/schema-rehearsal/unit/Tier-3/iOS/Android/Vercel). Live-browser verification (375×812) confirmed the new bottom bar + Home dashboard render correctly; the header fix was verified by code only, since the session's demo account has no populated leagues to render `HubTopbar`/`HubLeagueHeader` against. No migration, DB write, or Edge Function touched.

## ✅ Clubhouse UX bugs + cross-clubhouse Home dashboard (2026-08-30) — PR #866

**Context**: user-reported punch list of 5 bugs plus 2 new-feature requests around the Clubhouse/competition UX post-cutover.

**Bug fixes**: (1) restored the "Create a clubhouse" option in the clubhouse dropdown menu; (2) the football competition header bar (`HubTopbar`/`HubLeagueHeader` in `src/components/league/HubShared.jsx`) was flat dark `--ink` with no brand color — now uses `--accent` (brand blue) with matching `--on-shell`/`--on-shell-dim` text tokens; (3) restored the notification bell on the football competition screen and aligned its styling with the clubhouse-level bell; (4) the football competition Back button sent to a dead league list — now returns to the clubhouse; (5) removed the redundant "Leagues" tab from the football competition nav (`src/components/CompetitionScreenNav.jsx`) now that all leagues live at clubhouse level, not competition level.

**New feature — Home dashboard**: added `src/screens/HomeDashboardScreen.jsx`, a new `/home` route listing every clubhouse the user belongs to and its competitions (football/F1/tennis), with one-click navigation into any clubhouse or competition. Fetches per-circle competitions via `get_clubhouse_competitions` RPC (`Promise.all` across all of the user's circles, since the existing `useClubhouse` hook only scopes competitions to the single active circle). "Home" now sits above the clubhouse switcher in the sidebar (`AppLayout.jsx`) and is reachable via the mobile top-bar username link. Built per the user's explicit ask to use the `/design` (frontend-design) skill — executed the app's existing established aesthetic (dark navy shell, brand blue accent, JetBrains Mono/Archivo Black/Archivo) rather than introducing a new visual language.

**Moved, not duplicated**: "Find clubhouses" (search/join-by-code/create) moved from the clubhouse-level Find tab into the new Home dashboard. Removed the `find` tab and the ~137-line `FindTab` component from `ClubhouseScreen.jsx`. All 3 prior call sites that linked to `/clubhouse?tab=find` (`AppLayout.jsx`'s sidebar add button, `LeagueScreen.jsx`'s empty-state redirect, `CompetitionTopBar.jsx`'s add-competition button) now point to `/home`. `ClubhouseLobby` (the zero-clubhouse empty state on `/clubhouse`) was left untouched — Home is an additional destination, not a replacement.

**Follow-up flagged, not fixed (out of scope)**: `ClubhouseScreen.jsx`'s `AllCompetitions` still navigates to tennis competitions via `/tennis/${item.id}`, which matches no registered route (correct path, used by the new Home dashboard and `CompetitionTopBar.jsx`, is `/tennis/tournament/${id}`) — a pre-existing dead link, worth a small follow-up PR.

Verified: `npm run lint` 0 errors (677 pre-existing warnings, none new), `npm run build` clean (no Rolldown TDZ regression), `npx playwright test e2e/platform.spec.js` 84/84 passed, full CI green (lint/security/build/E2E/schema-rehearsal/unit/Tier-3-local-full-stack/iOS/Android/Vercel). No migration, DB write, or Edge Function touched this session.

## ✅ P2P challenge, direct-bet, and mock-payment test coverage (2026-08-28)

**Context**: following up on the prior Tier 3 CI-wiring session, the user asked what to prioritize next given the upcoming pilot launches with no Stripe account — fake coins only. Grepping migrations 204/205/224/234/235/237/238/239 and the relevant hooks/components confirmed three concrete, named zero-coverage gaps (not general "could always test more"): the P2P challenge lifecycle (the actual fake-coin-moves-between-two-users mechanism, which has already shipped two real prod bugs that a test would have caught), direct-bet creation/submission (`resolve_bet` was already well tested; creation and `submit_bet` were not), and the Stripe-free `purchase-coins` mock-payment path. The user approved a 4-task plan; this entry covers its full landed state.

**Task 1 — `tests/unit/p2p-challenges.test.js` (new, Tier 1)**: full RPC-level coverage of `create_p2p_challenge` (both `gw_total` and `freeform` shapes; error paths `NOT_CIRCLE_MEMBER`, `OPPONENT_NOT_CIRCLE_MEMBER`, `STAKE_TOO_LOW`/`STAKE_TOO_HIGH`, `DUPLICATE_CHALLENGE`, `DAILY_LIMIT_REACHED`, `QUESTION_REQUIRED`/`QUESTION_TOO_LONG`, `INSUFFICIENT_BALANCE`), `accept_p2p_challenge` (`NOT_OPPONENT`, `CHALLENGE_NOT_PENDING`, `CHALLENGE_EXPIRED`), `decline_p2p_challenge`/`cancel_p2p_challenge` (asserting the challenger's balance is restored to *exactly* its pre-stake value — a direct regression test for the migration-204 double-refund bug fixed in 235), `resolve_p2p_challenge` (win/loss payout math, the tie path crediting no extra coins — a direct regression test for migration-205's double-credit bug fixed in 224 — plus `CHALLENGE_NOT_ACCEPTED`/`MATCHDAY_NOT_SETTLED`), and the full freeform `declare_freeform_result`→`confirm_freeform_result`/`dispute_freeform_result`→`arbitrate_freeform_result` chain (win/loss/push payouts, `CANNOT_CONFIRM_OWN_PROPOSAL`, `NO_PROPOSAL`, `NOT_PARTICIPANT`, `NOT_CIRCLE_OWNER` on non-owner arbitration attempts). Writing the freeform-dispute tests surfaced **`BUG-P2P-DISPUTE`** (new row, P1 table) — a real, previously-unknown, currently-latent production bug; its fix migration is drafted but deliberately withheld pending explicit per-session approval, per CLAUDE.md.

**Task 2 — `tests/unit/bet.test.js` extended (Tier 1)**: `submit_bet` happy path, re-submit resets `is_correct`/`reward_awarded` to `NULL` even after a prior resolve (exercises the migration-76 `ON CONFLICT ... DO UPDATE` path), the migration-98/AUDIT-57-02 `UNAUTHORIZED` regression (submitting with a squad you don't own), `BET_NOT_FOUND`, `BET_CLOSED`, `DEADLINE_PASSED`.

**Task 3 — `e2e/bet-lifecycle.spec.js` (new, Tier 3)**: proves the commissioner-only RLS policy on the raw `bet_instances` table INSERT actually holds — both the positive case (commissioner insert succeeds) and the negative case (a non-commissioner member's identical insert is rejected by RLS), something Tier 1's Postgres-role-based harness can't exercise. Then a full real-session cycle: member calls `submit_bet` and sees the answer render on `/league`; commissioner calls `resolve_bet`; member's points update on the league standings screen (read via a fresh RPC baseline rather than a hardcoded value, since points are recomputed not incremented and both Playwright projects share one DB). Needed a small production fix to make the League screen's own-squad/draft lookups assert correctly against a real signed-in session: `src/screens/LeagueScreen.jsx` gained a `resolveUserId()` helper (mirrors the already-shipped `F1RacePickForm.jsx`/`TennisAdminScreen.jsx` pattern) so 4 `user_id` filters use the real session id instead of `useAuth()`'s possibly-frozen demo identity.

**Task 4 — `e2e/p2p-challenge-lifecycle.spec.js` (new, Tier 3) + `e2e/wallet-screen.spec.js` mock-payment addition**: a live `create_p2p_challenge` (freeform) → `accept_p2p_challenge` → `declare_freeform_result` → `confirm_freeform_result` cycle between two real signed-in sessions, with service-role assertions on both wallets' `escrow` and final balances — proves the RLS/grants/UI plumbing genuinely works (Task 1 already proves the payout math in isolation). The mock-payment addition calls `purchase-coins/create-payment-intent` directly with a real session and a seeded `coin_packs` row, asserting `mock:true`, `coins_credited` matches the pack, and the wallet balance increases accordingly — enabled by a new gitignored `supabase/functions/.env` (`MOCK_PAYMENTS=true`, local-stack-only, confirmed covered by `.gitignore`'s `.env*` pattern). `supabase/seed.sql` extended: a `coin_wallets` row for `USER_B` (P2P opponent — `debit_coins_to_escrow` raises `WALLET_NOT_FOUND` without it) and one active `coin_packs` row.

**Verification-driven fix**: the full local Tier 3 suite (`npm run test:e2e:local`) initially came back with 2 failures in `e2e/tennis-screens.spec.js` (SUITEs 2.1 and 3.1, `desktop-chrome` only) — root-caused via the failure's error-context stack trace plus the fact that the identical tests passed on `mobile-chrome` in the same run: `fullyParallel: false` does **not** stop `desktop-chrome`/`mobile-chrome` from running the same spec file concurrently in separate workers against the one shared local DB, and SUITEs 2, 3, and 5 each reset-then-mutate rows keyed to the single shared `USER_A` identity — the identical race class already fixed once this session in `wallet-screen.spec.js`'s balance test. Fixed by applying that same established pattern (`test.skip(testInfo.project.name !== 'desktop-chrome', ...)`) to all three affected tests (2.1, 3.1, and 5.1, which shared the same hazard shape though it hadn't yet flaked) and rewriting the file's header comment, which had wrongly assumed sequential project execution.

**A second, unrelated bug surfaced during that same full-suite verification pass**: `src/screens/f1/F1RacePickForm.jsx`'s mount-time fetch of a user's existing race picks applied unconditionally in its `.then()` callback, with no guard against the user (or a concurrent Playwright worker) having already started filling in the form — under the heavier concurrent load from this session's new specs, the fetch could resolve after selections were made and silently revert them, leaving "SAVE PICKS" permanently disabled. Fixed with an `editedRef` interaction-guard (`useRef(false)`, set by every onChange in the form, checked before the fetch applies fetched values) — the same shape of fix as the `tennis-screens.spec.js` race above, but in application code rather than a test file.

Verified, after both fixes: `npm run test:e2e:local` full suite genuinely green in a clean isolated run (0 failures, 311 passed, 9 skipped — an earlier concurrent run showed 8 unrelated failures, traced to resource contention from running `npm run build` in the foreground at the same time, not a real regression), `npm run lint` 0 errors (668 pre-existing warnings, all raw-hex-color, none from this session's changes), `npm run build` clean (no Rolldown TDZ regression, 9.33s). No migration applied to production, no Edge Function deployed, no `db query --linked` write — `BUG-P2P-DISPUTE`'s fix is documented but explicitly not applied. `docs/testing/TESTING_STRATEGY.md` updated to match. See the updated `TEST-P2P-1`/`TEST-E2E-1` rows and new `BUG-P2P-DISPUTE` row in the P1 table below.

## ✅ Tier 3 CI wiring + 5 new screen smoke specs + BUG-DRAFT-SVC fix (2026-08-28)

**Context**: closing out the three gaps named explicitly when the user asked "what can we tackle in terms of tests now": Tier 3 (`npm run test:e2e:local`) never runs in CI, six screens (Wallet, Challenge, Trophy Cabinet, Draft, Settings, Auth) have no e2e coverage, and `BUG-DRAFT-SVC` (logged in the prior session) blocks two existing draft specs from ever running their real assertions. Research this session found Draft's actual gap is `BUG-DRAFT-SVC` itself — `draft-mode-complete.spec.js` already exists and covers the screen, it's just permanently skipped — narrowing the "new coverage" work to 5 specs, not 6.

**Shipped**:
- **CI wiring**: new `e2e-tier3` job in `.github/workflows/ci.yml` — installs the pinned Supabase CLI (`supabase/setup-cli@v1`, matching `migrate.yml`'s convention) and Playwright's real `chrome` channel, then runs `npm run test:e2e:local` as-is (reusing `scripts/e2e-local.mjs`'s existing bootstrap rather than reimplementing it in YAML). Deliberately **not** added to branch protection's required checks yet — lands as an informational/non-blocking status check first so real GH-hosted-runner timing and Docker stability can be observed across a few runs before it's made merge-blocking. `timeout-minutes: 45` (generous, since no cold-start timing data exists yet for a fresh runner's Docker pull).
- **5 new screen smoke specs** (`e2e/wallet-screen.spec.js`, `e2e/challenge-screen.spec.js`, `e2e/trophy-cabinet-screen.spec.js`, `e2e/settings-screen.spec.js`, `e2e/auth-screen.spec.js`), all added to `playwright.config.js`'s CI `testIgnore` list alongside `f1-screens.spec.js`/`tennis-screens.spec.js` (Tier 3-only, not runnable in the demo-mode `e2e` job). Two new `supabase/seed.sql` rows: one resolved `p2p_challenges` bet (Challenge) and one `trophy_ledger` row owned by the demo user (Trophy Cabinet); Wallet and Settings needed no new seed data.
- **Demo-mode split-identity finding** (documented inline in each new spec, and worth flagging for any future Tier 3 UI spec): `AuthContext.jsx`'s demo build (`VITE_AUTH_ENABLED=false`) permanently freezes `useAuth().user` at a fixed zero-UUID `DEMO_USER`, but the raw `supabase-js` client reads its own session from localStorage independently — so RPC calls (`get_my_wallet()`) correctly reflect whichever real session `injectSession()` puts there, while screens/hooks doing **client-side** comparisons against `useAuth().user.id` (`useChallenges.js`'s bucket filters, `ChallengeScreen.jsx`'s own `HistoryItem` win/loss label logic, `TrophyCabinetScreen.jsx`'s query, `SettingsScreen.jsx`'s query) always see the frozen demo identity regardless. This shaped both the seed design (Trophy Cabinet's row must be owned by the demo user, not a real seeded user) and the assertion strategy (Challenge's spec asserts on the identity-agnostic "Settled this season" section rather than a per-item win/loss label, since even `history`-bucket items render their win/loss text off the frozen demo id).
- **`BUG-DRAFT-SVC` fixed** (test-only, per explicit user scope decision — no Edge Function or migration changes): corrected root cause was that `run-draft-lottery` was never designed to accept a service-role caller alongside `league_id` (a never-supported invocation pattern, not a regression). `draft-mode-complete.spec.js` now mints a real user, grants it commissioner via the service-role client, signs in, and invokes the function through that authenticated session; `draft-allocation-e2e.spec.js` signs in as the already-seeded `USER_A` commissioner instead. Both files' gating env var renamed `SUPABASE_SERVICE_ROLE_KEY` → `E2E_LOCAL_SERVICE_ROLE_KEY` (un-skipping test bodies that previously always skipped). Separately fixed a mismatched `onConflict` target on a `draft_submissions` upsert (`league_id,user_id` → `league_id,user_id,phase`, matching the real constraint from migration 104).

**Auth coverage is deliberately smoke-only** (explicit user scope decision): asserts the Sign In/Create Account tabs render their expected fields and that client-side password-mismatch validation shows the right error text — no real `signInWithPassword` round-trip against the screen's own UI, which would need a second `VITE_AUTH_ENABLED=true` build out of scope for this pass.

No migration applied to production, no Edge Function deployed, no `db query --linked` write this session — `supabase/seed.sql` only ever loads onto the ephemeral local/CI stack via `scripts/e2e-local.mjs`, never against the linked project.

---

## ✅ Multi-Sport Test Coverage — Pass 1: F1, Draft Allocation, Cup Elimination, Tennis (2026-08-27)

**Context**: after Tier 3 Foundation shipped (see entry below), the user reviewed the overall test-coverage picture and named the real risk directly — football is well-tested, but F1, tennis, chat, coins, and draft mechanics are almost entirely untested — and asked for a plan to close that gap "gradually," first pass now, more later. Two Explore passes mapped every candidate Edge Function against the two Tier 1 patterns already established in this repo (pg-client tests against real SQL RPCs, and pure-JS extraction tested with plain `node:test`), splitting candidates into **zero-refactor** (logic already isolated, purely additive test-writing) and **needs-refactor-first** (logic inline in an async handler, needs extracting before it's unit-testable).

**Shipped** (all four zero-refactor targets, plus tennis pulled forward from "needs-refactor" at the user's explicit request):
- **F1 bet-scoring**: `supabase/functions/score-f1-race/scoring-logic.ts` (new) — `SCORING` constants + `scoreRaceBet(bet, race)` moved out verbatim (already 100% pure, no DB calls — this is F1's entire scoring mechanism, a prediction/pick'em game scored against a race result, not a fantasy roster). `index.ts` now imports it. `tests/unit/f1-scoring-logic.test.js` — 14 tests: exact-position picks, wrong-spot-but-podium, DNF hit/miss, team-most-points hit/miss, special-category match (case/whitespace-insensitive), all-correct bonus, empty bet.
- **Draft allocation**: `supabase/functions/_shared/snakeDraft.ts` was already an extracted, explicitly-labeled pure module (`normalisePosition()`, `shuffleOrder()`, `runSnakeDraft()` — the shared allocation core behind both `run-wishlist-draft` and `run-draft-lottery`) with zero tests. `tests/unit/snakeDraft.test.js` (new, no extraction needed) — 15 tests covering position-cap/budget/club-cap constraints and the snake turn order.
- **Cup elimination**: `tests/unit/cup-elimination.test.js` (new, pg-client pattern like `bet.test.js`) — 10 tests against `eliminate_cup_club` (migration 06) and `sync_cup_eliminations` (migration 221) across its real branches: future-fixture guard, incomplete-matchday guard, clear win/loss, penalty-shootout-resolved draw, unresolved draw with no shootout data, and self-healing a stale elimination.
- **Tennis tournament scoring** (the one genuine refactor this pass): `supabase/functions/score-tennis-tournament/scoring-logic.ts` (new) — `TIER_PTS`/`DEEP_ROUNDS`/`EARLY_EXIT`, `rosterPlayerIds()`, and `scorePlayer()` moved out verbatim (already pure); the real per-roster scoring loop, previously inline inside the `Deno.serve` handler closing over handler-local variables, was genuinely lifted into a new named `scoreRoster(roster, playerMap, captainId)`. `index.ts`'s handler now just calls it — everything else (tournament/players/rosters/captains load, score upsert, gazette entry, trophy award, `admin_complete_tournament` RPC) is byte-for-byte unchanged. `tests/unit/tennis-scoring-logic.test.js` (new) — 23 tests: per-tier scoring, the `dark_horse_insurance` floor, captain-bonus doubling, and all three flat ace-card bonuses (`underdog_boost`, `safety_net`, `surface_specialist`) across hit/miss cases. Because this is a real code-shape change and not just relocation, it was additionally verified by a before/after manual trace against a seeded roster (17 pts under both the old inline logic and the new extracted function).

**Verified**: `npm run test:unit` — 129/129 passing across 37 suites (up from 106/33 before this pass). `npm run lint` — 0 errors (668 pre-existing unrelated warnings). `npm run build` — clean, no Rolldown TDZ regression from the two new sibling modules. `docs/testing/TESTING_STRATEGY.md` updated to reflect closed and remaining gaps.

**Explicitly out of scope for this pass, tracked as follow-up** (per the plan, nothing silently dropped): the relaxation formula (`calculate_relaxation_state`/`apply_relaxation_state`) — dropped from the whole plan per explicit user instruction, since the feature itself may be retired; ATP Finals scoring extraction (`score-atp-finals/index.ts:129-158`, no pure function exists yet) and `run-reverse-standings-draft`'s own duplicated allocation/tiebreak logic — both flagged as "Pass 2," needing the same refactor care tennis just got; Tier 3 e2e coverage for F1/tennis/chat/coins/draft-timing flows — tracked as a "Tier 3 multi-sport" follow-up once Tier 3 CI wiring lands.

---

## ✅ Tier 3 Foundation — seed script, spec retargeting, local orchestration (2026-08-27)

**Context**: the previous entry's close-out left Tier 3 (local full-stack E2E) at groundwork-only — `supabase/config.toml` wired for a full local stack, but no seed data and no specs actually retargeted. The user scoped this session's follow-up as **"Foundation only"**: get all 8 gated specs (everything under `e2e/` except `platform.spec.js`, which has its own separate CI-required demo-mode run path) passing against a local stack, with no CI wiring and no new coverage areas — those stay tracked as open follow-up, not attempted here.

**Shipped**:
- **`supabase/seed.sql`** — new committed synthetic dataset: 2 deterministic `auth.users`/`auth.identities` accounts (`e2e_a@fantasykit.test`/`e2e_b@fantasykit.test`, `E2ePass!99`) matching what `autofill-draft-classic.spec.js` already hardcoded, avoiding the need to write the missing `e2e-setup` Edge Function the prior entry flagged; 1 circle with both as members; synthetic EPL (~120) and WC (~40) player pools sized for real position-cap/budget headroom; one finished EPL fixture with a hand-worked deterministic scoring scenario (goal, clean sheet, card — expected fantasy-point totals documented inline and asserted in the rewritten `scoring-pipeline.spec.js`); a classic EPL league and a draft/`noduplicate` WC league with membership rows for both seeded users; a `matchday_deadlines` row for the scenario round.
- **`scripts/e2e-local.mjs`** (new `npm run test:e2e:local`) — bootstraps the local stack by loading `supabase/schema.sql` directly (not migration replay — a from-scratch replay hits a real, uncaptured prod schema mismatch at migration 09, and the append-only migration rule means that can't just be patched) then `seed.sql` on top, reads the local stack's URL/anon key via `supabase status -o json`, and execs Playwright across all 8 gated specs.
- **3 previously hard-blocked specs rewritten** onto the seeded data: `autofill-draft-classic.spec.js`, `draft-allocation-e2e.spec.js` (hardcoded prod UUIDs → seeded ones), `scoring-pipeline.spec.js` (hardcoded real-match assertions → the seeded scenario's worked values).
- **`[db.migrations]`/`[db.seed]` left disabled in `supabase/config.toml`** — `e2e-local.mjs` is the one thing that loads schema/seed data now, not the CLI's own hooks.

**Bugs found and fixed during verification** (not scoped work, but blocking/silent-failure findings surfaced by actually running this for the first time):
1. **Silent prod-targeting bug**: `playwright.config.js`'s `webServer` runs `npm run build && npm run preview`; without an explicit override, Vite's build step falls back to `.env.local`'s `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY`, which point at **live production** — meaning every UI-driven spec run locally, before this fix, was silently exercising prod rather than the local stack regardless of what `SUPABASE_URL`/`SUPABASE_ANON_KEY` were set to for the Node/Playwright side. A read-only check against prod afterward confirmed no writes actually landed (RLS/FK/JWT-signature mismatches blocked every attempt) — no data was corrupted, but the tier would have been silently non-functional as a safety net. Fixed by having `e2e-local.mjs` explicitly set both `VITE_`-prefixed vars from the local stack's own creds before spawning Playwright.
2. **Demo-mode + RLS identity mismatch** (UI-driven specs only): `AuthContext.jsx` in demo mode (`VITE_AUTH_ENABLED` unset, the default locally) freezes React's `user` at a fixed zero-UUID `DEMO_USER` and never restores a session from storage, but `src/lib/supabase.js`'s client still auto-sends whatever session sits in `localStorage` on every actual request, independent of `AuthContext`. RLS policies keyed off `auth.uid()` (e.g. `is_circle_member()`) see that real injected session; the app's own client-side `.eq('user_id', user.id)` filters see the frozen `DEMO_USER` id instead. A demo-mode UI spec touching per-user RLS-gated data needs **both** a seeded row for `DEMO_USER`'s id *and* an injected real session for a genuine member of the same resource — documented in `docs/testing/TESTING_STRATEGY.md`'s Tier 3 section for future specs, worked example in `draft-allocation-e2e.spec.js`'s "league creation wizard shows Draft as an option" test.
3. **Local `edge_runtime` resource contention under full concurrency**: `autofill-draft-classic.spec.js` test 1.3 makes 7+ sequential Edge Function round-trips (one per player bought) against the local stack's single `edge_runtime` container; running all 8 specs across both Playwright projects (desktop-chrome + mobile-chrome) concurrently pushed that past the file's 20s default timeout even though each call was correct. Fixed with a per-test `test.setTimeout(45000)`, not a weakened assertion — same fix pattern flagged for any future spec doing several sequential Edge Function calls in one test.

**Verified**: `npm run test:e2e:local` — 225 passed, 31 skipped (service-role-gated scenarios + a couple of pre-existing graceful `test.skip()` fallbacks), 0 failed. `npm run lint` — 0 errors (668 pre-existing warnings, unrelated). `npm run build` — clean, no Rolldown TDZ regression. `docs/testing/TESTING_STRATEGY.md` updated throughout to match landed state.

**Explicitly still open** (per the user's "Foundation only" scope, tracked as separate follow-up): CI wiring for this tier (`npm run test:e2e:local` is local-only, no GitHub Actions job invokes it yet), and Tier 3 build-order step 4 — new coverage for F1 (zero coverage of any kind today), tennis (same), Realtime league chat, `resolve_bet` write-in UI flow, coins/P2P beyond the RPC layer, draft lottery/wishlist-draft timing, cron-driven scoring orchestration, `trophy_ledger` emission, notification delivery. No migration, live DB write, or Edge Function deploy was performed or needed this session — every local-stack write went through direct `pg` connections or the standard reseed mechanism, never the linked production project.

---

## ✅ Testing Strategy — 4-tier plan decided and closed (2026-08-27) — PR #856 + PR #857

**Context**: after this session's live-verification pass (see the `TEST_QA_Manager` work in the "Phase A UI Polish Pass" entry below), the user asked whether we had a durable process for platform-level testing beyond scratch scripts, then explicitly asked for a full testing strategy: *"Can you propose one considering all our parts? ... What I always feel is that we're a bit 'short' on the tests, we never cover everything, some parts are always untested."* Follow-up: *"Also consider ... the E2E tests we have on git. Lets ensure we are covering everything but no need for an overkill ... I leave up to you to completely decide on the approach and strategy."*

**Audit findings** (grep-verified against `tests/unit/`, `e2e/*.spec.js`, `.github/workflows/ci.yml`, `docker-compose.yml`, `supabase/config.toml`, `scripts/rehearse-schema.sh`):
- CI already runs `unit-tests` (ephemeral-Postgres RPC/logic tests) and `e2e` (`platform.spec.js` only, demo mode) on every PR. No duplication existed between CI and the manual Docker paths — they test genuinely different things today.
- The hand-rolled `docker-compose.yml` Postgres service was never a substitute for Auth/RLS/Realtime — it explicitly lacks them. The 8 non-`platform.spec.js` E2E specs (`classic`/`draft`-mode flows, scoring pipeline, draft allocation, multi-league/bets, autofill) have **no local target at all** — `e2e/supabase-target.js`'s B-12 guard requires an explicit `SUPABASE_URL`/`SUPABASE_ANON_KEY`, and the only realistic one today is live prod, so these specs are manual-only and rarely run.
- `supabase/config.toml` only declares `[functions.*]` entrypoints — no `[db]`/`[api]`/`[auth]`/`[studio]` sections — so `npx supabase start` (the Supabase CLI's full local Docker stack: Postgres + GoTrue Auth + PostgREST + Realtime + Edge Runtime) has never been wired up for this project, despite being free and being the actual fix for the Auth/RLS gap.
- **F1 and tennis have zero automated coverage of any kind** — no unit tests, no E2E specs reference either sport. Also zero coverage: Realtime league chat, admin RPCs (`competition_admins`), `resolve_bet` write-in UI flow (RPC level is covered, UI is not), coins/P2P beyond the RPC layer (`purchase-coins` untested), draft lottery/wishlist-draft timing (`run-wishlist-draft`/`WishlistDraftScreen` referenced in zero test files), cron-driven scoring orchestration (only the extracted pure-logic path of `calculate-scores`/`score-f1-race`/`score-tennis-tournament` is unit-tested, not the functions as HTTP-invoked orchestration), `trophy_ledger` emission, notification delivery.

**Decision — 4 tiers, each scoped so nothing is tested twice**:
1. **Tier 1 (existing, CI-required)** — `tests/unit/` RPC/logic tests. Extend with F1/tennis scoring-logic and admin-RPC tests (pure logic belongs here, not in a heavier tier).
2. **Tier 2 (existing script, now also CI)** — schema rehearsal against `supabase/schema.sql` (a verified prod snapshot — migration-replay-from-scratch was proven 2026-08-01 to diverge from prod). New `schema-rehearsal` CI job auto-runs only when a PR touches `supabase/migrations/**`, applying the changed file(s) on top with `ON_ERROR_STOP=1`. The interactive local script stays for pre-PR rehearsal + serving Edge Functions against the rehearsed schema, which CI can't do.
3. **Tier 3 (new, groundwork landed, full build-out still open)** — local full-stack E2E via `npx supabase start`. Closes the actual gap: expand `supabase/config.toml`, write one committed synthetic multi-sport `TEST_`-prefixed seed script, retarget (move, not copy) the 8 currently-prod-only specs to this local stack by default, then add new coverage for every confirmed gap above. Explicitly scoped to not re-prove Tier 1's math — Tier 3 tests Auth/RLS/Realtime/UI/orchestration only.
4. **Tier 4 (formalized)** — narrowed live-platform verification for what genuinely can't be faked locally (real Forza API freshness, real wall-clock cron, post-deploy sanity). Formalizes this session's ad hoc `TEST_QA_Manager` scratch scripts into a committed, `TEST_`-prefixed pre-launch gate script instead of one that's reinvented and discarded every session.

**Shipped, PR #856 (foundation)**: rewrote `docs/testing/TESTING_STRATEGY.md` from a stale, self-contradictory 2026-06-01 doc into the authoritative 4-tier reference; added the `schema-rehearsal` CI job to `.github/workflows/ci.yml` (mirrors the existing `unit-tests` job's ephemeral-Postgres pattern, path-gated via `git diff` against the PR base so most PRs skip it as a fast no-op). Verified: YAML syntax valid, `npm run lint` clean (0 errors, pre-existing warnings only — no `src/` files touched).

**Shipped, PR #857 (this entry, closing the strategy)**:
- **Tier 1**: new `tests/unit/admin.test.js` — 9 tests covering the `competition_admins` family (`set_competition_admin`/`remove_competition_admin`/`is_competition_admin`), the shared admin-gating primitive across all 3 sports: success, idempotency, and all 5 rejection paths (`UNAUTHENTICATED`, `NOT_OWNER`, `INVALID_TYPE`, `NOT_LINKED_TO_CIRCLE`, `TARGET_NOT_CIRCLE_MEMBER`). Required 2 new fixture rows in `tests/unit/seed.sql` (`circle_members`, `circle_leagues`) the original seed never populated.
- **Tier 3 (groundwork only, step 1 of 4)**: expanded `supabase/config.toml` with the full `[api]`/`[db]`/`[db.migrations]`/`[realtime]`/`[studio]`/`[auth]`/`[auth.email]`/`[edge_runtime]` sections so `npx supabase start` now boots a fully wired local stack (Postgres 17.6, real migration replay, unconfirmed-signup sessions for immediate test-account use). `[db.seed]` left disabled — no seed script exists yet. While scoping the remaining 3 steps (seed script / spec retargeting / new coverage), found the retargeting work is bigger than it looked: the `e2e-setup` Edge Function `autofill-draft-classic.spec.js` depends on has **never existed** in this repo's git history (that spec can't currently pass against any target), and the other 7 "manual" specs' demo-mode writes against prod only work at all because of a narrow, deliberate RLS carve-out covering exactly 4 tables (`matchday_recaps`, `projection_snapshots`, `squads`, `top_scorer_predictions`) — not a general local-targeting mechanism. Full detail in `docs/testing/TESTING_STRATEGY.md`'s Tier 3 section. Steps 2–4 (seed script, spec retargeting, new F1/tennis/chat/coins/draft-timing/cron/trophy coverage) remain genuinely not-yet-built — tracked as follow-up work, not part of this close-out.
- **Tier 4 (formalized)**: new `scripts/TEST_pilot_gate.js` — a committed, 100%-read-only pre-launch gate script replacing the ad hoc `TEST_QA_Manager` scratch checks. 6 checks against production (cron failure streaks, inactive cron jobs, stale-scheduled fixtures, unpriced players, recent edge-function errors, same-day real-league hygiene). Run once against live prod this session: `GATE: PASS` (1 PASS, 4 WARN, 1 INFO) — the WARNs (22 inactive cron jobs, 21 stale fixtures, 499 unpriced players, recent `generate-frontpage-edition` errors) line up with the site's deliberate post-cutover `MAINTENANCE_MODE=true` pause, not new defects.

**Full run before closing, per explicit user instruction**: `npm run lint` (0 errors), `npm run build` (clean, no Rolldown TDZ regression), `npm run test:unit` (67/67 passing across 14 suites, including the 9 new admin tests) — all green. `scripts/TEST_pilot_gate.js` run once and returned `GATE: PASS`.

**Still genuinely open, tracked as separate follow-up work, not blocking this close-out**: Tier 3 steps 2–4 (multi-sport seed script, retargeting the 8 manual E2E specs — including fixing/replacing the missing `e2e-setup` function — and new coverage for F1/tennis/chat/admin-UI/coins/draft-timing/cron-orchestration/trophy_ledger/notifications). No migration, live DB write, or Edge Function deploy was performed or needed — every prod interaction this session (including the new gate script) was read-only.

---

## ✅ Phase A UI Polish Pass — shared rank badge, notification bell, header/scroll fixes + TEST_QA_Manager live-verification infra (2026-08-27) — PR #854

**Context**: a cross-sport UI-polish pass (Phase A of a larger multi-part request the user sequenced as "UI polish first, then testing/revision") addressing 8 chrome/UX inconsistencies across football, F1, and the Clubhouse shell: an uncollapsible League Activity feed, three divergent rank/podium treatments, a "very mono-color" football League screen, F1's next-race row using only a left-accent border instead of a full highlight, misaligned Clubhouse/League headers, two duplicated notification-bell implementations, a scroll-position bug on Clubhouse switch, and a duplicated "+ INVITE" button at League level. Ahead of merging, also built a reusable `TEST_QA_Manager` passwordless QA account (created via the Supabase Admin API + a server-side magic-link exchange — no password ever set, verified against auth rules) and joined it into two real pilot circles ("Mundial do Eder" football league, "Cascais 16 F1" paddock) plus a newly-created `TEST_Wimbledon_Box` tennis competition inside `CH_TEST_TENNIS`, then used that session in the Browser pane to live-verify all 8 tasks against real, authenticated production data — a step up from the code-inspection-only verification some recent Clubhouse-scoped entries had to settle for.

- **Per-entry Activity feed collapse/expand** — [LeagueDetailView.jsx](src/components/league/LeagueDetailView.jsx)'s League Activity feed piled up one full card per Gameweek with no way to collapse it, unusable across a 34-GW season. Added per-entry `expandedIds` state (default collapsed) with a `+`/`−` toggle on both the desktop headline row and a new mobile equivalent (mobile previously never rendered bullets at all), applied uniformly across every `entry_type`. Confirmed live: clicking a headline flips the glyph and reveals its bullet content.
- **Shared `CompetitionRankBadge` component** — replaced 3 inconsistent rank treatments (football's colored numeral boxes, F1's bare medal emoji, `CompetitionResultsHeader`'s bare emoji) with one new `src/components/CompetitionRankBadge.jsx`: ranks 1–3 always get fixed gold/silver/bronze coloring, ranks 4+ use the passed-in sport accent color, collapsing the old `useMedals` boolean prop entirely. Swapped into [LeagueDetailView.jsx](src/components/league/LeagueDetailView.jsx)'s podium, [F1HomeScreen.jsx](src/screens/f1/F1HomeScreen.jsx)'s "Top of the Paddock" list, and `CompetitionResultsHeader.jsx`'s mobile/desktop rank column. Confirmed via source inspection at all 3 call sites plus a live tennis-leaderboard load (post-`useMedals`-removal) with no crashes.
- **Mono-color polish on the football League screen** — [LeagueDetailView.jsx](src/components/league/LeagueDetailView.jsx): standings header row now uses a `2px solid var(--accent)` bottom border instead of the flat `--rule` gray, the current user's standings row gets an `--accent-bg` highlight, and the GW/lead-score spotlight card's divider was tinted to `--accent`, echoing F1's existing next-race tint. Confirmed live via computed-style inspection (`rgb(26, 111, 168)` accent border, accent-tinted current-user row) against the real "Mundial do Eder" league.
- **F1 next-race row: full border, not left-accent** — [F1HomeScreen.jsx](src/screens/f1/F1HomeScreen.jsx) replaced the old `1px` full border + `3px` left-accent-only override with a single uniform `2px solid var(--f1)` border on the upcoming race's calendar row. Confirmed via source inspection (no `borderLeft` override remains).
- **Clubhouse/League header alignment** — aligned the right-side action-cluster gap (12px) and element ordering (`[metadata] → [invite/tour control] → [NotificationBell]`) between `ClubhouseScreen.jsx` and the shared `HubTopbar` used by `LeagueScreen.jsx`, without merging the two header components (their left-side eyebrow styles differ enough that forcing a shared component would be unnecessary churn). Confirmed visually consistent live between both screens.
- **Shared `NotificationBell` component** — consolidated the two previously-divergent bell UIs into one new `src/components/NotificationBell.jsx` (lifted from `ClubhouseScreen.jsx`'s more polished existing implementation — CSS-drawn tray icon, mobile full-screen panel / desktop dropdown), generalized via a `renderRow` prop so each call site keeps its own row-rendering logic for its differently-shaped data (`clubhouse_notifications` vs. `useNotifications.js`'s league shape) instead of forcing a fake shared schema. `ClubhouseScreen.jsx` now imports it directly; `LeagueScreen.jsx`'s desktop and mobile bells were migrated off the old `NotificationPanel.jsx`, which was then deleted (confirmed via grep it had no other importers). Confirmed live: League-level bell opens, shows the correct empty state, no console errors.
- **Clubhouse-switch scroll-reset fix** — [AppLayout.jsx](src/components/AppLayout.jsx)'s persistent `#main-content` scroll container never unmounted across route changes, so switching Clubhouses via the sidebar dropdown left the new screen scrolled to the previous screen's position. Added a ref + `useEffect` resetting `scrollTop` to 0 on every `location.pathname` change. Confirmed live: scrolled to a capped `scrollTop` of 116 in the F1 paddock, switched to the "Mundial do Eder" league via the Clubhouse switcher, confirmed `scrollTop` reset to 0 on the new screen.
- **Removed duplicate "+ INVITE" button from League level** — [LeagueScreen.jsx](src/screens/LeagueScreen.jsx) dropped both the desktop and mobile "+ INVITE" header buttons (Clubhouse-level invite, in `ClubhouseScreen.jsx`, already covers this); retargeted the onboarding tour step that pointed at the now-removed `data-tour="league-invite"` element so the tour doesn't break. Confirmed live: button absent from both League header layouts, "Replay league tour" control still present and functional.

Verified: `npm run lint` (0 errors), `npm run build` (clean, no Rolldown TDZ regression), CI 84/84 E2E green, and — unlike several recent Clubhouse-scoped entries that could only be verified by code inspection for lack of a Clubhouse-member test account — a full live-browser pass of all 8 tasks against real, authenticated production data via the new `TEST_QA_Manager` account, joined into two real circles plus the newly-created `TEST_Wimbledon_Box` tennis competition (which also rendered end-to-end with no crashes: calendar, roster confirmation, and the post-`useMedals`-removal leaderboard chain all loaded cleanly with the expected empty "No scores yet" state). PR #854 squash-merged to `main` (commit `5446a4b`); no `supabase/functions/` files touched, so no Edge Function deploy was needed.

## ✅ F1 picks roster + Clubhouse activity nav fixes (2026-08-27) — PR #852

**Context**: a third round of pilot feedback (5 items, 5 screenshots) — 2 F1-specific, 3 generic Clubhouse-level issues. The user reported some F1 predictions not appearing on certain races, a redundant winner display, unexplained value in having a Clubhouse-level Activity feed alongside the league-level equivalent, a confusing navigation target when clicking a Clubhouse activity entry, and a suspected cache/leak issue where activity from one Clubhouse briefly appeared under a different one.

- **Missing predictions on some races** — root-caused to [f1-data.js](src/lib/f1/f1-data.js)'s `DRIVERS`/`TEAMS` constants being a stale 2024-vintage roster: a React `<select value={x}>` renders with no visible selection whenever `x` doesn't exactly string-match any `<option>`, even though the underlying stored pick is intact — so any pick referencing a driver/team not in the stale list (Antonelli, Hadjar, Bortoleto, Lindblad, or the new Cadillac team) rendered blank, alongside several accent/formatting mismatches (`Sergio Perez`→`Sergio Pérez`, `Nico Hulkenberg`→`Nico Hülkenberg`, `Carlos Sainz`→`Carlos Sainz Jr.`, `Alexander Albon`→`Alex Albon`, `Red Bull Racing`→`Red Bull`). Cross-referenced live DB data (`f1_bets_race`/`f1_races` distinct driver/team values) to build the correct 22-driver/11-team 2026 list. This is at least the fourth instance of the same "stale reference/seed data" bug class found this multi-session task.
- **Redundant winner badge** — [F1HomeScreen.jsx](src/screens/f1/F1HomeScreen.jsx) showed a small 🏆 winner badge in the race's collapsed header row unconditionally for any finished race, duplicating the full podium already shown in the RACE RESULT box once expanded. Now suppressed once `isExpanded` is true.
- **Confusing screen on Clubhouse activity click** — [ClubhouseScreen.jsx](src/screens/ClubhouseScreen.jsx)'s `FeedEntry.onEnter` was calling `onEnter(entry.league_id)` (a raw UUID string) where the wired handler `enterLeague(league)` expects an object with an `.id` property — evaluating to `navigate('/league/undefined')`, which still matches `LeagueScreen`'s detail-hub route and renders a broken-looking screen with no real league behind it. Fixed to call `onEnter({ id: entry.league_id })`.
- **Global "My Leagues" list ignoring Clubhouse scoping** — same click's second-order complaint: `LeagueScreen.jsx`'s LEAGUES list queried `league_members` by `user_id` only, showing every league the user belongs to across every Clubhouse, not just the active one. Now filters through `circle_leagues` for the active Clubhouse (`useClubhouseContext()`), falling back to the unfiltered list only when there's no active Clubhouse. Checked for the Rolldown TDZ import-depth crash pattern before adding the import (no descendant of `LeagueScreen` also imports `useClubhouseContext`) — confirmed build clean.
- **Stale activity leaking across Clubhouses** — root-caused to [useClubhouse.js](src/hooks/useClubhouse.js)'s `fetchCircleData(circleId)`: it only reset its five state slices (`competitions`, `feed`, `members`, `metaStandings`, `notifications`) on the `!circleId` branch, never when switching between two different real Clubhouses, and had no cancellation/race guard around its `Promise.all` — unlike the correctly-implemented sibling `useClubhouseFrontpage.js`. Fixed: state now resets unconditionally at the start of every call, and a `latestCircleIdRef` guard discards any response that resolves after a newer switch has already started.
- **Clubhouse-level Activity feed "duplication"** — not a bug: `ClubhouseScreen`'s Activity panel is scoped to everyone's activity across every league linked to that Clubhouse (via `circle_leagues`), while Recap's "My Digest" is the logged-in user's own activity across all their Clubhouses (via RLS only, no Clubhouse scoping) — genuinely different value once a Clubhouse has 2+ linked competitions, but they look identical for a single-league Clubhouse like Cascais 16 F1, which is what prompted the report. Put to the user as a 3-way choice (keep as-is / hide for single-league Clubhouses / remove entirely) — user chose **keep as-is**, no code change.

Verified: `npm run lint` (0 errors, pre-existing style warnings only), `npm run build` (clean, no Rolldown TDZ crash), CI 84/84 E2E green. F1 roster and winner-badge fixes confirmed live in browser against real data. The three Clubhouse-scoped fixes were verified by code inspection + build only — this session's demo account has no Clubhouse membership, and creating/modifying real users' Clubhouse membership rows purely to enable a live click-through was judged out of scope without an explicit ask.

## ✅ F1 paddock feedback round 2 — calendar merge, picks clarity, report parity, top bar/sidebar polish (2026-08-26) — PR #848

**Context**: the "Cascais 16 F1" paddock creator sent a second, 8-item feedback pass after PR #846 went live (6 screenshots attached). Diagnosed and fixed the 6 items that were pure code changes on `claude/f1-picks-report-chat-fixes`, verified live against the real paddock (real production Supabase data, not demo mode) before merging. The remaining 2 items are held for user input — see below.

- **PADDOCKS merged into CALENDAR, separator removed** — [F1HomeScreen.jsx](src/screens/f1/F1HomeScreen.jsx)'s old two-section toggle (CALENDAR vs. PADDOCKS) is gone. The 2×2 "Paddock Shortcuts" grid now renders inline at the bottom of the single CALENDAR view, below the hero card, leaderboard preview, and full season list.
- **Misleading PICKS lock banner** — [F1RaceBetScreen.jsx](src/screens/f1/F1RaceBetScreen.jsx) previously showed the same "picks open, save anytime" copy for a race 2 months out as it did moments before lock, and a stale "picks locked — qualifying has started" for races that had already finished. Now branches on `race.status === 'finished'` first (🏁 "Race finished"), then the existing lock check, so a finished race never claims qualifying "has started" in the present tense.
- **Report screen breakdown showing "+0" for every category** — root cause: every currently-scored round (1, 2, 3, 6–13) was written by an older scoring pipeline using a different `f1_scores.breakdown` key schema (`podium_p1_pts`, `wrong_spot_pts` as one lump sum, `dnf_pts`, etc.) that predates the current `p1`/`p2`/`p3`/`dnf`/`team`/`special`/`bonus` schema [F1ReportScreen.jsx](src/screens/f1/F1ReportScreen.jsx) was reading. Added a `normalizeBreakdown()` translation helper so old-schema rows render correctly without touching any stored data, plus a new "🎯 Podium (right driver, wrong spot)" row surfacing the old schema's `wrong_spot_pts` bucket, which the current UI had no row for at all. Verified against real production data — R1's breakdown now shows each member's actual per-category points instead of a uniform +0.
- **Top bar too close to the viewport edge** — added a 12px desktop-only spacer (`hidden lg:block`) in [AppLayout.jsx](src/components/AppLayout.jsx) directly above the competition tab strip; mobile is unaffected (it already sits under the mobile top bar).
- **Sidebar "The FrontRow" bullet smaller than other items** — same file: it was the only `NavItem` still passing a leftover `sub` prop, shrinking its dot/padding/font relative to the other four. Removed; all five now render identically (5px dot, 7.5px/10px padding, 12.5px font — confirmed via computed styles).

Both items originally deferred here — the CALENDAR/PICKS overlap design question and the missing Cascais 16 chat channel — are now resolved. See "✅ F1 PICKS folded into CALENDAR" directly below for the former; the latter was a one-row `INSERT` into `clubhouse_channels` (circle `f5cc62be-1ca3-4d49-a0a5-3997e7319e6c`, `General`/`is_default=true`, `created_by` set to the paddock creator), run with fresh in-session approval and confirmed live in the DB — no PR, data-only fix, same pattern as the earlier `DATA-3` per-clubhouse channel backfill.

Verified: `npm run lint` (0 errors, only pre-existing unrelated warnings), `npm run build` (clean, no Rolldown TDZ), `npx playwright test e2e/platform.spec.js` (84/84 passed), plus live-browser verification against real production data for every item above.

## ✅ F1 PICKS folded into CALENDAR — race rows expand in place (2026-08-26) — PR #850

**Context**: the last open item from the round-2 feedback batch above — "don't like the PICKS tab" plus the CALENDAR/PICKS overlap — needed a design recommendation before any nav change. Recommended folding PICKS into CALENDAR the same way PADDOCKS was merged in for PR #848 (race cards expand in place to reveal the pick form, instead of a separate tab); user approved ("Let's go with your suggestion").

- Extracted the pick-submission form out of the old standalone `F1RaceBetScreen.jsx` into a new [F1RacePickForm.jsx](src/screens/f1/F1RacePickForm.jsx), scoped to a single `{ race, paddock }` — no URL params, no races-list fetch, no round-selector strip.
- [F1HomeScreen.jsx](src/screens/f1/F1HomeScreen.jsx)'s CALENDAR race rows now toggle an `expandedRound` state — clicking a row (or the hero's "next race" button, which also scrolls to it) reveals `F1RacePickForm` inline below that row instead of navigating away. Every row is clickable now (the old `isDimmed`-disables-click behavior for non-"next" races is gone; `isDimmed` is now purely a cosmetic opacity on collapsed rows).
- Removed the PICKS entry from both independent F1 nav configs — [CompetitionScreenNav.jsx](src/components/CompetitionScreenNav.jsx) (desktop) and [AppLayout.jsx](src/components/AppLayout.jsx) (mobile bottom nav) — each has its own `buildF1Screens`/`buildF1Nav` list and had to be edited separately.
- Old `/f1/:paddockId/picks/:round?` links redirect to the paddock's CALENDAR root (`F1PicksRedirect` in [App.jsx](src/App.jsx), same `Navigate`-based pattern as the existing `JoinRoute`) instead of 404ing, so bookmarked/shared links keep working.
- Deleted the now-orphaned `F1RaceBetScreen.jsx`.

Verified live against the real "Cascais 16 F1" paddock (not demo mode): PICKS gone from desktop and mobile nav; hero button and calendar rows correctly expand/collapse; an upcoming race (R15) and a finished race (R13) both render correctly (R13 shows the race-result block and no Save button since picks are locked); old `/picks/:round` URLs redirect to CALENDAR; Paddock Shortcuts grid still renders (now 3 cards, PICKS removed). `npm run lint` (0 errors), `npm run build` (clean, no Rolldown TDZ). This closes out the entire 8-item F1 paddock feedback batch — no open items remain from that report.

## ✅ F1 Paddock pilot feedback batch — admin gate, calendar, nav, report parity (2026-08-26) — PR #846

**Context**: the "Cascais 16 F1" paddock creator tested the live pilot after the earlier F1 data migration and reported 7 issues, comparing against the original single-tenant reference platform (`SMTCB/FantasyF1`). Diagnosed and fixed all 7 on `claude/f1-paddock-pilot-fixes`, verified live against the real paddock (logged in as a real non-creator member, not demo mode) before merging.

- **Admin access denied to the paddock creator** — [F1AdminScreen.jsx](src/screens/f1/F1AdminScreen.jsx) was gating on the stale global `users.is_admin` flag (`false` for the creator's account). Now calls the existing `is_competition_admin('paddock', paddockId)` RPC (migration 243), which correctly recognizes the paddock's `created_by` or the linked circle's owner. No DB change needed.
- **Bahrain GP & Saudi Arabian GP shown as "UPCOMING" despite being cancelled** — both never existed in the reference platform's calendar, confirming they were dropped upstream. Confirmed zero dependent rows in `f1_bets_race`/`f1_scores` for rounds 4-5 across both F1 paddocks on the platform, dumped a full DB backup, got explicit in-session "yes, run it" approval, then ran `DELETE FROM f1_races WHERE season = 2026 AND round_number IN (4, 5);`. Remaining rounds keep their original numbers (gap after round 3) rather than renumbering — no UI dependency on sequential round numbers found, and renumbering would touch 19 already-scored rows for no functional benefit.
- **Dutch GP shown as "UPCOMING" despite having already happened** — [F1HomeScreen.jsx](src/screens/f1/F1HomeScreen.jsx)'s `RaceStatusBadge` fell through to a generic "UPCOMING" for any race whose `race_date` had passed but wasn't yet scored (`status` only flips to `finished` when an admin submits results). Now shows "RESULTS PENDING" for that case.
- **Tab underline artifact stays lit on the home tab** — found in two places sharing the same root cause: the group's home path (e.g. `/f1/<id>`) is a genuine string-prefix of every sibling sub-route path, so a naive `pathname.startsWith(path)` check matched both the home tab and the actual active tab simultaneously. Fixed in [CompetitionScreenNav.jsx](src/components/CompetitionScreenNav.jsx) (desktop nav, the one originally reported) by excluding the group's home path from prefix-matching; the identical bug was then found independently in [AppLayout.jsx](src/components/AppLayout.jsx)'s mobile bottom nav during verification (confirmed via computed-style check: CAL and STD were both lit at once) and fixed the same way.
- **Report screen only showed the logged-in user's bets** — [F1ReportScreen.jsx](src/screens/f1/F1ReportScreen.jsx) hardcoded `.eq('user_id', user.id)`, a pure client-side choice, not an RLS restriction (`f1_bets_race`/`f1_bets_year`/`f1_scores` all have public-read policies). Rewritten to fetch paddock membership via the existing `get_paddock_leaderboard` RPC and show every member's picks — Race Bets tab lists all members per finished race with the same P1/P2/P3/DNF/Team/Special breakdown; new Year Bets tab shows all members' season picks once `f1_year_results.is_bets_locked` is true, a placeholder before that. No new migration or RPC needed.
- **Standings didn't flag season points as preliminary** — [F1StandingsScreen.jsx](src/screens/f1/F1StandingsScreen.jsx) now reads `f1_year_results.is_final` (already existed in schema, never read) and shows an amber "SEASON PTS ARE PRELIMINARY" banner while false.
- **Combined dashboard request** — folded the existing top-5 leaderboard preview (previously only shown on a separate PADDOCKS tab) into the CALENDAR tab's render, between the hero card and the full race list — no new route/tab, matches the reference platform's single-screen Dashboard without inventing new navigation.

Verified: `npm run lint` (0 errors), `npm run build` (clean, no Rolldown TDZ), `npx playwright test e2e/platform.spec.js` (84/84 passed), plus a full live-browser walkthrough logged in as a real non-creator paddock member — admin gate correctly denied, RESULTS PENDING badges correct, only one tab underline lit on both desktop and mobile nav, Report screen showing all 7 members' race and year bets, Standings preliminary banner rendering, combined dashboard rendering, and Bahrain/Saudi Arabia confirmed removed from the live calendar post-delete.
...
| **L-10** | ✅ **Done 2026-08-28** — E2E coverage for the 14 uncovered screens, plus the 6 screens with none at all | `TEST-E2E-1` | F1 (7 screens) + tennis (7 screens) covered by `e2e/f1-screens.spec.js`/`e2e/tennis-screens.spec.js`. Wallet, Challenge, Trophy Cabinet, Settings, Auth covered by 5 new smoke specs; Draft's gap turned out to be `BUG-DRAFT-SVC` blocking existing `draft-mode-complete.spec.js` coverage from running, not a missing spec — fixed same session. `npm run test:e2e:local` now also wired into CI as a new, non-blocking `e2e-tier3` job. |
...
2. **0 of 22 pg_cron jobs are active** (`DD-P0-2`). Deliberately frozen 2026-07-31 while the site was walled and nothing was live — the right call at the time, but it means *nothing* scores, syncs fixtures/players, resolves bets, or closes auctions. A tester onboarded today would see a permanently static app with no failure message. Reversible, but it must be an explicit step in whatever runbook precedes tester onboarding, not an afterthought.
   - **2026-09-01 update**: re-checked both gating prerequisites while shipping PR #903 (F1 cron automation). Neither is met yet: `DD-P0-3` still has no `status='active'` tournament with a future fixture (the same-day tournament cleanup removed 2 stale rows but added no new forward-looking data to the remaining `1593`/`429`), and `OPS-SENTRY`'s backend `SENTRY_DSN` secret is still unconfigured. Reactivation deliberately not attempted this session — both blockers need their own dedicated work first.
...
> ✅ **Site reopen (Phase 4 cutover: `MAINTENANCE_MODE=false` + redeploy) — done 2026-08-30.** The wall came down and the site is live and publicly reachable again. Per CLAUDE.md's standing instruction, do not re-enable `MAINTENANCE_MODE` in any session without the user explicitly asking in that session. **Cron re-enable status is separate and unconfirmed** — see `DD-P0-2` below; verify before assuming scoring/sync is running.
...
| TEST-P2P-1 | 🟡 **PARTIALLY RESOLVED 2026-08-28, tests fully active 2026-08-30** — **[TECH DEBT] The entire P2P challenge / escrow lifecycle has zero automated tests** | ~2–3 days; core lifecycle done | New `tests/unit/p2p-challenges.test.js` now covers `create_p2p_challenge`, `accept_p2p_challenge`, `decline_p2p_challenge`/`cancel_p2p_challenge` (incl. a direct regression test for the migration-204/235 double-refund bug), `resolve_p2p_challenge` (incl. a regression test for the migration-205/224 tie-double-credit bug), and the full freeform chain (`declare_freeform_result`/`confirm_freeform_result`/`dispute_freeform_result`/`arbitrate_freeform_result`) — all confirmed error codes plus payout-math assertions. Writing this suite immediately surfaced a real, previously-unknown production bug — see resolved row **`BUG-P2P-DISPUTE`** directly below — which is exactly the class of bug this item predicted ("a stuck or double-released stake would be... the most damaging first-cohort bug"). The 6 `dispute`/`arbitrate` tests were initially `.skip()`-marked because of that bug; un-skipped 2026-08-30 once migration `261` fixed it — all 190 unit tests pass, 0 skipped. `submit_bet` also gained coverage this session (extended `tests/unit/bet.test.js`). **Still open**: `auto_resolve_p2p_challenges`, `expire_stale_challenges`, `admin_grant_coins` (cron-context auto-resolvers), `void_bet`, `process_auction_deadlines`, `sweep_void_auction_confirmations`, `resolve_auction_listing`, `submit_trade_proposal`/`accept`/`reject`. |
| ~~BUG-P2P-DISPUTE~~ | ~~**[BUG] `dispute_freeform_result` has never worked in production — violates a CHECK constraint on every call**~~ | — | ✅ **RESOLVED 2026-08-28** — migration `261_fix_p2p_dispute_notification_check.sql` applied live via `npx supabase db query --linked` (backup: `backups/pre_migration_20260828_232502.sql`), adding `'p2p_challenge'` to `clubhouse_notifications_source_type_check`'s allowed values. Verified post-write via `pg_get_constraintdef` — constraint now reads `CHECK ((source_type = ANY (ARRAY['league'::text, 'paddock'::text, 'box'::text, 'clubhouse'::text, 'p2p_challenge'::text])))`. `supabase/schema.sql` updated to match. Original finding, preserved for the record: found 2026-08-28 while writing `TEST-P2P-1`'s freeform-chain tests — `dispute_freeform_result` (migration 239) does `INSERT INTO clubhouse_notifications (..., source_type, ...) VALUES (..., 'p2p_challenge', ...)`, but the constraint only allowed `source_type IN ('league','paddock','box','clubhouse')`, so every real call threw `23514` and rolled back — the freeform P2P dispute path had never functioned in production. Was latent (0 `p2p_challenges` rows in prod), but would have broken the first real dispute once P2P challenges went live. |
| TEST-E2E-1 | 🟡 **PARTIALLY RESOLVED 2026-08-28** — **[TECH DEBT] 14 screens — the whole multi-sport and coins surface — have zero E2E coverage** | ~3–4 days total; F1/tennis half done | **F1 (7 screens) and tennis (7 screens) are now covered** — see `L-10` in the [Launch Plan](#-launch-plan--pilot-relaunch-living-plan-target-2026-09-04) for the session detail. New `e2e/f1-screens.spec.js` + `e2e/tennis-screens.spec.js` run against Tier 3's local full-stack harness (`npm run test:e2e:local`), not `platform.spec.js` (which stays a demo-mode, no-DB smoke test and isn't the right harness for flows that need real seeded Postgres rows and RLS). **Still zero coverage**: `WalletScreen`, `ChallengeScreen`, `TrophyCabinetScreen`, `DraftScreen`, `SettingsScreen`, `AuthScreen`. Original finding, preserved for the record: found 2026-08-21 via grep — the strings `f1`/`tennis`/`wallet`/`challenge`/`paddock` appeared nowhere in `e2e/` except two incidental `TROPHY` assertions in a nav test. |
...
| BUG-DRAFT-SVC | ✅ **FIXED** — **[BUG] `run-draft-lottery` E2E tests invoke the Edge Function with a service-role JWT, which its own auth logic rejects — plus a mismatched `onConflict` target on a `draft_submissions` upsert** | ~2–4h (test-only) | Found 2026-08-28 while building the Tier 3 F1/tennis harness (`L-10`); fixed same session. **Corrected root cause**: this was never a regression — `run-draft-lottery` was never designed to accept a service-role caller alongside `league_id` (DD-C4, migration 108 paired service-role auth only with the now-permanently-disabled cron path, which always 405s). The two spec files were calling an invocation pattern the function never supported. **Fix (test-side only, no Edge Function or migration changes)**: `draft-mode-complete.spec.js` now mints a real auth user via `serviceSupabase.auth.admin.createUser(...)`, grants it `league_members.role = 'commissioner'` for the throwaway per-scenario league, signs in with `signInWithPassword`, and invokes `run-draft-lottery` through that authenticated client. `draft-allocation-e2e.spec.js` instead signs in directly as `USER_A` (`e2e_a@fantasykit.test` / already seeded as commissioner of `DRAFT_LEAGUE_ID` in `supabase/seed.sql`) — simpler than minting a throwaway user since that league and its commissioner are fixed seed fixtures, not per-test. Both files' gating env var renamed `SUPABASE_SERVICE_ROLE_KEY` → `E2E_LOCAL_SERVICE_ROLE_KEY` (matching `scripts/e2e-local.mjs`), un-skipping test bodies that previously always skipped. **(b)** `draft_submissions` upserts fixed: `onConflict: 'league_id,user_id'` → `onConflict: 'league_id,user_id,phase'` with explicit `phase: 'group'` in the payload, matching the real constraint `draft_submissions_league_id_user_id_phase_key` (migration 104). |
...
### 🗒️ Pilot Feedback — Backlog-Worthy Gaps (added 2026-09-06)

Found during the 2026-09-06 pilot feedback coverage audit (`FORZA Feeback Implementation.docx`, 19 items checked against `main`). These are the items with **no code coverage found at all**, plus the still-unstarted half of the player-card visual-identity ask (club badges are already in progress separately — not this list). Full "what's requested / what the code does now / how to test it" detail for all 19 audited items lives in the published coverage report artifact shared with the user; this table exists so these specific gaps are tracked here too, per this file's own single-source-of-truth rule. **Complexity ratings are relative to this codebase, not absolute** — weigh them against everything else already queued above before scheduling any of this.

| # | Item | Complexity | Notes |
|---|------|-----------|-------|
| FB-15 | **[FEATURE] Market sort options (price / form / minutes)** | **Low** — ~1–2 days | Market results render in a fixed price-descending order with no sort control at all. All the underlying fields are already fetched client-side, so this is UI-only: a sort control plus a comparator, no schema or RPC change. The cheapest win in this batch. |
| FB-28 | **[FEATURE] Admin bet management — edit deadline, lock/reopen** | **Medium** — ~3–5 days | Admin control today is limited to `resolve_bet` and `void_bet`, both one-way terminal actions — no way to edit a bet's deadline or lock/reopen a live one. Needs 2–3 new RPCs with state-machine guards (can't edit a resolved/voided bet; can't reopen past a hard cutoff) and new `CommissionerPanel` UI. Bounded, well-understood work — no schema redesign. |
| FB-30 | **[FEATURE] Scale bet payouts by consensus size** | **Medium** — ~2–4 days | `167_resolve_bet_points_aggregation.sql` already computes `winners_count` at settlement but the payout formula ignores it — every correct guess earns the same flat reward regardless of how many others also got it right. The data needed already exists; the work is designing the inverse-scaling formula (and whether it applies retroactively or only to new bets) and wiring it into the existing settlement migration. |
| FB-29 | **[FEATURE] Stake/risk option on commissioner league bets** | **High** — ~1–2 wk | No stake/amount column exists on `bet_submissions` — bets are flat-reward only today. Needs new schema (a stake field plus escrow/ledger handling — the coverage report's flag 04 notes the existing P2P Challenges coin-escrow mechanics could plausibly be extended rather than rebuilt), new stake-setting/accepting UI, and payout-distribution logic. **The source doc itself flags that a real-money variant needs separate compliance review** — this estimate covers the coin/points-based opt-in version only; treat real-money as a distinct, larger item if ever pursued. |
| FB-24 | **[FEATURE] Larger draft squads with bench-only reserve slots (e.g. 20 rostered, 15 count)** | **Very High** — multi-week, blocked | `useLeagueConfig.js` hardcodes `squadSize` to 15 with no beyond-15 reserve concept anywhere in the schema or docs. Rated Very High not because changing a number is hard, but because **the source doc itself flags `squadSize` as a constant shared with the scoring system**, which is currently paused (see the platform revision items above) — implementing this in isolation risks a scoring-layer regression once that system is unpaused. Do not schedule without first deciding the scoring system's status. |
| PCARD-2 | **[FEATURE] Player cards — national flags + player photos** (remaining half of the visual-identity ask; club badges are tracked separately and already in progress) | **High** — asset-sourcing bound, not code-bound | `PlayerCard.jsx` renders name/position/stats as text only. Flags need a nationality field mapped to a flag-icon set (moderate effort). Photos need a licensed image per player across every tracked competition, a storage/CDN pipeline, and a no-photo fallback — that sourcing/rights question is the actual blocker, not implementation time. Recommend scoping flags and photos as two separate efforts rather than one. |
...
| PUSH-1 | **[FEATURE] Mobile push notifications (chat, bets, challenges) via installed PWA — no App Store needed** | 2–3 days (bets/challenges/league notifications); +0.5–1 day if chat is included, see gap noted below | Requested 2026-08-30 — user asked whether notifications are possible without a native app, just the existing "Add to Home Screen" shortcut. **Yes, and free** — Web Push is an open protocol with no paid service or App Store fee; delivery through Apple's/Google's push relays is free, VAPID keys are self-generated. See detail below. |
| BUG-CW1 | **[BUG] `guard_coin_columns()` trigger silently cancels every DELETE on `coin_wallets`, regardless of role** | ~15 min | Found 2026-09-06 while cleaning up a test account for the migration-284 signup fix (see changelog entry above). For a `BEFORE DELETE` trigger, Postgres cancels the operation when the function returns `NULL` — but `guard_coin_columns()` unconditionally does `RETURN NEW`, and `NEW` is always `NULL` in a DELETE context, so **every** delete of a `coin_wallets` row is silently a no-op, even for `postgres`/service-role callers who should be exempt (the function's own `IF current_user IN ('authenticated','anon')` check never matters for DELETE — it always falls through to the no-op `RETURN NEW`). Practical impact: whenever a user is deleted (`auth.users` cascade or otherwise), their `coin_wallets` row is never actually removed — orphaned wallet rows accumulate forever. Fix: change the function to branch on `TG_OP` — `RETURN OLD` for the DELETE case (and only cancel via `RAISE EXCEPTION`, never silently, when blocking `authenticated`/`anon`). Worked around manually for one row via `ALTER TABLE ... DISABLE/ENABLE TRIGGER` around the single DELETE; the trigger itself is still broken. |
| CI-UT1 | **[BUG] `tests/unit/scoring-logic.test.js` red on `main` since PR #943 — CI `Unit Tests` job failing on every push** | ~30 min | Test expectations were never updated when Scoring v3 Phase 1 (PR #943) changed the formula (clean-sheet minute thresholds, conceded-goal penalty, card/own-goal/missed-penalty deductions, shootout scoring). 7/24 assertions fail. Confirmed pre-existing and unrelated to PRs #946–#951 (all merged anyway per their own passing checks) via `gh run list --branch main` (red for every push since PR #943) and a local `node --test tests/unit/scoring-logic.test.js` run — still red as of PR #951, the latest push at session end. Fix: update the test file's expected values to match the live Scoring v3 formula in `supabase/functions/calculate-scores/scoring-logic.js` (source of truth), not the other way around. |

#### PUSH-1 detail — Web Push notifications via installed PWA

**Platform support**: Android has long supported Web Push for installed PWAs. iOS added it in 16.4 (2023), but *only* for a site added to the Home Screen — not for Safari tabs. `public/manifest.json` already has `display: "standalone"` + icons configured, so the app is already installable; only the push-specific pieces below are new. The real-world limiter is discoverability — iOS has no native "install" prompt like Android's `beforeinstallprompt`, so a user has to manually tap Share → Add to Home Screen before push can ever work for them; a UI nudge (detect non-standalone via `navigator.standalone` / `matchMedia('(display-mode: standalone)')`, show an "Add to Home Screen to get notifications" banner) is needed to drive that.

**New pieces needed**:
1. **Service worker** (`public/sw.js` or similar, registered from `main.jsx`) — none exists today. Listens for `push` events → `self.registration.showNotification()`; handles `notificationclick` to deep-link into the relevant screen (chat thread, bet, etc.).
2. **VAPID keypair** — one-time, self-generated (`web-push` npm package locally), stored as a Supabase secret. No third-party account.
3. **New migration** — `push_subscriptions` table (`user_id`, `endpoint`, `p256dh`, `auth`, `created_at`), RLS: a user can insert/delete their own rows only.
4. **Client subscribe flow** — a permission-request toggle (natural home: `SettingsScreen.jsx`) calling `Notification.requestPermission()` then `registration.pushManager.subscribe({ applicationServerKey: VAPID_PUBLIC_KEY })`, upserting the resulting subscription into `push_subscriptions`.
5. **Send-side Edge Function** (`send-push-notification`, new) — the good news: `clubhouse_notifications` (schema.sql, 4 existing `INSERT` sites — `dispute_freeform_result` and 3 others around lines 5665/5693/5720) already centralizes in-app notification fan-out for `p2p_challenge`/`league`/`paddock`/`box`/`clubhouse` source types (bets, challenges, arbitration, etc.). Wiring a Postgres trigger (or a Supabase Database Webhook) on `INSERT INTO clubhouse_notifications` → `send-push-notification` covers **all of those for free** — one integration point, no per-feature wiring.
6. **Chat is the one real gap**: `chat_messages` currently has *zero* integration with `clubhouse_notifications` — chat is pure Supabase Realtime (in-app only, nothing persisted for offline/background delivery). Getting chat pushes needs a new trigger on `chat_messages` insert that either writes a `clubhouse_notifications` row (reusing the pipeline above) or calls `send-push-notification` directly, with sensible scoping (e.g. only when the recipient isn't already an active Realtime subscriber, to avoid double-notifying an open chat window) — this is the +0.5–1 day beyond the base estimate.

**Doesn't affect the native path**: if the app ever ships to the App Store via the existing Capacitor iOS/Android projects, that would use native push (APNs/FCM) — a separate mechanism from Web Push, needing its own wiring later. This item is scoped purely to the current installed-PWA/no-App-Store case.
