...
| **L-10** | 🟡 **F1/tennis done 2026-08-28** — E2E coverage for the 14 uncovered screens | `TEST-E2E-1` | F1 (7 screens) + tennis (7 screens) now covered by `e2e/f1-screens.spec.js`/`e2e/tennis-screens.spec.js` against Tier 3's local full-stack harness — smoke on all 14, plus real interaction specs (race-pick, paddock join, tennis roster + QF-captain, player-box join) and admin/scoring specs asserting `trophy_ledger` writes. **Still zero coverage**: Wallet, Challenge, Trophy, Draft, Settings, Auth. |
...
| BUG-DRAFT-SVC | **[BUG] `run-draft-lottery` E2E tests invoke the Edge Function with a service-role JWT, which its own auth logic rejects — plus a mismatched `onConflict` target on a `draft_submissions` upsert** | ~2–4h | Found 2026-08-28 while building the Tier 3 F1/tennis harness (`L-10`). Two unrelated, pre-existing, out-of-scope bugs surfaced when a service-role key was briefly wired into the shared `SUPABASE_SERVICE_ROLE_KEY` env var that `draft-mode-complete.spec.js`/`draft-allocation-e2e.spec.js` gate their real Edge-Function-invoking test bodies behind (`if (!serviceSupabase) test.skip()` — previously always skipped locally because that env var was never set). **(a)** `supabase/functions/run-draft-lottery/index.js` requires a real, non-service-role commissioner user JWT for any `league_id`-scoped call — `userClient.auth.getUser()` fails for a service-role JWT (its `sub` claim doesn't correspond to a real `auth.users` row) → 401 → `FunctionsHttpError`. Both spec files call the function using the service-role client itself as caller, which cannot work under the function's own auth model. **(b)** `draft-allocation-e2e.spec.js` upserts `draft_submissions` with `onConflict: 'league_id,user_id'`, but the real table constraint doesn't match that column pair — Postgres returns `42P10` ("no unique or exclusion constraint matching the ON CONFLICT specification"), likely missing a `phase` column matching `run-draft-lottery`'s own filtering logic. **Not fixed this pass** — deliberately scoped around by renaming the Tier 3 harness's own service-role env var to `E2E_LOCAL_SERVICE_ROLE_KEY` (see `scripts/e2e-local.mjs`) so these two files return to their prior (skipped) behavior, unaffected by F1/tennis's need for a service-role client. **Fix, when picked up**: either give these tests a real commissioner user JWT (sign in as the seeded league commissioner rather than using the service-role client to call the function), or drop the Edge-Function-invocation half of these tests and assert directly against `run-draft-lottery`'s SQL side; separately, find and use the correct `onConflict` target for `draft_submissions` (check the table's actual unique constraint, likely `league_id,user_id,phase`). |
